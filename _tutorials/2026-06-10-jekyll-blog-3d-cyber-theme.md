...
description: "用 Anthropic 最新发布的 Fable 5 模型把 Chirpy 博客改造成统一的深空赛博 3D 主题：Three.js 环形书架图书馆、全站玻璃拟态皮肤、随鼠标转动的全息阅读面板、3D 翻页过渡、可拖拽的标签星球。既是主题介绍，也是一次对模型能力的试用记录。（后续补记：为可读性砍掉了三个一直在动的效果——随鼠标的卡片倾斜、全息扫描线、阅读进度光束，其余 3D / 玻璃 / 星空全部保留，原因见文末。）"
---
...
> 这一节是上线一段时间后的回访补记。（中途其实先一刀切地把阅读页特效几乎全撤、退回原生暗色，但很快发现砍过头了——绝大多数效果无伤大雅、还挺好看。于是改成精确下掉真正碍事的那几个。）

真当读者天天读下来才体会到：**博客的本质是获取知识，阅读页第一位的诉求是“好读”，不是“好看”。** 但“好看”和“好读”并不天然冲突——上面那套效果里，绝大多数只是入场动一下就安静了，留着无妨；真正在和阅读作对的，是少数几个**一直在动**的，尤其是跟着鼠标动的。最终只砍掉三个：

1. **随鼠标的 3D 倾斜**——卡片跟着指针倾斜+眩光，左右侧栏与右侧 TOC 面板静止时斜立、鼠标移上去转正、移开又斜回去，逐字阅读时挥之不去；
2. **全息扫描线**——正文玻璃板上来回扫的那道光，逼着眼睛反复重新聚焦；
3. **阅读进度光束**——顶部那条进度条，纯装饰，却始终在视野边缘动。

其余一律保留：青蓝赛博配色、星空背景、玻璃拟态、霓虹渐变、悬浮玻璃阅读面板（去掉扫描线后是静态的）、滚动透视入场、3D 翻页过渡，以及书架 / 火箭 / 魔方 / 标签星球 / 太空 404。

判断标准很简单：**一次性或入场即停的效果可以留（点一下、滑入一次就安静下来），持续运动、尤其跟着鼠标动的要砍**——后者是阅读时挥之不去的注意力税。实现上仍是原文“换肤优于重写”的便利：只删掉 `cyber-depth.js` 里的两段（卡片倾斜、进度光束）和 `cyber-skin.scss` 里对应的 `.tilt-card`、全息扫描线、`#reading-progress`，其余分毫未动。

留作记录的一条经验：**“炫酷”和“可读”不是非此即彼，真正伤阅读的往往只是少数几个“一直在动”的效果**；与其全撤或全留，不如挑出那几个持续动的精确下掉。而哪个动得碍眼，只有自己长期当读者用一阵子才分得清——这一点没有无头测试能替你回答。
