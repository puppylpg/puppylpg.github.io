...
description: "一次完整的前端排查实录：用无头 Chromium + CDP 搭建截图验证闭环，定位 Mermaid 文字被裁的根因是 webfont 加载时序，并顺手发现暗色主题对比度、fonts.ready 挂起，以及它在冷加载下假性提前 resolve 导致首屏错位等一连串隐藏缺陷。"
---
