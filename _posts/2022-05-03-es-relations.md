...
tags: [elasticsearch]
description: "Elasticsearch 处理关系型文档的方式：object、nested、parent-child 和反规范化取舍。"
---
...
es对关系型数据的处理方式可以参考[权威指南：处理关联关系](https://www.elastic.co/guide/cn/elasticsearch/guide/current/relations.html)。

| 模型 | 物理存储 | 查询代价 | 更新代价 | 适合场景 |
| --- | --- | --- | --- | --- |
| `object` | flatten 成多个字段 | 最低 | 最低 | 一对一、不会出现对象数组交叉匹配 |
| `nested` | 父文档 + 隐藏子文档，同 segment | 较低 | 更新任一子对象要重建整条父文档 | 子对象一起写、一起更新不频繁 |
| `join` parent-child | 独立父子文档，同 shard | 较高 | 父子可独立更新 | 子文档很多或频繁更新 |
| denormalization | 冗余多份文档 | 最低 | 维护冗余成本最高 | 多对多、读多写少 |
...
```mermaid
flowchart LR
    A["[{first: Lucy, last: James}, {first: Kate}]"] --> B[object flatten]
    B --> C["manager.name.first = [Lucy, Kate]"]
    B --> D["manager.name.last = [James]"]
    C --> E[查询 first=Kate AND last=James 可能误命中]
    D --> E
...
```mermaid
flowchart TD
    A[逻辑文档 user + blogs[]] --> B[Lucene segment]
    B --> C[hidden nested doc: blog 1]
    B --> D[hidden nested doc: blog 2]
    B --> E[root doc: user]
    C --> F[nested query 先命中子文档]
    D --> F
    F --> G[回到 root doc 返回整条文档]
...
```mermaid
flowchart LR
    P[parent user u1] -->|routing=u1| S[shard 3]
    C1[child blog b1] -->|routing=u1| S
    C2[child blog b2] -->|routing=u1| S
    S --> Q[has_child / has_parent 不跨节点 join]

    style S fill:#e8f5e9,stroke:#51cf66
    style Q fill:#fff3bf,stroke:#f59f00
```
