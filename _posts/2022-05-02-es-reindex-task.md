...
tags: [elasticsearch]
description: "Elasticsearch alias、reindex 和 task API 笔记，串起索引版本切换、异步任务和脚本迁移。"
---
...
- [alias 官方文档](https://www.elastic.co/guide/en/elasticsearch/reference/8.1/aliases.html)
- [权威指南：索引别名](https://www.elastic.co/guide/cn/elasticsearch/guide/current/index-aliases.html)

```mermaid
flowchart LR
    A[应用只访问 my_index alias] --> B[my_index_v1]
    B --> C[新建 my_index_v2]
    C --> D[reindex / 脚本改结构 / 校验数据]
    D --> E[一次 _aliases 原子切换]
    E --> F[应用无感访问 my_index_v2]
...
reindex 的本质是一次“读旧索引 snapshot -> bulk 写新索引”的任务。它不是移动 segment，也不是直接复制磁盘文件，所以 mapping 变更、字段重命名、脚本清洗都可以塞在这个过程中；代价也在这里：所有目标文档都要重新走一遍索引链路。

```mermaid
sequenceDiagram
    participant C as Client
    participant ES as ES task
    participant S as source index
    participant D as dest index

    C->>ES: POST _reindex?wait_for_completion=false
    ES->>S: scroll/snapshot 读取旧文档
    ES->>ES: 可选 painless script 改 _source/_id/_routing
    ES->>D: bulk index 到新索引
    C->>ES: GET _tasks/<task_id>
    ES-->>C: created / batches / version_conflicts
```
...
| 调整项 | 加速点 | 代价 |
| --- | --- | --- |
| 增大或关闭 `refresh_interval` | 少建 segment，少 refresh | 新数据暂时不可搜索，Kibana count 可能看起来不动 |
| `number_of_replicas=0` | 目标索引只写 primary | reindex 期间容灾能力下降，结束后要恢复副本 |
| 异步 task | 客户端不等长连接 | 要自己轮询 task 并处理失败 |
| 合理的 slices/batch | 提高并行度 | 并行过头会压垮源集群或目标集群 |
