...
tags: [spring-security, csrf, cookie, samesite, browser-security]
description: "从浏览器自动携带凭据的行为出发，理解 CSRF 攻击、同步 token、SameSite 和 Spring Security 的防护边界。"
---

Spring Security 默认会开启一组[常见攻击防护](https://docs.spring.io/spring-security/reference/features/exploits/index.html)，其中最典型的是 CSRF，Cross Site Request Forgery，跨站请求伪造。理解 CSRF 的关键不是“别人偷到了 cookie”，而是“浏览器会自动把已有凭据带给目标站点”。

1. Table of Contents, ordered
{:toc}

文章的主线是：**CSRF 利用了浏览器自动携带认证凭据的行为；防护策略要么要求请求带上外站拿不到的 token，要么限制跨站请求携带 cookie**。

CSRF 成立和 STP 防护的区别，可以用同一个请求对比出来：

```mermaid
flowchart TD
    User["用户浏览器<br/>已登录 bank.example"] --> Cookie["浏览器自动携带 Cookie"]
    Evil["evil.example 页面"] --> Forge["伪造 POST /transfer"]
    Forge --> Cookie
    Cookie --> Server{"服务端只看 Cookie？"}
    Server -->|"是"| Attack["请求看起来合法<br/>CSRF 成功"]
    Server -->|"否，需要 CSRF token"| NeedToken{"请求体/header 有正确 token？"}
    NeedToken -->|"恶意站点拿不到 token"| Block["拒绝请求"]
    NeedToken -->|"本站页面提交"| Pass["允许修改状态"]

    classDef browser fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef attack fill:#ffe3e3,stroke:#c62828,color:#7f1d1d
    classDef protect fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef decision fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    class User,Cookie browser
    class Evil,Forge,Attack attack
    class NeedToken,Block,Pass protect
...
`Strict`策略虽然安全，但是也会造成一些困惑，所以`Lax`策略可能更好：比如一个用户已经登录twitter了，此时从google邮箱点击twitter主页地址，如果twitter用了strict策略，这个从邮箱发送的请求不会带上cookie信息，用户打开的twitter还是未登录状态，就比较迷惑。
...
> 只要我不用 cookie，就不怕别人偷 cookie :D

还真有人这么想：[Is CSRF possible if I do not even use cookies?](https://security.stackexchange.com/questions/62080/is-csrf-possible-if-i-dont-even-use-cookies)

但是并不完全能防止 CSRF：[Is CSRF possible without cookies?](https://stackoverflow.com/questions/3732087/is-csrf-possible-without-cookies)
...
# 感想
CSRF 的重点不在 cookie 这个存储介质本身，而在浏览器是否会替用户自动携带某种认证凭据。只要请求能被浏览器自动拼成“看起来合法”的样子，服务端就需要额外手段区分它是不是来自可信页面。
