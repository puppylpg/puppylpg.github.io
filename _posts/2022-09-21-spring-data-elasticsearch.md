...
categories: [tech, elasticsearch, spring-data, spring-data-elasticsearch]
tags: [elasticsearch, spring-data, spring-data-elasticsearch, repository, routing]
description: "围绕 Spring Data Elasticsearch 的 mapping、routing、_id、repository、scroll、save 与 update，整理常见机制和踩坑点。"
---

[Spring Data](https://spring.io/projects/spring-data) 的目标是减少数据访问层的样板代码。落到 Elasticsearch 上，它一方面保留了 Spring Data 的 repository、mapping、converter 等通用抽象，另一方面又不得不面对 Elasticsearch 自身的 `_id`、routing、mapping、scroll、refresh 等机制。

> 后续系统梳理见 [Spring Data]({% post_url 2023-12-20-spring-data %})。

1. Table of Contents, ordered
{:toc}

本文按问题展开：先补 Spring Data 的基础抽象，再进入 Spring Data Elasticsearch 的 mapping、routing、`_id` 与 `_source` 关系，随后记录 repository、自定义 repository、scroll、`save`、`update` 等常见场景。

Spring Data Elasticsearch 最大的理解成本，是通用抽象和 Elasticsearch 特有机制叠在了一起：

```mermaid
flowchart TD
    Entity["Java Entity"] --> Mapping["Object Mapping<br/>@Document / @Field / @Id"]
    Mapping --> Source["_source<br/>业务字段"]
    Mapping --> Meta["_id / _routing<br/>ES 元信息"]
    Repository["Repository 接口"] --> QueryDerive["方法名派生查询"]
    Repository --> Custom["Custom Repository<br/>手写 Query / Update"]
    QueryDerive --> Template["ElasticsearchOperations / Template"]
    Custom --> Template
    Template --> Request["ES Request<br/>search / scroll / index / update"]
    Source --> Request
    Meta --> Request
    Request --> ES[(Elasticsearch)]

    classDef java fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef spring fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef es fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    class Entity,Repository java
    class Mapping,QueryDerive,Custom,Template spring
    class Source,Meta,Request,ES es
```

# spring data
Spring Data 的核心文档是 [Spring Data Commons reference](https://docs.spring.io/spring-data/commons/docs/current/reference/html/)，其中 repository 的使用方法可以重点看 [repositories](https://docs.spring.io/spring-data/commons/docs/current/reference/html/#repositories) 一章。

## object mapping
类似于 ORM，第一步是创建 Object Mapping，告诉 Spring Data 怎么把对象和数据库里的数据映射。通用规则见 [Spring Data Commons mapping fundamentals](https://docs.spring.io/spring-data/commons/docs/current/reference/html/#mapping.fundamentals)，Elasticsearch 的规则见 [Spring Data Elasticsearch mapping](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.mapping)。

## repository
spring data的核心就是repository了。

创建 repository 时，通用规则见 [Spring Data Commons repositories](https://docs.spring.io/spring-data/commons/docs/current/reference/html/#repositories)，Elasticsearch 的扩展见 [Spring Data Elasticsearch repositories](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.repositories)。

实现一个interface，继承spring data的repository：
1. 就自动具有了一堆增删改查方法；
2. 还可以只写方法名字，让spring data根据方法名自动生成方法实现；
3. 使用`@Query`直接把 query 绑定到方法上，见 [Elasticsearch query methods](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.query-methods.finders)；
3. 还可以继承spring data已经实现好的分页返回数据功能；
4. 返回 stream 也是一个不错的方法，可以避免一次性接收大数据，撑爆内存，见 [Streaming query results](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#repositories.query-streaming)。

关于分页的一些示例：
- [Baeldung spring-data-elasticsearch 示例](https://github.com/eugenp/tutorials/tree/master/persistence-modules/spring-data-elasticsearch)
    + [FrontBackend Thymeleaf 分页示例](https://frontbackend.com/thymeleaf/spring-boot-bootstrap-thymeleaf-pagination-jpa-liquibase-h2)
    + [Martin Wojtuś Thymeleaf 示例](https://github.com/martinwojtus/tutorials/tree/master/thymeleaf)

当需要自定义实现一些方法时，可以自己实现repository，并通过spring data默认的约定，将实现类自动整合到repository里：
- [Custom Repository Implementations](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#repositories.custom-implementations)
...
这个问题可参考 [CrudRepository 不支持 routing 的讨论](https://stackoverflow.com/questions/73781461/default-spring-data-elasticsearch-crudrepository-doesnt-support-routing)。
...
- [How to provide a dynamic index name in Spring Data Elasticsearch using SpEL](https://www.sothawo.com/2020/07/how-to-provide-a-dynamic-index-name-in-spring-data-elasticsearch-using-spel/)
...
- [Add field to existing index 的讨论](https://stackoverflow.com/questions/70189254/spring-data-elasticsearch-7-9-3-add-field-to-existed-index)

所以不如就自己动手创建吧。

## type hint
type hint 的规则见 [Mapping Rules](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.mapping.meta-model.rules)。
...
具体规则见 [Routing Values](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.routing)。
...
这个行为可参考 [`id` vs `_id` 的讨论](https://stackoverflow.com/questions/37277017/spring-data-elasticsearch-id-vs-id)。

**如果不设置这个id的值，直接把对象存入es，spring data
 elasticsearch往`_source`里写的`id` field值就是null。又因为es会自动给没有`_id`值的文档生成个`_id`值，所以存入后`_id`有值，`id`为null**。在get的时候，**getId方法返回的是`_id`的值**，所以返回有值，而不是null，见这个 [Stack Overflow 回答](https://stackoverflow.com/a/37277492/7676237)。
...
这个 workaround 见 [Using `@Id` forces id field in source](https://stackoverflow.com/questions/62765711/spring-data-elasticsearch-4-x-using-id-forces-id-field-in-source)。
...
相关背景见 [spring-data-elasticsearch issue 2230 的评论](https://github.com/spring-projects/spring-data-elasticsearch/issues/2230#issuecomment-1319230419)。
...
跟踪记录见 [spring-data-elasticsearch issue 2364](https://github.com/spring-projects/spring-data-elasticsearch/issues/2364)。
...
- `@Transient`：该注解会起作用。行为就是：这个字段什么也不干，不读不写，也不会写入mapping；
...
- [Spring Data Elasticsearch 中 `@Id` 和 `_id` 的说明](https://juejin.cn/post/6844904068037476365)
...
- [`_id`和`id`同时存在的讨论](https://stackoverflow.com/questions/62029613/set-different-id-and-id-fields-with-spring-data-elasticsearch)
...
```
自定义 converter 一般用不到，具体机制见 [Elasticsearch Object Mapping Conversions](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.mapping.meta-model.conversions)。

还能spel……离谱……

另外，既然 es 的 field 可以是单值或 list，那无论 es 里存储的是单值还是 list，都应该能转为`List`/`Set`。但是之前 Spring Data Elasticsearch 不支持这一点，如果将单值转成 list/set，会报错，见 [spring-data-elasticsearch issue 2280](https://github.com/spring-projects/spring-data-elasticsearch/issues/2280)。
...
- [Elasticsearch null value](https://www.elastic.co/guide/en/elasticsearch/reference/current/null-value.html)
...
- [部分更新忽略 null 的回答一](https://stackoverflow.com/a/63895726/7676237)
- [部分更新忽略 null 的回答二](https://stackoverflow.com/a/63685474/7676237)
...
相关讨论见 [spring-data-elasticsearch issue 1494](https://github.com/spring-projects/spring-data-elasticsearch/issues/1494)。

### join type
甚至还支持es的join（话说回来，它不支持elasticsearch支持谁……）：
- [Join-Type implementation](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.jointype)
...
- [Elasticsearch date formats](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#elasticsearch.mapping.meta-model.date-formats)
...
- [Incompatible date formats](https://www.elastic.co/guide/en/elasticsearch/reference/current/migrate-to-java-time.html#java-time-migration-incompatible-date-formats)

era - There are two eras, 'Current Era' (CE) and 'Before Current Era' (BCE)。前者用AD表示，后者用BC。y只能表示CE的year，正整数。而u能表示广义的year，比如-1年。

> 另外需要注意，0 year 等同于 1 AD，因为使用 era 的人没有 0 的概念，就像楼房没有 0 层；这个细节可参考这个 [Stack Overflow 回答](https://stackoverflow.com/a/29014580/7676237)。

## analyzer
spring data elasticsearch甚至还能在创建index的时候加入analyzer：
- [Create custom analyzer with asciifolding filter](https://stackoverflow.com/questions/63810021/create-custom-analyzer-with-asciifolding-filter-in-spring-data-elasticsearch)
...
> 开启 tracer 可以参考这个 [Stack Overflow 回答](https://stackoverflow.com/a/68737018/7676237)。
...
- [Partial update with Spring Data Elasticsearch Repository](https://stackoverflow.com/questions/40742327/partial-update-with-spring-data-elasticsearch-repository)
- [Spring Data Elasticsearch 局部更新示例](https://www.jianshu.com/p/b320ace6db2f)
...
Here it is:
- [issue 2304](https://github.com/spring-projects/spring-data-elasticsearch/issues/2304)
- [PR 2305](https://github.com/spring-projects/spring-data-elasticsearch/pull/2305)
- [PR 2310](https://github.com/spring-projects/spring-data-elasticsearch/pull/2310)
...
- [Client logging](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/index.html#elasticsearch.clients.logging)

底层的client：
- [Elasticsearch Clients](https://docs.spring.io/spring-data/elasticsearch/docs/current/reference/html/#reference)

# 长连接

[Spring Boot PR 32051](https://github.com/spring-projects/spring-boot/pull/32051) 里讨论了相关支持。
...
具体见 Spring Boot 文档里的 [Elasticsearch support](https://docs.spring.io/spring-boot/docs/current/reference/html/data.html#data.nosql.elasticsearch)。
...
> 也可能是对 Spring Data 本身的理解还不够深入，所以学习成本才会显得这么高。
