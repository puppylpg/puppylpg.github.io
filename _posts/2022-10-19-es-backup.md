...
description: "Elasticsearch snapshot/restore 到 HDFS 的实战记录，包含 repository、SLM、增量备份和跨集群恢复。"
---
...
```mermaid
flowchart TD
    A[安装 repository-hdfs 插件并重启] --> B[PUT _snapshot/<repo> 配置 HDFS repository]
    B --> C[手动 snapshot 或 SLM policy]
    C --> D[复制不可变 segment 到 HDFS]
    D --> E[后续 snapshot 只复制新增 segment]
    E --> F[restore 时按 snapshot 元数据恢复索引]
...
```mermaid
flowchart LR
    A[snapshot-1<br/>seg A + seg B] --> R[repository]
    B[snapshot-2<br/>seg A + seg B + seg C] --> R
    C[删除 snapshot-1] --> D{seg A/B 是否仍被 snapshot-2 引用?}
    D -->|是| E[保留共享 segment]
    D -->|否| F[删除独占 segment]
...
restore 的关键约束是“目标索引名不能已经打开且存在”。所以恢复前要么删掉原索引，要么用 `rename_pattern` / `rename_replacement` 换个名字恢复；跨集群迁移时，先把 repository 配成只读，避免两个集群同时写同一个仓库。
