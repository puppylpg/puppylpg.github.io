...
categories: [tech, springboot]
tags: [springboot, springapplication, autoconfiguration, environment]
description: "沿着 SpringApplication.run 启动链路，拆解 Spring Boot 如何准备环境、加载配置、创建 ApplicationContext 并触发自动配置。"
---

写 Spring Boot 测试时，很容易只接触到 `@SpringBootTest`、slice test 和自动配置结果，却没有看清 `SpringApplication.run()` 本身做了什么。顺着启动入口 debug 一遍，会发现 Spring Boot 的流程并不神秘：它先准备一组 Spring 还没接管的启动组件，再创建环境、加载配置、创建 `ApplicationContext`，最后把控制权交还给 Spring 的 `refresh()`。

1. Table of Contents, ordered
{:toc}

本文沿着 `SpringApplication.run()` 展开：先解释启动类为什么也是配置类，再看 `SpringFactoriesLoader` 如何在 `ApplicationContext` 之前加载 listener 和 initializer，最后落到 environment、profile、配置文件和 auto-configuration 的加载顺序。

`SpringApplication.run()` 的节奏可以先看成“Spring Boot 外层准备 + Spring 内层刷新”两段：

```mermaid
flowchart TD
    Main["main(args)"] --> New["new SpringApplication<br/>推断 web 类型 / mainClass"]
    New --> SPI["SpringFactoriesLoader<br/>加载 listener / initializer"]
    SPI --> Start["run(args)"]
    Start --> Env["prepareEnvironment<br/>命令行参数、profile、配置文件"]
    Env --> Ctx["createApplicationContext"]
    Ctx --> Init["applyInitializers"]
    Init --> Load["load sources<br/>启动类注册为 BeanDefinition"]
    Load --> Refresh["refresh(context)<br/>回到 Spring 核心启动流程"]
    Refresh --> Runners["callRunners"]
    Runners --> Finished["发布 started / ready / failed 等事件"]

    classDef boot fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef spring fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef event fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    class Main,New,SPI,Start,Env boot
    class Ctx,Init,Load,Refresh,Runners spring
...
```mermaid
flowchart TD
    A["SpringApplication.run(SourceClass, args)"] --> B["new SpringApplication(sources)"]
    B --> C["initialize()"]
    C --> C1["保存 source（启动类=配置类）"]
    C --> C2["deduceWebEnvironment()"]
    C --> C3["SPI 加载 ApplicationContextInitializer"]
    C --> C4["SPI 加载 ApplicationListener"]
    C --> C5["deduceMainApplicationClass()"]
    C1 & C2 & C3 & C4 & C5 --> D[".run(args)"]
    D --> E["started 事件"]
    E --> F["准备 Environment"]
    F --> G["environmentPrepared 事件"]
    G --> H["创建 ApplicationContext"]
    H --> I["contextPrepared 事件"]
    I --> J["加载 Bean 定义"]
    J --> K["contextLoaded 事件"]
    K --> L["refresh()"]
    L --> M["执行 CommandLineRunner"]
    M --> N["finished 事件"]
```
...
5. 推断 main 函数所在的类：这个思路很野，直接`new RuntimeException().getStackTrace()`获取栈帧，然后遍历 stacktrace，判断哪个栈帧的 method 是 main，它对应的类就是启动类；
...
```mermaid
flowchart LR
    A["SpringFactoriesLoader"] --> B["扫描 classpath 下所有\nMETA-INF/spring.factories"]
    B --> C["按 key 读取 value\n（Properties 格式）"]
    C --> D1["key=ApplicationContextInitializer\n→ 实例化并缓存"]
    C --> D2["key=ApplicationListener\n→ 实例化并缓存"]
    C --> D3["key=SpringApplicationRunListener\n→ 实例化并缓存"]
    C --> D4["key=EnableAutoConfiguration\n→ 实例化并缓存"]
    D1 & D2 & D3 & D4 --> E["返回给调用方"]
```
...
那ApplicationContextInitializer为什么也急着用SPI初始化？大概是ApplicationContextInitializer的设计就是在ApplicationContext完全准备好之前就需要发挥作用，所以没法等到ApplicationContext创建好之后再有。按照它的使用场景来看，这个接口好像是springboot定义的，其实它是spring定义的。spring用它主要是初始化web的ApplicationContext。
...
sequenceDiagram
    ...
    participant L as EventPublishingRunListener
    participant C as ApplicationContext

    R->>L: started()
    R->>R: 创建 Environment<br/>(StandardServlet / Standard)
    R->>L: environmentPrepared()
    Note over R: ConfigFileApplicationListener<br/>在此加载 application.yml
    R->>R: 打印 Banner
    R->>R: 创建 ApplicationContext
    R->>C: setEnvironment(Environment)
    R->>C: registerShutdownHook()
    R->>L: contextPrepared()
    R->>R: 加载 Bean 定义<br/>（从 source 类开始）
    R->>L: contextLoaded()
    R->>C: refresh()
    R->>R: 执行 CommandLineRunner
    R->>L: finished()
```
...
flowchart TD
    subgraph Servlet 环境
        S1["servletConfigInitParams"]
        S2["servletContextInitParams"]
    end
    subgraph 通用
        C1["commandLineArgs<br/>（--spring.profiles.active=prod）"]
        C2["systemProperties<br/>（-Dspring.xxx=yyy）"]
        C3["systemEnvironment<br/>（SPRING_XXX=yyy）"]
        C4["application-{profile}.yml"]
        C5["application.yml"]
    end
    S1 --> S2 --> C1 --> C2 --> C3 --> C4 --> C5
...
> 取值时从链头往尾遍历，先找到的就返回，所以**位置越靠前优先级越高**。
...
> 为什么 springboot 的 args 要写成：`--spring.profiles.active=prod`？因为 springboot 使用的是 Spring 的`CommandLineArgs`来解析 args，所以它就得以`--`开头，具体可参考这个 [Stack Overflow 回答](https://stackoverflow.com/a/37439625/7676237)。
...
拿到profiles之后，配置了一个：
- **名为`configurationProperties`的`ConfigurationPropertySources`**：配置在最开头，我也不知道有什么用……
...
> - [Profiles 文档](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.profiles)
...
所以都可以通过什么set profiles？只要不是profiles-specific properties就行。比如系统环境变量、系统properties，或者用户的命令行参数**！也就是之前说的args。**ConfigurableEnvironment有一堆PropertySource，命令行参数也是PropertySource的一种！所以命令行参数就作为一种PropertySource注册到ConfigurableEnvironment上了。
...
```mermaid
flowchart TD
    A["source = 启动类（@SpringBootApplication）"] --> B{"source 类型？"}
    B -->|Class| C["AnnotatedBeanDefinitionReader<br/>.register(source)"]
    B -->|Resource| D["XmlBeanDefinitionReader"]
    B -->|Package| E["ClassPathBeanDefinitionScanner"]
    B -->|CharSequence| F["按资源路径加载"]
    C --> G["判断 isComponent()"]
    G -->|"含 @Component\n（@Configuration 也是）"| H["注册为 BeanDefinition"]
    G -->|否| I["跳过"]
    D & E & F & H --> J["ApplicationContext 拥有完整 Bean 定义"]
```
```java
...
```
创建BeanDefinitionLoader：它是xml、java config的聚合体，里面XmlBeanDefinitionReader/AnnotatedBeanDefinitionReader/GroovyBeanDefinitionReader都有，所以什么类型的配置都能加载。它里面还有一个ClassPathBeanDefinitionScanner，从classpath扫描“component class”（同时创建一个包含启动类的ClassExcludeFilter，放防止自己再扫描一遍，无限递归了……）；
...
发送 `finished` 事件！完结，撒花，打印 startup log（同样可以设置为不打印）。

# springboot
学习spring的非常好的工程！相当于帮忙从大框架上梳理spring了！

`spring.factories` 里还有很多其他类的配置，它们不是统一在某个时刻全部加载，而是由不同组件在需要时通过 `SpringFactoriesLoader` 按 key 读取：想什么时候加载就什么时候加载，反正是 SPI。
...
flowchart TD
    ...
    C --> D["DeferredImportSelector<br/>——所有 @Configuration 处理完后再执行"]
    D --> E["SPI 加载 META-INF/spring.factories<br/>key=EnableAutoConfiguration"]
    E --> F["得到候选 AutoConfiguration 类列表"]
...
**DeferredImportSelector** 是关键：它保证 auto-configuration 在所有用户自定义的 `@Configuration` 处理完之后才执行，这样 `@Conditional` 判断才能基于用户已定义的 bean 做出正确决策。
...
springboot test autoconfig的ApplicationContextRunner其实也相当于自己配置了一个ApplicationContext，详见 [Testing your Auto-configuration](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.developing-auto-configuration.testing)。
...
- 现在的测试使用 [ApplicationContextRunner 示例](https://github.com/spring-projects/spring-boot/blob/main/spring-boot-project/spring-boot-autoconfigure/src/test/java/org/springframework/boot/autoconfigure/jackson/JacksonAutoConfigurationTests.java)；
- 以前的测试直接使用 [ApplicationContext 示例](https://github.com/spring-projects/spring-boot/blob/2.7.x/spring-boot-project/spring-boot-autoconfigure/src/test/java/org/springframework/boot/autoconfigure/jackson/JacksonAutoConfigurationTests.java)；
- autoconfig 也有[集成测试示例](https://github.com/spring-projects/spring-boot/blob/2.7.x/spring-boot-project/spring-boot-autoconfigure/src/test/java/org/springframework/boot/autoconfigure/elasticsearch/ElasticsearchRestClientAutoConfigurationIntegrationTests.java)，这里用到了 Docker 环境。
