...
description: "从 JAR 的 ZIP/Manifest 机制讲到 Maven jar、assembly、shade 和 Spring Boot 可执行包的打包差异。"
---
...
Java 打包这事儿看起来名字一堆，核心其实就两条线：

1. **JAR 本质是 ZIP + Manifest**，能不能 `java -jar`，关键看 `META-INF/MANIFEST.MF` 里有没有合适的入口；
2. **Maven 插件决定依赖怎么塞进去**：只打自己、拆开第三方 jar、重定位冲突类，或者像 Spring Boot 那样把 jar 嵌 jar 里。

```mermaid
flowchart TD
    A["src/main/java<br/>src/main/resources"] --> B["maven 编译<br/>target/classes"]
    B --> C["maven-jar-plugin<br/>只打本工程 class/resource"]
    B --> D["maven-assembly-plugin<br/>依赖 jar 解压成 class"]
    B --> E["maven-shade-plugin<br/>解压依赖并可 relocation"]
    B --> F["spring-boot-maven-plugin<br/>BOOT-INF/classes + BOOT-INF/lib"]
    C --> C1["dependency jar<br/>给别人依赖"]
    D --> D1["fat jar<br/>java -jar 可跑"]
    E --> E1["fat jar<br/>还能处理包名冲突"]
    F --> F1["可执行 boot jar<br/>JarLauncher 加载嵌套 jar"]
```

| 插件 | 依赖怎么处理 | 能否直接 `java -jar` | 典型用途 |
|---|---|---|---|
| `maven-jar-plugin` | 不打第三方依赖 | 默认不能 | 发布 library，让别人依赖 |
| `maven-assembly-plugin` | 把依赖 jar 解压为 class 混进一个 jar | 可以 | 简单粗暴 fat jar |
| `maven-shade-plugin` | 解压依赖，还能 relocation 改包名 | 可以 | 处理依赖冲突的 fat jar |
| `spring-boot-maven-plugin` | 依赖保持 jar 形态放进 `BOOT-INF/lib` | 可以 | Spring Boot 应用发布 |
...
参考 [Oracle JAR manifest entry point 文档](https://docs.oracle.com/javase/tutorial/deployment/jar/appman.html)。
...
参考 [Oracle JAR Class-Path 文档](https://docs.oracle.com/javase/tutorial/deployment/jar/downman.html)。

**如果想引用jar里嵌套的jar，必须自己写代码来实现这套逻辑**。就像`spring-boot-loader`做的一样：
参考 [Stack Overflow 上关于 Manifest `Class-Path` 和 Spring Boot Loader 的讨论](https://stackoverflow.com/questions/66023001/does-jar-manifest-support-class-path-why-use-spring-boot-loader-instead)。
...
如果有兴趣，可以看看一个demo：
参考 [Oracle Jar-related API 示例](https://docs.oracle.com/javase/tutorial/deployment/jar/apiindex.html)。
...
shade插件比assembly强的地方在于，用户打jar包的时候可以按照意愿，将一些依赖class所在的package改名（修改字节码）。这样修改后的类和原来的类就不会同包名了，可以做到同时存在。具体操作见 Maven Shade Plugin 的 [class relocation 示例](http://maven.apache.org/plugins/maven-shade-plugin/examples/class-relocation.html)。
...
- 优点：第三方依赖以jar包的形式嵌入到jar里，看起来相当清晰；
...
参考 [Spring Boot executable jar 格式文档](https://docs.spring.io/spring-boot/docs/current/reference/html/appendix-executable-jar-format.html)。

## 总结
- jar plugin：把工程打为依赖；
- assembly plugin：把工程以纯class文件的形式打成可执行jar包；
- shade plugin：同assembly，可以修改类的package name，解决类冲突；
- spring-boot-maven-plugin：把工程代码和第三方依赖分类打成可执行jar包。为此，自己实现了从jar内的jar里load class的逻辑。

# Ref
参考 [Oracle JAR 教程](https://docs.oracle.com/javase/tutorial/deployment/jar/index.html)。
