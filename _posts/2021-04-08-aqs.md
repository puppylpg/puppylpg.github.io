...
description: "从 ReentrantLock 入手拆解 AQS 的 state、等待队列、独占/共享获取和公平/非公平差异。"
---
...
```mermaid
flowchart TD
    Caller["线程调用 lock/acquire"] --> Try{"tryAcquire / tryAcquireShared"}
    Try -->|成功| Own["占用 state<br/>独占或共享"]
    Try -->|失败| Enqueue["加入 AQS CLH 队列"]
    Enqueue --> Park["LockSupport.park 挂起"]
    Release["release / unlock"] --> TryRelease{"tryRelease / tryReleaseShared"}
    TryRelease -->|释放成功| Unpark["唤醒队列后继节点"]
    Unpark --> Try

    style Try fill:#e3f2fd,stroke:#1976d2
    style Enqueue fill:#fff3bf,stroke:#f59f00
    style Own fill:#e8f5e9,stroke:#2e7d32
    style Park fill:#ffe3e3,stroke:#c62828
```
...
Ref：[美团技术团队对 AQS 的原理与应用总结](https://tech.meituan.com/2019/12/05/aqs-theory-and-apply.html)。
