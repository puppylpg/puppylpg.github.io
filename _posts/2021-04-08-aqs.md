...
## `tryLock(long, TimeUnit)` - 有限阻塞可中断获取锁：试一下，不成我再试，时间到了就撤
...
- 对非公平锁来讲：试着抢一下，抢到就撤，抢不到在timeout时间内一直在抢；
- 对公平锁来讲：试着**公平抢一下**，拿到就撤，拿不到在timeout时间内一直在抢；
...
# CountDownLatch - 共享锁
ReentrantReadWriteLock里的ReadLock不是一个独立的共享锁，是和WriteLock相关的。

CountDownLatch的实现有点儿绕，其实是借了AQS的壳，但是并没有所谓的“加锁解锁”逻辑，因为在它的逻辑里，没有“锁”这个东西。

CountDownLatch的语义：
- await：等着，其实就是线程挂起；
- countdown：倒数一次，数到0，所有await的线程都可以执行了。

CountDownLatch既然用AQS实现，await要挂起线程，那么AQS什么时候挂起线程？当线程要获取锁却又没有的时候。所以await就是“获取锁”，用了acquire shared：
```
    public void await() throws InterruptedException {
        sync.acquireSharedInterruptibly(1);
    }
```
acquireShared用到了tryAcquireShared，所以CountDownLatch的tryAcquireShared让他们全都挂起：
```
...
countdown就是让线程执行。AQS什么时候让线程执行？获取锁成功的时候。所以countdown就是释放锁其实就是“释放锁”，用了release shared：
```
    public void countDown() {
        sync.releaseShared(1);
    }
```
releaseShared用到了tryReleaseShared，所以CountDownLatch的tryReleaseShared把计数器-1：
```
...
所以对于计数器为1的latch，减一次，所以state == 0的条件就都满足了，其实就是tryAcquireShared被满足了，所以所有挂起“等待锁”的线程都可以执行了。
