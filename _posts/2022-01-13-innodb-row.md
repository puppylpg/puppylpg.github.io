...
tags: [mysql, innodb]
description: "拆开 InnoDB COMPACT 行格式，看变长列长度、NULL 位图、记录头和隐藏列如何一起组成一行。"
---
...
block-beta
    columns 1
    block:row["COMPACT 一行记录，从前到后大致可以这样看"]
        columns 5
        VarLen["变长列长度列表"]
        NullMap["NULL 位图"]
        Header["记录头信息<br/>deleted_flag / heap_no / next_record"]
        Hidden["隐藏列<br/>row_id / trx_id / roll_pointer"]
        Data["真实列值<br/>按 schema 反序列化"]
    end

    style VarLen fill:#e3f2fd,stroke:#1976d2
    style NullMap fill:#fff3bf,stroke:#f59f00
    style Header fill:#e8f5e9,stroke:#2e7d32
    style Hidden fill:#f3e5f5,stroke:#8e24aa
    style Data fill:#ffe3e3,stroke:#c62828
```
