...
tags: [spring-security, authentication, security-context, session, remember-me]
description: "梳理 Spring Security Authentication 的核心对象、认证过滤器、会话持久化与 remember-me 机制。"
---
...
前两个问题是最基础的递进关系：先知道“谁在访问”（Authentication），才能判断“他有没有资格访问”（Authorization）。本文聚焦 Authentication，重点看认证信息如何产生、如何存放、如何在后续请求中复用。

1. Table of Contents, ordered
{:toc}

文章的主线是：**认证方式可以很多，但最终目标都是构造一个可信的`Authentication`，并放入`SecurityContextHolder`供后续鉴权使用**。

以表单登录为例，认证成功前后最关键的状态变化如下：

```mermaid
sequenceDiagram
    participant Browser as Browser
    participant ETF as ExceptionTranslationFilter
    participant Login as UsernamePasswordAuthenticationFilter
    ...
    Browser->>ETF: 访问受保护资源
    ETF-->>Browser: 302 /login，并缓存原始请求
    Browser->>Login: POST /login username/password
    Login->>AM: authenticate(token)
    AM->>Provider: 校验用户和密码
    Provider-->>AM: Authentication(authenticated=true)
    AM-->>Login: Authentication
    Login->>SCH: setContext(authentication)
    Login->>Session: 保存 SecurityContext
    Login-->>Browser: 跳回原始请求
```
...
`Authentication`的wrapper，所以其实它也没干什么，就是get/set `Authentication`。
...
>
> 666，原来是这样约定的。浏览器表单登录需要服务器帮忙保存原始请求，Basic 客户端则通常可以自己重放请求。Spring Security 属实把需求拿捏了。
...
**登录成功后要换 session id，以防止[session fixation attacks](https://docs.spring.io/spring-security/reference/5.8/servlet/authentication/session-management.html#ns-session-fixation)**。
...
**这和缓存请求以重定向的思路一样，也是放到session里**。当然，basic认证的时候不需要缓存请求，因为client能自己发，所以用的`NullRequestCache `。这里也类似，对于oauth，应该也可以自己一直带上认证，所以不需要把`SecurityContext`存下来，用`NullSecurityContextRepository`就行，其实也是什么也不干。
...
## 并发session控制
一个比较有意思的问题：[一个账户最多能多少人同时使用](https://docs.spring.io/spring-security/reference/5.8/servlet/authentication/session-management.html#ns-concurrent-sessions)？比如一个账号同时间只能登录一次（同一时刻只能有一个关于这个账号的session）。第二次登录默认踢掉第一次，或者直接禁止第二次登录。
...
# 感想
Spring Security 对需求拿捏得是真准啊，我反而是通过功能在认识需求了。

Authentication 设计可以概括为两层：入口层用不同 filter 适配 form、basic、remember-me 等认证方式；核心层统一产出`Authentication`并放入`SecurityContextHolder`。理解这一点之后，请求缓存、session 持久化、remember-me 这些功能就不再是分散的细节，而是围绕“如何让后续请求继续拥有可信身份”的不同实现。
