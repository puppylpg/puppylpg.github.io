...
#### 响应时间分布
![vthread](/pics/jmeter/vthread/vthread-flot-response-time-distribution.png)

![thread](/pics/jmeter/vthread/thread-flot-response-time-distribution.png)

可以看到虚线程对大部分请求的响应都很快（除了最后timeout的那些请求），os线程响应明显比较慢。

#### 响应时间分位数
![vthread](/pics/jmeter/vthread/vthread-flot-response-times-percentiles.png)

![thread](/pics/jmeter/vthread/thread-flot-response-times-percentiles.png)

虚线程由于对大部分请求的响应都很快，所以中位数比较低，只有1771ms。os线程对大部分请求的响应都偏慢，所以中位数比较高，达到了19881ms，差了一个量级。

#### 响应时间区间统计
![vthread](/pics/jmeter/vthread/vthread-flot-response-time-overview.png)

![thread](/pics/jmeter/vthread/thread-flot-response-time-overview.png)

os线程的响应时长几乎全在1500ms以上，虚线程有很多响应都在1500ms以下。结合整个程序的负载（`Thread.sleep(Duration.ofMillis(1000))`）来看，显然虚线程要合理得多。

#### 不同压力下的响应时间
![vthread](/pics/jmeter/vthread/vthread-flot-response-time-vs-request.png)

![thread](/pics/jmeter/vthread/thread-flot-response-time-vs-request.png)
