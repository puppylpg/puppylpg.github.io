...
- [从 Attention 到 KV Cache：理解 Transformer 的注意力机制与推理加速]({% post_url 2026-06-12-transformer-qkv-kv-cache %}) 已经讲过 Q/K/V、Multi-Head Attention 和原生 KV Cache。
- [从 Prefill 到 Decode：用两层 Transformer 走完一次 LLM 推理]({% post_url 2026-06-13-transformer-prefill-decode %}) 已经讲过 prefill/decode 的计算形态和性能差异。
- [从 KV Cache 到 Prefix Caching：LLM 推理为什么能复用前缀]({% post_url 2026-06-13-transformer-prefix-caching %}) 已经讲过 GQA、KV Cache 形状、PagedAttention 和 Prefix Caching。

所以下面不再重复解释 KV Cache “存的是什么”、prefill 为什么要处理整段 prompt、Prefix Caching 为什么能复用公共前缀，而是直接把这些概念拿来做部署显存估算。
...
QwQ-32B 使用 GQA：40 个 query heads，但只有 8 个 KV heads。为什么 Query head 和 KV head 可以不一样、GQA 为什么能直接减少 KV Cache 体积，前文已经在 [Prefix Caching 那篇]({% post_url 2026-06-13-transformer-prefix-caching %}) 展开过。这里直接用部署估算结论：**KV Cache 只需要存 K/V heads，所以公式应该用 `num_key_value_heads`，不能直接用所有 attention heads 或完整 hidden size 粗暴代替。**
...
请求结束后，它的 KV Cache 通常会被释放，显存留给新的请求。完全相同 prompt 后续能不能少算一部分 prefill，要看系统是否启用 prefix caching / prompt caching；这部分机制已经在 [Prefix Caching 那篇]({% post_url 2026-06-13-transformer-prefix-caching %}) 讲过。放到本文的显存估算里，只需要抓住一点：prefix cache 本质上仍然是热缓存，也会占显存或 CPU 内存，也会被淘汰。因此长上下文推理的核心调度问题不是“请求数”，而是：
...
所以 “120K 是 64K 的 3.5 倍慢” 只能作为 prefill 计算量直觉，不能直接当成所有推理阶段的吞吐比例。更完整的 prefill/decode 性能画像可以回看 [Prefill 到 Decode 那篇]({% post_url 2026-06-13-transformer-prefill-decode %})。
