...
description: "理解 Tomcat 如何用 server.xml、context.xml、web.xml 和 Digester 把配置解析成组件关联关系。"
---
...
```mermaid
flowchart LR
    XML["server.xml / context.xml / web.xml"] --> Digester["Digester 规则<br/>遇到标签就 new/set/add"]
    Digester --> Server["Server"]
    Server --> Service["Service"]
    Service --> Connector["Connector"]
    Service --> Engine["Engine"]
    Engine --> Host["Host"]
    Host --> Context["Context / Webapp"]
    Context --> Wrapper["Wrapper / Servlet"]

    style XML fill:#e3f2fd,stroke:#1976d2
    style Digester fill:#fff3bf,stroke:#f59f00
