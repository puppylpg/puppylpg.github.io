...
tags: [elasticsearch]
description: "Elasticsearch 搜索笔记：realtime get、search API、常用 Query DSL、倒排索引和 highlight。"
---
...
sequenceDiagram
    ...
    participant L as Lucene segment

    C->>ES: PUT /index/_doc/1
    ES->>T: 写 translog
    ES-->>C: 写入成功
    C->>ES: GET /index/_doc/1
    ES->>T: realtime get 可从 translog/内存路径补齐
    ES-->>C: 能查到
    C->>ES: GET /index/_search
    ES->>L: 走已 refresh 的 segment
    L-->>ES: refresh 前没有这条倒排记录
    ES-->>C: 可能搜不到
```
...
`_search`和`_update_by_query`都是受影响的。写入之后，如果没有refresh，update by query返回结果是0，找不到要更新的文件：
```json
{
  "took" : 1,
  "timed_out" : false,
...
| 查询 | 典型字段 | 是否分析查询词 | 直觉 |
| --- | --- | --- | --- |
| `term` | `keyword` / 精确值 | 否 | 完整值匹配，像 `where field = ?` |
| `terms` | `keyword` / 精确值 | 否 | 多值精确匹配，像 `where field in (...)` |
| `match` | `text` | 是 | 查询词也分词，再去倒排索引里找 token |
| `match_phrase` | `text` | 是 | 还要求 token 的位置关系符合短语顺序 |
| `bool.filter` | 任意可过滤字段 | 取决于子查询 | 不计分，适合缓存和缩小范围 |
...
如果搜索`quick brown`，本质上是先拿 token 找 posting list，再把 posting list 合并和计分：

```mermaid
flowchart LR
    Q[query: quick brown] --> A[analyzer]
    A --> T1[quick]
    A --> T2[brown]
    T1 --> P1[Doc_1]
    T2 --> P2[Doc_1, Doc_2]
    P1 --> R[合并 / 计分]
    P2 --> R
    R --> O[Doc_1 命中 2 个词<br/>Doc_2 命中 1 个词]

    style A fill:#e3f2fd,stroke:#4dabf7
    style R fill:#fff3bf,stroke:#f59f00
```
