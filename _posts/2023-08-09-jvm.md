...
categories: [java, jvm]
tags: [java, jvm]
description: "把 class 文件当成 JVM 的二进制协议，从字段结构、字节序、常量池和执行模型看虚拟机实现。"
---
...
```mermaid
flowchart TD
    Java["Java 源码"] --> Compiler["javac / 其他 JVM 语言编译器"]
    Compiler --> ClassFile[".class 文件<br/>JVM 二进制协议"]
    ClassFile --> ClassLoader["ClassLoader"]
    ClassLoader --> Runtime["JVM Runtime<br/>方法区/堆/栈/PC/本地方法栈"]
    Runtime --> Interpreter["解释执行"]
    Runtime --> JIT["JIT 编译执行"]

    style ClassFile fill:#e3f2fd,stroke:#1976d2
    style Runtime fill:#fff3bf,stroke:#f59f00
    style JIT fill:#e8f5e9,stroke:#2e7d32
```
...
> 详细介绍可以参考这篇非常不错的[Mastering the mechanics of Java method invocation](https://blogs.oracle.com/javamagazine/post/mastering-the-mechanics-of-java-method-invocation)
