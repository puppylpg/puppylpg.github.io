...
description: "梳理 Java 显式锁：ReentrantLock、tryLock、可中断锁、公平锁、ReadWriteLock 和 StampedLock 乐观读的使用场景。"
---
...
flowchart TD
    Need["需要同步共享状态"]
    Basic{"只需要进入/退出\n临界区吗？"}
    Sync["优先 synchronized\n简单且自动释放"]
    Advanced{"需要高级能力吗？"}
    Try["tryLock / timeout\n避免死等或实现降级"]
    Interrupt["lockInterruptibly\n支持取消任务"]
    Fair["公平锁\n减少饥饿但牺牲吞吐"]
    RW{"读多写少吗？"}
    RWL["ReadWriteLock\n读读并发"]
    Stamped["StampedLock\n乐观读 + 校验"]
    ...
    Advanced --> RW
    RW -->|"读多写少，但能接受悲观读"| RWL
    RW -->|"读很多、写较少，能处理校验失败"| Stamped

    style Sync fill:#e8f5e9,stroke:#51cf66
    style Try fill:#e3f2fd,stroke:#4dabf7
    style Interrupt fill:#e3f2fd,stroke:#4dabf7
    style Fair fill:#fff3bf,stroke:#f59f00
    style Stamped fill:#ffe3e3,stroke:#ff6b6b
```

所以选择锁的时候，不要上来就问“哪个高级”。先问自己：**我是不是只需要最普通的互斥？如果是，`synchronized`反而是更稳的默认答案。**
...
Ref:
- [廖雪峰：StampedLock](https://www.liaoxuefeng.com/wiki/1252599548343744/1309138673991714)
...
```mermaid
flowchart TD
    Start["tryOptimisticRead()\n拿到 stamp"]
    Read["无锁读取共享字段"]
    Validate{"validate(stamp)\n期间有写锁吗？"}
    Return["直接返回读取结果"]
    Pessimistic["获取 readLock()\n悲观读一次"]
    Unlock["unlockRead(stamp)"]

    Start --> Read --> Validate
    Validate -->|"没有写入"| Return
    Validate -->|"发生写入，读到的可能是拼接值"| Pessimistic --> Unlock --> Return

    style Start fill:#e3f2fd,stroke:#4dabf7
    style Validate fill:#fff3bf,stroke:#f59f00
    style Return fill:#e8f5e9,stroke:#51cf66
    style Pessimistic fill:#ffe3e3,stroke:#ff6b6b
```

`StampedLock`的乐观读不是“读一定正确”，而是“先读，读完再确认这期间没人写”。这就是它和普通读锁最大的差别。
```java
public class Point {
