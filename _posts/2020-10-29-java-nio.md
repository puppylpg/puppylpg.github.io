...
categories: [io, nio]
tags: [io, nio]
description: "从 BIO 的线程绑定问题倒推 Java NIO 的 Selector 事件模型，并串起 Channel、SelectionKey 和 epoll 的关系。"
---
...
把BIO和NIO放到一张时序图里，就很容易看清楚：NIO不是把“读写”变魔法了，而是把“等待可读可写”这件事集中交给Selector了。
...
sequenceDiagram
    ...
    participant Channel as socket channel

    rect rgb(255, 243, 191)
        Conn->>Worker: BIO read()
        Worker-->>Worker: 阻塞等待数据到达
        Conn-->>Worker: 数据到达
        Worker->>Conn: 读取并处理
    end

    rect rgb(227, 242, 253)
        Channel->>Selector: 注册 OP_READ
        Selector-->>Selector: select() 阻塞等待所有 channel
        Conn-->>Selector: 某个 channel 可读
        Selector-->>Worker: 返回 ready key
        Worker->>Channel: read(ByteBuffer)
    end
```

所以NIO的“非阻塞”不是说线程永远不阻塞，`select()`本身就会阻塞；它的重点是**不让业务线程在每个connection上单独阻塞**。阻塞还是有的，只是从“一堆线程分别等”变成了“一个Selector集中等”。
...
# Ref
- [Baeldung: Java NIO Selector](https://www.baeldung.com/java-nio-selector)
- 一篇比较高屋建瓴的文章：[美团技术团队：Java NIO浅析](https://tech.meituan.com/2016/11/04/nio.html)
...
- [Linux epoll 的一个通俗解释](https://www.jianshu.com/p/37a2bb9d1cae)
