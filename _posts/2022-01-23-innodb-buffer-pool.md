...
description: "解释 Buffer Pool 如何缓存 InnoDB 页、维护 free/LRU/flush 链表，并与 OS page cache 区分开。"
---
...
```mermaid
flowchart TD
    Query["读写某个表页"] --> Hash["用 space id + page number 查 hash"]
    Hash -->|命中| BufferPage["Buffer Pool 缓冲页"]
    Hash -->|未命中| Free{"free 链表有空页?"}
    Free -->|有| Load["从磁盘读 16KB 页到缓冲页"]
    Free -->|没有| LRU["从 LRU old 区淘汰冷页"]
    LRU --> Dirty{"被淘汰页是脏页?"}
    Dirty -->|是| Flush["先刷到磁盘 / page cache"]
    Dirty -->|否| Load
    Flush --> Load
    BufferPage --> Modify["修改后进入 flush 链表"]

    style Hash fill:#e3f2fd,stroke:#1976d2
    style Free fill:#fff3bf,stroke:#f59f00
    style LRU fill:#e8f5e9,stroke:#2e7d32
    style Dirty fill:#ffe3e3,stroke:#c62828
```
...
- **主存和磁盘就是这样的关系**，所以：主存用来存放正在使用的那部分虚拟内存数据，磁盘用来存放暂时用不到的虚拟内存数据。主存中的page，指的就是虚拟内存的一部分（一页）；
    > 可参考维基百科对[虚拟内存](https://zh.wikipedia.org/wiki/%E8%99%9A%E6%8B%9F%E5%86%85%E5%AD%98)的定义。
...
可以参考两个相关讨论：[DBA StackExchange 上对 InnoDB page 与 OS page 的区分](https://dba.stackexchange.com/a/305894/245984)，以及[知乎上关于 InnoDB 页和操作系统页关系的回答](https://www.zhihu.com/question/447918756/answer/1783879900)。
...
page cache脏数据写回也有自己的时机，可以参考这篇对[Linux 脏页回写参数的整理](https://blog.csdn.net/rikeyone/article/details/105783596)。
...
参阅：[sync、fsync 与 fdatasync 的区别](https://www.jb51.net/article/101062.htm)。
