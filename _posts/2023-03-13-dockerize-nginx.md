...
**[`VIRTUAL_PORT`](https://github.com/nginx-proxy/nginx-proxy#virtual-ports)代表容器的访问端口，同时也是nginx配置里的upstream的端口**：
...
```
netdata默认的port是[`19999`](https://learn.netdata.cloud/docs/configuring/securing-netdata-agents/web-server).
...
2. 给v2ray[配置`DEFAULT_ROOT=none`以禁用`puppylpg.top`的`location /`](https://github.com/nginx-proxy/nginx-proxy#default_root)；
2. 然后在[`/etc/nginx/vhost.d/puppylpg.top`](https://github.com/nginx-proxy/nginx-proxy#per-virtual_host)或者[`/etc/nginx/vhost.d/default`](https://github.com/nginx-proxy/nginx-proxy#per-virtual_host-default-configuration)配置`location /`的行为；
