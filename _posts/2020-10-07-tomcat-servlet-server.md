...
description: "在原始 Web Server 之上加入 Servlet 规范，理解容器如何把 HTTP 请求转成 ServletRequest/ServletResponse。"
---
...
```mermaid
flowchart TD
    Socket["Socket 字节流"] --> HttpParse["解析 HTTP request"]
    HttpParse --> ServletRequest["创建 ServletRequest"]
    HttpParse --> ServletResponse["创建 ServletResponse"]
    ServletRequest --> Servlet["Servlet#service(req, res)"]
    ServletResponse --> Servlet
    Servlet --> Writer["ServletResponse#getWriter()"]
    Writer --> HttpResponse["写回 HTTP response"]

    style HttpParse fill:#e3f2fd,stroke:#1976d2
    style Servlet fill:#e8f5e9,stroke:#2e7d32
    style Writer fill:#fff3bf,stroke:#f59f00
```
