...
categories: [network]
tags: [network]
description: "围绕以太网、MAC 帧、广播、CSMA/CD、交换机和 PPPoE，补齐数据链路层的关键直觉。"
---

[计算机网络]({% post_url 2021-12-19-network %})整体梳理了比较宏观的知识，但是在一些问题上还是让人有很多问号。比如为什么数据链路层要使用广播发消息、为什么最小报文得是64Byte。
...
```mermaid
flowchart LR
    A["主机A<br/>源MAC=A"] --> Bus["共享介质/交换网络"]
    Bus --> B["主机B<br/>目的MAC=B"]
    Bus --> C["主机C<br/>目的MAC不是C，丢弃"]
    Bus --> D["主机D<br/>目的MAC不是D，丢弃"]

    A -. "物理上：大家都能收到信号" .-> C
    B -. "逻辑上：只有目的MAC接收" .-> A
```

所以“广播”这个词要看层次：**物理传播方式可以是广播，逻辑通信目标可以是单播**。这俩不冲突，只是观察角度不同。
...
最小帧长度就是为了保证：**如果真的发生最晚碰撞，发送方还没把帧发完，所以它能检测到并重发**。如果帧太短，发送完才知道刚才撞车了，那就很尴尬，跟发完消息才发现对方早拉黑你一样。
...
由于材质改变，以太网传输速度不再是10Mb/s了，出现了100Mb/s，甚至千兆网，万兆网。上文说过，**在使用CSMA/CD的情况下**，网速变快了，要么主机间距离得缩短，要么最小帧不能再是64byte了，得增大。
...
> - [What is an ISP?](https://whatismyipaddress.com/isp)
>
...
```mermaid
flowchart TD
    Bus["总线以太网<br/>共享介质"] --> CSMA["需要 CSMA/CD"]
    CSMA --> Limit["距离受争用期限制"]
    Switch["交换机 + 全双工"] --> NoCollision["基本没有碰撞"]
    NoCollision --> NoCSMA["不再依赖 CSMA/CD"]
    NoCSMA --> WAN["以太网可扩展到更远距离"]
    WAN --> Auth["接入运营商需要认证/计费"]
    Auth --> PPPoE["Ethernet 封装 PPP<br/>PPPoE"]
```
