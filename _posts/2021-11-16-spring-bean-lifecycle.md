...
categories: [tech, spring, bean-lifecycle]
tags: [spring, bean-lifecycle, beanpostprocessor, applicationcontext]
description: "从实例化、属性注入、Aware、初始化到销毁，梳理 Spring Bean 生命周期和后处理器扩展点。"
---

Spring Bean 的生命周期看起来复杂，是因为 Spring 在普通对象创建过程的多个节点都插入了扩展点：实例化前后、属性注入前后、初始化前后、销毁前。只看完整流程图会很容易迷失，但如果按“普通 Java 对象如何逐步被容器接管”来拆解，脉络会清晰很多。

1. Table of Contents, ordered
{:toc}

本文先从最朴素的反射创建对象开始，再逐层加入 bean 级回调、容器级后处理器和容器启动流程，最后回到 `@PostConstruct`、`@PreDestroy` 这些常用注解为什么能生效。

把后面所有扩展点压成一张状态图，大概就是下面这条链路。后文每引入一个接口，基本都是在图上的某个节点前后插一段逻辑：

```mermaid
stateDiagram-v2
    [*] --> BeanDefinition: 读取配置
    BeanDefinition --> BeforeInstantiation: InstantiationAwareBeanPostProcessor
    BeforeInstantiation --> Instantiated: 反射/构造函数实例化
    Instantiated --> PropertiesPopulated: 属性注入
    PropertiesPopulated --> AwareCallbacks: BeanNameAware / BeanFactoryAware ...
    AwareCallbacks --> BeforeInit: BeanPostProcessor#before
    BeforeInit --> Init: InitializingBean / init-method / @PostConstruct
    Init --> AfterInit: BeanPostProcessor#after
    AfterInit --> Ready: 放入 singleton 池
    Ready --> Destroy: 容器关闭
    Destroy --> [*]: DisposableBean / destroy-method / @PreDestroy
```
...
> 上图使用 Mermaid 的 [flowchart 语法](https://mermaid.js.org/syntax/flowchart.html)表达生命周期节点。
...
## `DisposableBean`
预销毁关联的接口是 `DisposableBean`：如果 bean 需要有预销毁操作，就实现该接口。容器在 bean 的预销毁阶段调用它的 `destroy()` 方法，完成资源释放。
...
> 或者使用 `@PostConstruct` / `@PreDestroy` 注解。
...
# 容器启动销毁
[这个 Spring 4.x 示例](https://github.com/puppylpg/spring4.x/tree/master/chapter4/src/main/java/com/smart)很适合观察 Spring Bean 生命周期。
...
别看名字长，细看，它是一个 `BeanPostProcessor`。由上文可知，`BeanPostProcessor` 是容器级的，在 bean 创建好后对 bean 做奇奇怪怪的事情。再看它的名字 `init destroy annotation`，它负责实现 init annotation（`@PostConstruct`）和 destroy annotation（`@PreDestroy`）这两个 annotation 的功能。
...
`BeanPostProcessor`（`DestructionAwareBeanPostProcessor`）由于和 `InitializingBean`、`DisposableBean` 位置基本重合，所以如果你有一个需求，要给 bean 在 init 或 destroy 时做些什么操作，直接用这两个容器级插件接口就行了。
