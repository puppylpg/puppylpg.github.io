...
categories: [tech, spring-data, spring-data-elasticsearch]
tags: [spring-data, spring-data-elasticsearch, repository, object-mapping]
description: "从 Spring Data Commons 的 object mapping 与 Repository 抽象出发，串联 Spring Data Elasticsearch 的扩展能力。"
---

给 Spring Data Elasticsearch 提交 PR（[#2793](https://github.com/spring-projects/spring-data-elasticsearch/pull/2793)、[#2802](https://github.com/spring-projects/spring-data-elasticsearch/pull/2802)）之后，再回头系统阅读 [Spring Data Commons](https://docs.spring.io/spring-data/commons/reference/) 和 [Spring Data Elasticsearch](https://docs.spring.io/spring-data/elasticsearch/reference/) 的文档，会更容易看清二者的关系：前者提供通用抽象，后者把这些抽象落到 Elasticsearch 这一类 store-specific 实现上。
...
本文先看 Spring Data Commons 的 object mapping 和 repository 抽象，再把这些概念映射到 Spring Data Elasticsearch：mapping 注解、converter、callback、scrolling、null 约定、auditing、projection 和 Elasticsearch 独有能力。

Spring Data Commons 和具体 store 的关系，可以先看成“通用抽象 + store-specific 落地”：

```mermaid
flowchart TD
    Commons["Spring Data Commons"] --> Mapping["Object Mapping<br/>对象创建 + 属性填充"]
    Commons --> Repository["Repository<br/>查询方法 + 代理对象"]
    Commons --> Callback["Entity Callback / Events"]
    Commons --> Auditing["Auditing / Projection / QBE"]

    Mapping --> StoreSpecific["Store-specific module"]
    Repository --> StoreSpecific
    Callback --> StoreSpecific
    Auditing --> StoreSpecific

    StoreSpecific --> Elasticsearch["Spring Data Elasticsearch"]
    Elasticsearch --> EsMapping["@Document / @Field / @Id / Routing"]
    Elasticsearch --> EsQuery["NativeQuery / CriteriaQuery / StringQuery"]
    Elasticsearch --> EsRuntime["scroll / PIT / join / script / runtime field"]

    classDef commons fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef store fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef es fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    class Commons,Mapping,Repository,Callback,Auditing commons
    class StoreSpecific store
    class Elasticsearch,EsMapping,EsQuery,EsRuntime es
```
...
规则很复杂，但一般使用场景都比较简单，一个 Lombok `@Data`走天下 :D 如果想更加合理，可以参考 Spring Data 给出的[General recommendations](https://docs.spring.io/spring-data/commons/reference/object-mapping.html#mapping.general-recommendations)。
...
## scrolling
Scrolling 的通用抽象可以参考 Spring Data Commons 的 [scrolling 文档](https://docs.spring.io/spring-data/commons/reference/repositories/scrolling.html)。
...
## Auditing
[Auditing](https://docs.spring.io/spring-data/commons/reference/auditing.html)的概念听起来很像git，为了能记录谁在什么时候对数据做了修改（但是似乎并不能记录改了什么）。
...
## QBE
[Query by Example](https://docs.spring.io/spring-data/commons/reference/query-by-example.html) 很像 Elasticsearch Java client 的查询构造方式。

## web拓展支持
Spring Data 也提供 [web 扩展支持](https://docs.spring.io/spring-data/commons/reference/repositories/core-extensions.html#core.web)，用于把分页、排序等 repository 相关参数接到 Web 层。
...
# 感想
Spring Data 的核心价值不是“替你屏蔽所有数据库差异”，而是把通用的数据访问形态沉淀成 repository、mapping、callback、conversion 这些抽象。理解这些抽象，再看 store-specific 的 Elasticsearch 支持，就不会只停留在“自动 CRUD 很方便”这一层。
