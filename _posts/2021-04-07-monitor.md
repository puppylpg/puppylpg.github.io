...
description: "从 ObjectMonitor、monitorenter/monitorexit 到偏向锁、轻量级锁和重量级锁，梳理 synchronized 的运行时结构。"
---
...
```mermaid
flowchart TD
    Obj["Java Object"] --> Header["Object Header / Mark Word"]
    Header -->|重量级锁时保存指针| Monitor["ObjectMonitor"]
    Monitor --> Owner["_owner<br/>当前持锁线程"]
    Monitor --> Entry["_EntryList<br/>抢锁失败的线程"]
    Monitor --> Wait["_WaitSet<br/>调用 wait() 的线程"]
    Monitor --> Rec["_recursions<br/>重入次数"]

    Owner -->|monitorexit| Entry
    Wait -->|notify/notifyAll| Entry
    Entry -->|重新竞争| Owner
```

这张图能解释很多“为什么”：`wait/notify`必须通过锁对象调用，因为线程确实就挂在这个对象关联的monitor队列上；`notify`也不是直接让线程继续跑，而是把它从wait set挪回可以竞争锁的位置。
...
Ref:
- [Java Monitor 机制的一篇整理](https://www.cnblogs.com/kundeg/p/8422557.html)
...
主要是为了优化早期 Java 集合 API的程序（JDK 1.1），这些 API（Hashtable 和 Vector） 每次访问时都进行同步。但是JDK 1.2 引入了针对单线程场景的非同步集合（HashMap 和 ArrayList），JDK 1.5 针对多线程场景推出了性能更高的并发数据结构。经过这么多年的发展，使用Vector的代码应该少之又少了。
...
- [一个关于 synchronized 锁升级的回答](https://www.zhihu.com/question/53826114/answer/236363126)

```mermaid
stateDiagram-v2
    [*] --> NoLock: 普通对象
    NoLock --> Biased: 单线程重复进入
    Biased --> Lightweight: 出现另一个线程竞争
    Lightweight --> Lightweight: CAS/自旋成功
    Lightweight --> Heavyweight: 自旋失败或竞争激烈
    Heavyweight --> Heavyweight: EntryList/WaitSet 排队
...
其他参考：
- [SegmentFault: synchronized 机制整理](https://segmentfault.com/a/1190000023315634)
- [美团技术团队：不可不说的Java锁事](https://tech.meituan.com/2018/11/15/java-lock.html)
