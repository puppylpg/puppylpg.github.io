---
title: "Transformer 推理系列（四）：QwQ-32B 部署显存与低精度计算"
...
这个问题看似只是算账，实际上会把 LLM 推理里最重要的部署变量全部牵出来：参数量、权重精度、KV Cache、上下文窗口、计算精度、GPU Tensor Core，以及为什么新一代 GPU 要支持更低精度格式。

如果直接从“模型精度是什么”讲起，文章很容易变成一堆概念拼接。更自然的路径是从部署问题开始：**QwQ-32B 的显存花在哪里？哪些部分固定，哪些部分随上下文增长，哪些部分能靠量化和硬件低精度优化？**

这篇文章可以看作前面三篇的部署延续：

- [Transformer 推理系列（一）：从 Attention 到 KV Cache]({% post_url 2026-06-12-transformer-inference-01-attention-kv-cache %}) 已经讲过 Q/K/V、Multi-Head Attention、GQA/MQA 和原生 KV Cache。
- [Transformer 推理系列（二）：从 Prefill 到 Decode]({% post_url 2026-06-13-transformer-inference-02-prefill-decode %}) 已经讲过 prefill/decode 的计算形态和性能差异。
- [Transformer 推理系列（三）：从 PagedAttention 到 Prefix Caching]({% post_url 2026-06-13-transformer-inference-03-paged-prefix-cache %}) 已经讲过 PagedAttention、Prefix Caching 和跨请求前缀复用。

所以下面不再重复解释 KV Cache “存的是什么”、prefill 为什么要处理整段 prompt、GQA 为什么要用 `num_key_value_heads` 估算 KV Cache、Prefix Caching 为什么能复用公共前缀，而是直接把这些概念拿来做部署显存估算。
...
一次 LLM 推理里至少有三类重要数据：权重、激活值、KV Cache。
...
**KV Cache**是生成式推理里最关键的动态显存。它的机制前面已经单独讲过：prefill 阶段为 prompt 建立逐层 K/V，decode 阶段每生成一个新 token 再追加这个 token 的 K/V。上下文越长、batch 越大、并发越高，KV Cache 越大。
...
QwQ-32B 使用 GQA：40 个 query heads，但只有 8 个 KV heads。为什么 Query head 和 KV head 可以不一样、GQA 为什么能直接减少 KV Cache 体积，前文已经在 [Transformer 推理系列（一）：从 Attention 到 KV Cache]({% post_url 2026-06-12-transformer-inference-01-attention-kv-cache %}) 展开过。这里直接用部署估算结论：**KV Cache 只需要存 K/V heads，所以公式应该用 `num_key_value_heads`，不能直接用所有 attention heads 或完整 hidden size 粗暴代替。**
...
请求结束后，它的 KV Cache 通常会被释放，显存留给新的请求。完全相同 prompt 后续能不能少算一部分 prefill，要看系统是否启用 prefix caching / prompt caching；这部分机制已经在 [Transformer 推理系列（三）：从 PagedAttention 到 Prefix Caching]({% post_url 2026-06-13-transformer-inference-03-paged-prefix-cache %}) 讲过。放到本文的显存估算里，只需要抓住一点：prefix cache 本质上仍然是热缓存，也会占显存或 CPU 内存，也会被淘汰。因此长上下文推理的核心调度问题不是“请求数”，而是：
...
但速度问题更复杂。prefill 和 decode 的差异前文已经专门拆过，这里只保留和 64K/120K 估算直接相关的结论。prefill 阶段需要处理整段 prompt，传统 attention 的计算量会受到上下文长度平方项影响：
...
所以 “120K 是 64K 的 3.5 倍慢” 只能作为 prefill 计算量直觉，不能直接当成所有推理阶段的吞吐比例。更完整的 prefill/decode 性能画像可以回看 [Transformer 推理系列（二）：从 Prefill 到 Decode]({% post_url 2026-06-13-transformer-inference-02-prefill-decode %})。
