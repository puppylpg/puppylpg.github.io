...
tags: [spring, beanfactory, applicationcontext, refresh]
description: "沿着 AnnotationConfigApplicationContext 启动路径，拆解 BeanFactory、ApplicationContext、refresh 和 Bean 创建链路。"
---

理解 Spring Boot 之前，最好先把 Spring 自己的启动路径走通。Spring Boot 最终仍然要创建并刷新一个 `ApplicationContext`，只是它在外面包了一层配置加载、事件发布和自动配置选择。回到 Spring 本身，核心问题会收敛成三件事：`BeanFactory` 管什么，`ApplicationContext` 多了什么，`refresh()` 为什么能让所有非懒加载单例就绪。

1. Table of Contents, ordered
{:toc}

本文以 `AnnotationConfigApplicationContext` 为主线：先拆解容器接口，再看配置类如何被注册为 `BeanDefinition`，最后进入 `refresh()`，观察 `BeanFactoryPostProcessor`、`BeanPostProcessor` 和 singleton 实例化的顺序。

从 `BeanDefinition` 到可用 bean 的路径，可以先记住这条最短链路：

```mermaid
flowchart TD
    Config["配置类 / XML / 扫描结果"] --> Reader["BeanDefinitionReader<br/>ClassPathBeanDefinitionScanner"]
    Reader --> Registry["BeanDefinitionRegistry<br/>注册 BeanDefinition"]
    Registry --> Refresh["ApplicationContext#refresh"]
    Refresh --> BFPP["BeanFactoryPostProcessor<br/>修改/补充 BeanDefinition"]
    BFPP --> BPP["BeanPostProcessor<br/>介入 bean 生命周期"]
    BPP --> Create["finishBeanFactoryInitialization<br/>创建非懒加载 singleton"]
    Create --> Pool["SingletonBeanRegistry<br/>singletonObjects"]
    Pool --> GetBean["getBean()"]

    classDef config fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef process fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef store fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    class Config,Reader,Registry config
    class Refresh,BFPP,BPP,Create process
    class Pool,GetBean store
```
...
classDiagram
    ...
    class ConfigurableBeanFactory {
        ...
        +setBeanExpressionResolver(resolver)
    }
    ...
    BeanFactory : lookup
    ListableBeanFactory : traverse
    HierarchicalBeanFactory : parent/child
    AutowireCapableBeanFactory : autowire rules
    ConfigurableBeanFactory : configuration
```
> BeanFactory 的子接口各管一摊：lookup、遍历、层级、注入规则、配置。
...
获取Environment。在AbstractApplicationContext里，如果没有就创建一个StandardEnvironment（profiles + properties）。这一步和springboot new StandardEnvironment是几乎一致的。

Environment 相关概念可以参考 Spring 官方文档中的 [Environment 抽象](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-environment) 和 [Profile 支持](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-definition-profiles-enable)。
...
`AbstractApplicationContext#refresh()` 的整体流程：

```mermaid
flowchart TD
    A["refresh()"] --> B["1. prepareRefresh()"]
    B --> C["2. prepareBeanFactory()"]
    C --> D["3. invokeBeanFactoryPostProcessors()\n——处理 BeanDefinition"]
    D --> E["4. registerBeanPostProcessors()\n——注册但尚未调用"]
    E --> F["5. initMessageSource()"]
    F --> G["6. initApplicationEventMulticaster()"]
    G --> H["7. onRefresh()\n——子类可覆写"]
    H --> I["8. registerListeners()"]
    I --> J["9. finishBeanFactoryInitialization()\n——实例化所有非 Lazy singleton"]
    J --> K["10. 发布 ContextRefreshedEvent"]
    style D fill:#fff9c4
    style J fill:#fff9c4
```
...
```
也没看出来要干什么。第一个好像是org.springframework.web.context.support.WebApplicationContextUtils.initServletPropertySources子类才override了这个空方法。

## prepare bean factory
设置一堆东西：
- StandardBeanExpressionResolver，好像是处理spel的。BeanExpressionResolver接口；
- ResourceEditorRegistrar：往PropertyEditorRegistrar里注册处理某个class所使用的PropertyEditor。具体编辑什么，后面可以查查
...
```mermaid
flowchart LR
    subgraph BeanFactoryPostProcessor
        A1["修改 BeanDefinition\n（属性、scope、lazy 等）"]
    end
...
其实想想，BeanFactoryPostProcessor是BeanFactory的PostProcessor。它是处理 **BeanFactory实例** 的。**BeanFactory实例里有什么？有BeanDefinition。所以它可以修改BeanDefinition**。
...
```
> 从这里开始，貌似后面都开始从BeanFactory（map）里根据type获取bean了（无论是spring自己提前put进去的，还是开发者自己的@Bean被解析了然后put进去的）。从这里开始，bean已经都注册上了，可以这么获取了。

再依次处理一遍。总体而言，他们的处理顺序是：

```mermaid
flowchart TD
    A["invokeBeanFactoryPostProcessors()"] --> B["第一阶段\npostProcessBeanDefinitionRegistry()"]
    B --> B1["1. PriorityOrdered 的\nBeanDefinitionRegistryPostProcessor"]
    B1 --> B2["2. Ordered 的\nBeanDefinitionRegistryPostProcessor"]
    B2 --> B3["3. 其余的\nBeanDefinitionRegistryPostProcessor"]
    B3 --> C["第二阶段\npostProcessBeanFactory()"]
    C --> C1["1. 所有 BeanDefinitionRegistryPostProcessor"]
    C1 --> C2["2. 纯 BeanFactoryPostProcessor\n（PriorityOrdered → Ordered → rest）"]
    C3["ConfigurationClassPostProcessor\n——最高优先级，先解析 @Configuration\n注册 @Bean 为 BeanDefinition"]
    C3 -.-> B1
...
Spring 官方文档对 [BeanFactoryPostProcessor](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-factory-extension-factory-postprocessors) 的定位是：修改 bean 定义，而不是 bean 实例。
...
主要还是看应用时机：
Spring 官方文档的 [BeanPostProcessor 小节](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-factory-extension-bpp) 解释了手动注册和自动注册的区别。
...
是一个空的protected方法，应该是写其他web相关的子类的时候需要干点儿什么，就在这里新定义了一个protected方法。
...
一个 singleton bean 从定义到就绪的完整生命周期：

```mermaid
flowchart TD
    A["BeanDefinition 已注册"] --> B["getBean()"]
    B --> C["InstantiationAwareBPP\n#postProcessBeforeInstantiation"]
    C --> D["反射创建实例\nBeanUtils.instantiateClass()"]
    D --> E["populate properties\n（依赖注入）"]
    E --> F["InstantiationAwareBPP\n#postProcessAfterInstantiation"]
...
### FactoryBean
`FactoryBean` 的特殊取值规则可以参考 Spring 官方文档的 [FactoryBean 小节](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-factory-extension-factorybean)。
...
至此，`ApplicationContext` 才算真正创建完成。
...
构造函数注入和 `@Autowired` 的规则可以参考 Spring 官方文档中的 [Autowired 注解说明](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-autowired-annotation)。
...
# Environment
Environment 和 profiles 可以继续参考 Spring 官方文档中的 [Environment 抽象](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-environment) 与 [Profile 配置](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-definition-profiles-enable)。

# @PropertySource
`@PropertySource` 对应的是 Spring 的 [PropertySource 抽象](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-property-source-abstraction)。

The @PropertySource annotation provides a convenient and declarative mechanism for adding a PropertySource to Spring’s Environment.

# @EventListener
使用注解，免去了接口继承。这是一个不错的思路：
注解式事件监听可以参考 Spring 官方文档中的 [`@EventListener`](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#context-functionality-events-annotation)。

# 关于Resource
`Resource` 是对 `java.net.URL` 的增强，更多背景见 Spring 官方文档的 [Resource 抽象](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#context-functionality-resources)。
...
BeanFactory 和 ApplicationContext 的差异可以参考 Spring 官方文档的 [BeanFactory 说明](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-beanfactory)。
...
# StartupStep
spring自己记录下每一步都有什么：
启动步骤追踪见 Spring 官方文档的 [ApplicationStartup](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#context-functionality-startup)。
