...
tags: [spring, springboot, test, slice-test, mockmvc]
description: "从 SpringBootTestContextBootstrapper、配置类探测、slice test、MockMvc 到 @MockBean，梳理 Spring Boot Test 的测试层次。"
---

Spring Boot Test 基于 [Spring Test]({% post_url 2022-11-25-spring-test %})，但它不是简单复用 `SpringExtension`。它用自己的 `SpringBootTestContextBootstrapper` 接管 context 构建过程，并叠加 Spring Boot 的配置类探测、自动配置、slice test、测试工具 bean 和 mock bean 支持。

[Spring Boot testing 文档](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.testing)里最重要的线索是：`spring-boot-test` 依赖 `spring-test`，使用 Spring Test 提供的 `@BootstrapWith` 构建自己的 context，至少提供了两方面便利：
...
本文先看 `SpringBootTestContextBootstrapper` 如何找到 primary configuration，再看 slice test 如何只加载某一层自动配置，最后比较 `MockMvc`、真实 server、`@MockBean` 和不同测试层次的取舍。

Spring Boot Test 的选择可以先按“加载多少上下文”和“是否启动真实 server”来分层：

```mermaid
flowchart TD
    Need["要测什么？"] --> Controller{"只测 MVC 层？"}
    Controller -->|"是"| WebMvc["@WebMvcTest<br/>slice context + MockMvc"]
    Controller -->|"否"| NeedFull{"需要完整 Bean 图？"}
    NeedFull -->|"否"| OtherSlice["@DataJpaTest / @JsonTest / ...<br/>局部自动配置"]
    NeedFull -->|"是"| NeedServer{"需要真实端口？"}
    NeedServer -->|"否"| BootMock["@SpringBootTest<br/>+ @AutoConfigureMockMvc"]
    NeedServer -->|"是"| BootServer["@SpringBootTest(webEnvironment=RANDOM_PORT)<br/>真实内嵌 server"]
    WebMvc --> MockBean["@MockBean<br/>替换缺失依赖"]
    BootMock --> MockBean

    classDef q fill:#fff3bf,stroke:#f9a825,color:#7a4f00
    classDef slice fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef full fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    class Need,Controller,NeedFull,NeedServer q
    class WebMvc,OtherSlice,MockBean slice
    class BootMock,BootServer full
```
...
默认`@SpringBootApplication`是带这个注解的。**所以`@*Test`其实就是在找`@SpringBootConfiguration`或者`@SpringBootApplication`**。
...
> 比较特殊的情况是，如果一个工程是纯lib项目，src里没有启动类，此时如果想用springboot test测这个包，那么test代码里就要写一堆configuration class，再写上一个`@SpringBootApplication`启动类，以进行自动配置。
...
找到`@SpringBootApplication`之后，**它默认是带`@ComponentScan`的**，所以scan的那些包里的配置也都找到了！和springboot一样的配置就都来了！ (o゜▽゜)o☆[BINGO!]

但是，**`@SpringBootApplication`带的这个`@ComponentScan`不是一个普通的`@ComponentScan`**，它其实是：
```java
...
一共有两个exclude filter：
- 第二个是名为auto configuration的exclude filter，所以它明显是为了排除掉`@EnableAutoConfiguration`配置的bean。但是因为`@SpringBootApplication`里额外加上了`@EnableAutoConfiguration`了，所以这些被`@ComponentScan`故意遗漏的bean又被`@EnableAutoConfiguration`加回来了。**为什么这么做？为了后面要介绍的slice test。springboot更倾向于slice test，所以不一次性配置所有的autoconfig，而选择只配置某一slice的autoconfig（加上哪个autoconfig注解，就只配置那个autoconfig）**；
- 第一个filter干嘛的？Javadoc提了一下：They are primarily used internally to `support spring-boot-test`。**它是特意为springboot test准备的。为了支持springboot的“部分测试”（slice test）**。它会排除掉springboot test里指定的一些`TypeExcludeFilter`。对于`@SpringBootApplication`，它没有指定任何`TypeExcludeFilter`。

所以，**实际上对于使用`@SpringBootApplication`的src代码来说，没有exclude掉任何bean。效果相当于没加任何东西的正常的`@ComponentScan`**。但是对于slice test，则不会加载所有的配置。
...
这些只关注slice的`@*Test`都配置了什么？除了从代码里找，还可以来这里找：
...
还得看`@SpringBootApplication`带的这个`@ComponentScan`：
```java
@ComponentScan(excludeFilters = { @Filter(type = FilterType.CUSTOM, classes = TypeExcludeFilter.class),
		@Filter(type = FilterType.CUSTOM, classes = AutoConfigurationExcludeFilter.class) })
```
...
### 不要加用户自己写的`@Component`
这一点官方文档在 [User Configuration and Slicing](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.testing.spring-boot-applications.user-configuration-and-slicing) 中也专门提醒过。
...
### 不侵入src代码
**但是这样算不算测试代码对src代码造成了影响**？如果不想刻意为了测试用例注意这些问题，可以自定义一个`@SpringBootConfiguration`，它什么也没有，就不会初始化任何bean。slice test还是只会初始化配置文件里写的那些autoconfig class里的bean。
...
[Testing Slices](https://docs.spring.io/spring-boot/docs/current/reference/html/howto.html#howto.testing.slice-tests) 一节介绍了这类场景的处理方式。

比如`@WebMvcTest`默认只配置mvc相关的bean，不配置service bean，可以使用`@Import`手动把service bean的config class导入到测试类里！

## 如果还想导入其他autoconfig class呢？
如果还想导入其他 auto-configuration class，可以参考 [Additional Auto-configuration and Slicing](https://docs.spring.io/spring-boot/docs/current/reference/html/features.html#features.testing.spring-boot-applications.additional-autoconfiguration-and-slicing)。
...
```
它是`@Configuration`的alias，所以spring test会把它当做一个正常的`@Configuration`（因为spring根本不知道`@TestConfiguration`是什么，只知道`@Configuration`），所以`@TestConfiguration`的static inner class可以[像`@Configuration`的static inner class一样被当做config class](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#testcontext-ctx-management-javaconfig)。**springboot test对它做了额外支持**，所以只有在springboot test里，它才不是一个正常的`@Configuration`。
...
# springboot测试的层次
参考：
- 代码示例：[spring-boot-testing-strategies](https://github.com/puppylpg/spring-boot-testing-strategies)
- 参考文章：[Guide to Spring Boot Controller Tests](https://thepracticaldeveloper.com/guide-spring-boot-controller-tests/)
...
# 感想
Spring Boot Test 看起来注解很多，但底层仍然是 Spring Test 的那套模型：测试框架触发生命周期，bootstrapper 决定怎样构建 `ApplicationContext`，自动配置决定加载哪些 bean。区别在于 Spring Boot Test 把这些步骤封装成了不同粒度的 `@*Test` 注解，让测试可以在“只测一个 slice”和“启动完整应用”之间选择。
