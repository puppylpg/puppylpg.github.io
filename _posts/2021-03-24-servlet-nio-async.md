...
tags: [nio, servlet]
description: "区分 Servlet NIO 和 Async Servlet：前者解开长连接与线程的绑定，后者在 server push 场景下解开长请求与线程的绑定。"
---
...
sequenceDiagram
    ...
    participant Selector as servlet container selector
    participant Http as http worker thread
    participant Servlet as servlet

    Client->>Selector: 建立长连接
    Selector-->>Selector: 等待 OP_READ / OP_WRITE
    Client-->>Selector: 请求数据到达
    Selector-->>Http: 分配线程处理本次 request
    Http->>Servlet: doPost()/doGet()
    Servlet-->>Http: 本次请求处理完成
    Http-->>Selector: 线程归还线程池
    Selector-->>Selector: 继续看守 connection
```

这张图的关键不是Selector很酷，而是**connection没事儿的时候不占http worker thread**。客户端慢、keep-alive空等这些脏活累活，交给Selector盯着；真正有请求了，再把线程叫过来干活。
...
nio的细节在哪里？servlet容器负责实现了，servlet开发者可以不关注。如果深入研究，其实就是**servlet容器使用[Java NIO]({% post_url 2020-10-29-java-nio %})介绍的jdk的Selector/ServerSocketChannel/SocketChannel实现了nio的流程，servlet开发者只需要告诉servlet容器socket读到那儿（ReadListener）、写什么（WriteListener）就够了**。
...
参阅：
- [Oracle Java EE Tutorial: Nonblocking I/O](https://docs.oracle.com/javaee/7/tutorial/servlets013.htm)
...
sequenceDiagram
    ...
    participant Http as http worker thread
    participant Async as AsyncContext queue
    participant Push as push worker

    Client->>Http: 建立一个长 request
    Http->>Async: startAsync() 保存 request/response
    Http-->>Http: 立即归还容器线程池
    Push-->>Push: 等待直播/拍卖/消息事件
    Push->>Async: 取出一批 AsyncContext
    Push-->>Client: 写 response / 推送事件
    Push->>Async: complete() 或继续挂起
```

所以Async Servlet的价值要看场景：如果只是一件活从A线程挪到B线程，确实没什么卵用；如果是一堆长request都在等同一个“新消息”事件，那就可以让它们统一挂到一个队列里，http线程该干嘛干嘛去。
...
参阅：
- [Oracle Java EE Tutorial: Asynchronous Processing](https://docs.oracle.com/javaee/7/tutorial/servlets012.htm)

## server push：长request绑定thread
分析一下servlet容器的发展：
- [InfoWorld: Asynchronous processing support in Servlet 3.0](https://www.infoworld.com/article/2077995/java-concurrency-asynchronous-processing-support-in-servlet-3-0.html)
- [InfoWorld: Asynchronous processing support in Servlet 3.0, page 2](https://www.infoworld.com/article/2077995/java-concurrency-asynchronous-processing-support-in-servlet-3-0.html?page=2)
...
- [Stack Overflow: Async Servlet server push 思路](https://stackoverflow.com/a/10933726/7676237)

周末代码实现一下，感觉可以直接搞个controller，使用httprequest获取AsyncContext即可。就像[这个Spring Boot例子](https://zhuanlan.zhihu.com/p/126495745)一样。
