...
categories: [java, io]
tags: [java, io]
description: "从介质流、包装流到转换流，串起 Java IO 读字节、读字符和字符集解码的核心链路。"
---
...
如果把它们画成“谁真正干活”的调用链，大概是这样：

```mermaid
sequenceDiagram
    participant App as 应用代码
    participant Buffer as BufferedInputStream 包装流
    participant File as FileInputStream 介质流
    participant Kernel as OS read
    participant Disk as 文件/磁盘

    App->>Buffer: read()
    alt 缓冲区还有 byte
        Buffer-->>App: 直接返回一个 byte
    else 缓冲区空了
        Buffer->>File: fill()
        File->>Kernel: native read()
        Kernel->>Disk: 读取文件块
        Disk-->>Kernel: bytes
        Kernel-->>File: bytes
        File-->>Buffer: 填满 byte[]
        Buffer-->>App: 返回一个 byte
    end
```

看懂这条链路之后，很多类名就没那么吓人了：**包装流负责加功能，介质流负责碰真实资源**。包装再多层也没关系，最后总得有一个人真的去读磁盘、内存、socket。层层转包是可以的，但不能全是中间商，不然包工头们一起喝西北风。
...
> 返回值同样为int，不过Reader使用的是低16bit。InputStream读的是byte，使用的是int的低8bit。
...
StreamDecoder利用InputStream读取byte，之后使用CharsetDecoder将 byte decode 为char。这个decode过程就是上面说的将文件编码转为UTF-16（char的编码）的过程。
...
sequenceDiagram
    ...
    participant CD as CharsetDecoder

    App->>ISR: read(char[])
    ISR->>SD: read(char[], off, len)
    SD->>IS: 读取原始 bytes
    IS-->>SD: 文件编码下的 bytes
    SD->>CD: decode(ByteBuffer, CharBuffer)
    CD-->>SD: UTF-16 char
    SD-->>ISR: char[]
    ISR-->>App: 字符数据
```

所以字符流不是“直接从文件里掏出了字符”。更准确地说，它是**先按字节读，再按字符集翻译成 Java 的 char**。文件编码搞错，翻译就搞错，乱码就会像下班前的线上告警一样准时出现。
...
# 其他
## FileReader读文件的问题
byte转char要按照文件原有编码转，所以要明确指定要读的文件的字符集，否则就用系统默认字符集。但是新问题来了：在JDK 8这些老版本里，FileReader并没有一个能指定Charset的构造函数！！！这是什么迷幻操作……

所以**如果在老版本里使用FileReader读取文件，只能使用系统默认的Charset**，比如UTF-8。但是系统要读的文件可以以任何编码存储，比如UTF-16！这样就相当于使用UTF-8编码标准去读一个UTF-16编码的文件，就会存在乱码问题！
...
JDK 11之后，FileReader终于补上了带Charset的构造函数，就能少写个InputStreamReader和一个FileInputStream：
```java
...
多合理！看起来也不晕。不知道Java当年怎么想的，过了这么久才给FileReader加这个构造函数。

> 这句吐槽保留一下历史现场：老版本确实没有，后来终于补了。迟到总比不到好，但当年看到那一长串构造函数的时候，人是真的会沉默。
