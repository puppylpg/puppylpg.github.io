...
description: "Elasticsearch 数据重分配机制：routing shard、虚拟分片、split API，以及和 rehash、一致性哈希、Redis slot 的对比。"
---
...
flowchart LR
    A[_routing] --> B[hash]
    B --> C[num_routing_shards<br/>虚拟分片]
    C --> D[routing_factor]
    D --> E[num_primary_shards<br/>真实 primary shard]
    E --> F[Lucene shard 文件]
...
| 方案 | 移动粒度 | 适合对象 | ES 为什么不用/怎么借鉴 |
| --- | --- | --- | --- |
| 普通 rehash | 几乎所有 key | 小数据、内存结构 | 文档重建索引太贵 |
| 一致性哈希 | 约 `1/N` key | KV 分布式系统 | 仍要移动并重建 Lucene 文档 |
| HashMap 一分二 | 原桶约一半数据 | 内存 HashMap | 思路类似，但 ES 需要预留虚拟分片 |
| Redis slot | 固定 slot | Redis Cluster | slot 暴露给集群元数据管理 |
| ES routing shard | segment/虚拟分片 | Lucene 文件 | 借硬链接和 delete-by-query 避免完整 reindex |
...
sequenceDiagram
    ...
    participant N as new split index
    participant FS as filesystem
    participant L as Lucene

    O->>N: 创建相同 mapping/settings 的目标索引
    N->>FS: hard-link/copy 原 segment
    N->>L: 重新按 routing shard 判断归属
    L->>N: delete-by-query 标记不属于本 shard 的文档
    N->>N: recovery 后开放新索引
```
...
> 虽然split api需要先设置索引禁止写入，看起来还不如reindex（新建一个索引重新设置主分片数），但是根据上面的介绍，在数据量比较大的时候，**使用split api直接copy索引的segment文件并删除一半数据，一定比reindex快非常多！**
