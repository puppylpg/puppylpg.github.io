...
description: "解释 Tomcat 为什么需要自己的 Loader/WebappClassLoader，以及 WEB-INF/classes/lib 的隔离边界。"
---
...
```mermaid
flowchart TD
    Bootstrap["JVM / System ClassLoader"] --> TomcatCore["Tomcat 自身类"]
    Context["Web 应用 Context"] --> Loader["WebappLoader"]
    Loader --> WebappCL["WebappClassLoader"]
    WebappCL --> Classes["WEB-INF/classes"]
    WebappCL --> Lib["WEB-INF/lib/*.jar"]
    WebappCL -. "隔离应用代码，避免直接乱摸 Tomcat 内部" .-> Boundary["ClassLoader 边界"]

    style WebappCL fill:#e3f2fd,stroke:#1976d2
    style Boundary fill:#fff3bf,stroke:#f59f00
    style TomcatCore fill:#e8f5e9,stroke:#2e7d32
```
