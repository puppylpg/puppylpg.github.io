...
tags: [java, jvm, gc]
description: "从可达性分析、分代假说、跨代引用、三色标记到 CMS、G1、ZGC，串起 JVM 垃圾收集的核心设计取舍。"
---
...
flowchart LR
    ...
    style Root1 fill:#e8f5e9,stroke:#51cf66
    style Root2 fill:#e8f5e9,stroke:#51cf66
    style A fill:#e3f2fd,stroke:#4dabf7
    style B fill:#e3f2fd,stroke:#4dabf7
    style C fill:#e3f2fd,stroke:#4dabf7
    style D fill:#ffe3e3,stroke:#ff6b6b
    style E fill:#ffe3e3,stroke:#ff6b6b
```

上图里 D 和 E 虽然抱团取暖，但从 GC Roots 走不到它们，所以依然是垃圾。**“有没有引用我”不是关键，“能不能从活着的根走到我”才是关键。**
...
flowchart LR
    ...
    New --> Eden
    Eden -->|"Minor GC 后仍存活"| S0
    Eden -->|"Minor GC 后不可达"| Dead
    S0 -->|"下一次 Minor GC 存活"| S1
    S1 -->|"年龄继续增长"| S0
    S0 -->|"年龄达到阈值 / Survivor 放不下"| Old
    S1 -->|"年龄达到阈值 / Survivor 放不下"| Old

    style Eden fill:#fff3bf,stroke:#f59f00
    style S0 fill:#e3f2fd,stroke:#4dabf7
    style S1 fill:#e3f2fd,stroke:#4dabf7
    style Old fill:#e8f5e9,stroke:#51cf66
    style Dead fill:#ffe3e3,stroke:#ff6b6b
```

这个生命周期解释了为什么 Minor GC 通常快：它赌的是“新对象大多马上死”，只要赌对，Eden 里绝大多数空间一下就空了。
...
> 参考：[Oracle：Available Collectors](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/collectors.html)
...
- `-XX:+UseAdaptiveSizePolicy`：自动设定新生代大小（`-Xmn`）等细节，自动设定Eden与Survivor的比例（`-XX:SurvivorRatio`），自动设定晋升老年代对象大小（`-XX:PretenureSizeThreshold`）；
...
sequenceDiagram
    ...
    participant GC as CMS GC 线程

    App->>GC: 触发 CMS
    Note over App,GC: STW: initial mark，枚举 GC Roots
    GC-->>App: 用户线程恢复
    par 并发标记
        GC->>GC: tracing reachable objects
    and 继续执行业务
        App->>App: 修改引用 / 产生浮动垃圾
    end
    Note over App,GC: STW: remark，修正并发阶段的引用变化
    par 并发清除
        GC->>GC: sweep unreachable objects
    and 继续执行业务
        App->>App: 继续分配对象
    end
```

CMS 的核心就是把最耗时的标记、清除阶段挪到“和业务线程并发”里，代价则是需要处理浮动垃圾、碎片和 concurrent mode failure。
...
所以G1的主要目的是为大内存应用、面向server、多处理器场景提供更优的GC解决方案。
...
```mermaid
flowchart TB
    subgraph Heap["G1 Heap: 一堆大小相等的 Region"]
        R1["Eden"]
        R2["Old"]
        R3["Survivor"]
        R4["Old\n垃圾很多"]
    ...
    R4 -->|"Garbage First\n优先回收收益最高的 Region"| R8
    R1 -->|"Young GC / Mixed GC\n存活对象复制"| R3

    style R1 fill:#fff3bf,stroke:#f59f00
    style R6 fill:#fff3bf,stroke:#f59f00
    style R3 fill:#e3f2fd,stroke:#4dabf7
    style R2 fill:#e8f5e9,stroke:#51cf66
    style R4 fill:#ffe3e3,stroke:#ff6b6b
    style R7 fill:#e8f5e9,stroke:#51cf66
    style R5 fill:#f3d9fa,stroke:#cc5de8
    style R8 fill:#f8f9fa,stroke:#adb5bd
```

G1 的“分代”是逻辑身份，不是连续内存块；它真正抓住的是 Region 这个回收粒度。谁垃圾多、回收收益高，就优先处理谁。
...
上述场景将grey对象（C）到white对象（D）的引用删除了，并插入到了black对象（B）上，导致D始终是white。
...
>
> 漏删垃圾是可以接受的，错删对象程序就错了。参考：[G1 SATB 相关解释](https://www.jianshu.com/p/cc6b98b1640e)
...
- [Oracle：Garbage-First Garbage Collector](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/g1_gc.html)
- [Oracle：G1 GC Tuning](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/g1_gc_tuning.html)
- [G1 相关中文资料](https://mp.weixin.qq.com/s/vmnBlrM7pTtVuyQU-GTcPw)

### 总结
**在标记垃圾的方式上，并发垃圾处理器使用了并发标记，能有效减少STW**。但是并发就会涉及到线程之间的抢占、通信，导致gc线程的效率不如并行收集器里的gc线程。可以理解为并行收集器的线程更专注于清垃圾（不用线程间考虑并发），所用总时间更短。
...
#### 分代
早期 ZGC 是不分代的，所以不涉及跨代引用（G1的RSet）的问题。后来 JDK 21 通过 [JEP 439：Generational ZGC](https://openjdk.org/jeps/439) 引入了分代 ZGC，ZGC 也要通过二八定律让自己变得更高效。
...
这些理念是递进的，实现起来也是越来越复杂的，但都是很合理的。一个垃圾收集器未必会实现所有的方面，比如G1支持二八定律但清理垃圾的时候用的是STW，很先进但又不完全先进；早期 ZGC 低延迟做得漂亮，但分代是后来才补上的。等等。
...
```
参阅：
- [Baeldung：`-verbose:gc`](https://www.baeldung.com/java-verbose-gc)
- [Baeldung：GC logging to file](https://www.baeldung.com/java-gc-logging-to-file)
- [JDK 9 JVM `-Xlog` 参数说明](https://www.cnblogs.com/flydean/p/jdk9-jvm-xlog.html)
...
参阅：
- [Why avoid `UseGCLogFileRotation`](https://dzone.com/articles/try-to-avoid-xxusegclogfilerotation)

# 其他jvm参数
一些非标准jvm参数（`-XX`）：
- [Oracle：Java VM options](https://www.oracle.com/java/technologies/javase/vmoptions-jsp.html)
...
参阅：
- [Baeldung：Handling OutOfMemoryError](https://www.baeldung.com/jvm-parameters#handling-out-of-memory)
