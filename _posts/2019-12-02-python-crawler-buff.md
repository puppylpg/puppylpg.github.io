...
tags: [python, csgo]
description: "记录一次网易 BUFF CSGO 饰品爬虫的 API 探索过程：从页面源码、Network 请求、Cookie 登录，到价格计算和反爬取舍。"
---
...
flowchart LR
    Category["BUFF 类别列表\ncategory"]
    Goods["goods API\n每个饰品的 buff 价格、goods_id"]
    History["price_history API\nSteam 最近成交价"]
    Filter["过滤条件\n价格区间 / 成交量 / 冷门饰品"]
    Rank["收益测算\n扣税后差价 / ROI"]
    ...
    style Category fill:#e3f2fd,stroke:#4dabf7
    style Goods fill:#fff3bf,stroke:#f59f00
    style History fill:#fff3bf,stroke:#f59f00
    style Filter fill:#e8f5e9,stroke:#51cf66
    style Rank fill:#ffe3e3,stroke:#ff6b6b
```

所以这事儿看起来像“爬网页”，本质上其实是**找到两类数据接口，然后把它们用同一个`goods_id`串起来**。能找到 API，后面才是写代码；找不到 API，就只能人肉抠页面，想想都头大。

# API
## 获取所有饰品
首先我们需要一个爬取buff上所有饰品的办法。

在[BUFF 饰品市场第一页](https://buff.163.com/market/?game=csgo#tab=selling&page_num=1)，随便打开一个饰品，比如鲍伊猎刀，观察它的访问按钮，网页显示的是`<li value="weapon_knife_survival_bowie">鲍伊猎刀</li>`，点击之后 URL 变成带有`category=weapon_knife_survival_bowie`的类别页。所以猜测它的访问办法就是前面的路径加上`category`参数即可。试了一下果然就是这样。
...
比如浏览器访问[鲍伊猎刀类别页](https://buff.163.com/market/?game=csgo#tab=selling&page_num=1&category=weapon_knife_survival_bowie)，返回的是所有的鲍伊猎刀。本来还想用和上面同样的套路：看网页源码，用正则匹配出所有的鲍伊猎刀种类，然后抠出来。结果一看源码我人傻了：没有！网页上明明显示各式各样的鲍伊猎刀，但是网页源码里竟然没这些数据，只有一些类似于JSP那种嵌入网页的模板代码。像这种：
```json
    <% } else { %>
...
flowchart TD
    Source["查看网页源码"]
    Template["只看到模板\n没有真实饰品列表"]
    Network["打开 DevTools Network"]
    Json["发现 goods.json"]
    API["直接请求 goods API"]

    Source --> Template
    Template -->|"页面内容不是源码直接给的"| Network
    Network --> Json
    Json --> API

    style Source fill:#e3f2fd,stroke:#4dabf7
    style Template fill:#ffe3e3,stroke:#ff6b6b
    style Network fill:#fff3bf,stroke:#f59f00
    style Json fill:#e8f5e9,stroke:#51cf66
    style API fill:#e8f5e9,stroke:#51cf66
```

这一步是整篇里最关键的转折：**不要执着于“网页源码里为什么没有”，而是反过来问“浏览器最终展示的数据从哪里来”**。
...
开心到飞起啊！这么一来，甚至都不需要用正则抓网页数据了，直接请求这个数据就行了！它的API是 BUFF 的 goods 接口，核心参数是`game=csgo&page_num=1&category=weapon_knife_survival_bowie`，既然可以知道总共6页，把url中的`page_num`从1遍历到6，所有115条鲍伊猎刀的数据不就全有了吗！
...
它走的是 BUFF 的 price history 接口，核心参数是`game=csgo&goods_id=776332&currency=&days=7`。看了看，比较复杂的就是`goods_id`了。这把刀在buff的id是776332。如果我知道所有饰品的id，然后替换到这个参数里，就能查到所有饰品的steam最近交易价格了！
...
参阅：
- [Python requests 使用 Cookie 登录示例](https://www.cnblogs.com/chenxiaohan/p/7654667.html)
- [Requests Session 和 Cookie 示例](https://www.cnblogs.com/jiayongji/p/7143662.html)
...
但是再加上steam历史价格，每个饰品都要爬一次，10000多条饰品要爬10000多次……爬取量一下子翻了好几十倍，这样就太慢了……所以我后来又加了一些限制，比如价格高于100的才爬售价纪录，其他饰品直接忽略了等等。这样就把饰品数量从10000多点儿降到了3500种左右。当然这是后话。
...
python中使用代理的做法参阅：
- [ScrapeHero：How to rotate proxies and IP addresses using Python 3](https://www.scrapehero.com/how-to-rotate-proxies-and-ip-addresses-using-python-3/)
...
获取免费代理的方法参阅：
- [免费代理获取思路](https://blog.csdn.net/weixin_43968923/article/details/86682068)
...
延伸阅读：
- [ScrapeHero：How to prevent getting blacklisted while scraping](https://www.scrapehero.com/how-to-prevent-getting-blacklisted-while-scraping/)
...
单位价钱收益最大——
```
首先，今天（2019年12月2日）如果buff买饰品卖到steam里：
```bash
...
# 代码
最后最重要的，代码在哪儿？
- [buff_csgo_skin_crawler](https://github.com/puppylpg/buff_csgo_skin_crawler)
