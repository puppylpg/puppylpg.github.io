...
description: "解释 HttpMessageConverter 如何根据 Content-Type 和 Accept，在客户端与服务端完成读写转换。"
---
...
```mermaid
flowchart TD
    Request["HTTP Request"] --> ContentType["Content-Type<br/>请求体是什么格式"]
    Request --> Accept["Accept<br/>希望响应是什么格式"]
    ContentType --> CanRead{"converter.canRead?"}
    CanRead -->|是| Read["read()<br/>body -> Java Object"]
    Accept --> CanWrite{"converter.canWrite?"}
    CanWrite -->|是| Write["write()<br/>Java Object -> response body"]
    CanRead -->|否| Unsupported["415 Unsupported Media Type"]
    CanWrite -->|否| NotAcceptable["406 Not Acceptable"]

    style ContentType fill:#e3f2fd,stroke:#1976d2
    style Accept fill:#fff3bf,stroke:#f59f00
    style Unsupported fill:#ffe3e3,stroke:#c62828
```
