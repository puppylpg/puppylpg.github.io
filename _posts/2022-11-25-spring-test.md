...
categories: [tech, spring, test]
tags: [spring, test, junit-jupiter, testcontext]
description: "从 TestContextManager、TestContext、TestExecutionListener 和 ContextLoader 出发，理解 Spring TestContext Framework。"
---

Spring Test 的核心不是某个具体注解，而是 TestContext Framework：它把测试框架的生命周期接到 Spring 容器上，在测试执行前后创建或复用 `ApplicationContext`，并通过 listener 完成依赖注入、事务回滚、SQL 脚本执行和事件发布。
...
本文先拆解 `TestContextManager`、`TestContext`、`TestExecutionListener`、`ContextLoader` 这几个核心抽象，再看它们如何和 Jupiter、Spring Boot、MyBatis 等上层测试注解组合起来。

从一次 test method 的执行过程看，Spring Test 主要是在 JUnit 生命周期前后插入自己的 `TestContextManager`：
...
sequenceDiagram
    ...
    J->>E: beforeEach / beforeAll
    E->>M: prepareTestInstance()
    M->>C: get or create TestContext
    C->>AC: load or reuse ApplicationContext
    M->>L: beforeTestMethod()
    L->>T: dependency injection / transaction / sql
    J->>T: execute test method
    J->>E: afterEach
    E->>M: afterTestMethod()
    M->>L: rollback / cleanup / events
```
...
5. 开发者：作为springboot使用者，写自己的测试代码，使用`@SpringBootTest`，也可能会手动添加上`@ContextConfiguration`等；
...
其实`AnnotationConfigContextLoader`加载config class最终还是用的`new AnnotatedBeanDefinitionReader(context).register(componentClasses)`，和非test spring代码创建的`AnnotationConfigApplicationContext`里的`new AnnotatedBeanDefinitionReader(this)`其实几乎一致。
...
**其实spring test可以看做spring core的另一种套壳，是spring core的另一种打开方式。进入spring core不再使用main函数，而是通过junit框架，通过`SpringExtension`，通过暴露给开发者的`@Test`测试类。**

比如同样是启动`ApplicationContext`：
- **spring可以直接`new AnnotationConfigApplicationConfig(config class)`**；
- **spring test因为不让开发者在test代码里显式创建test的`ApplicationContext`（如果写测试代码还要手动new一个context，那也太麻烦了），所以没法直接new，而是spring test框架通过`SpringExtension`自己new。那么new的时候怎么指定开发者想要的config class呢？于是spring test提供了个`@ContextConfiguration`注解，可以写在test class上，供开发者配置config class（看起来有点儿像缺什么就拿根棍儿随便搭一下）**。
...
`@TestPropertySource`指定properties文件的路径和`@ContextConfiguration`指定xml配置文件路径语法几乎一致。
...
具体机制见 [Bootstrapping the TestContext Framework](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#testcontext-bootstrapping)。
...
可参考 Spring Core 文档里的 [Using Meta-annotations and Composed Annotations](https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-meta-annotations)。
...
Spring Test 自己给出的 [TestExecutionListeners 组合注解示例](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#spring-testing-annotation-testexecutionlisteners)也很清楚。

# 感想
Spring Test 和 Spring Boot Test 最容易混淆的地方，是测试注解很多，但背后的启动模型其实很稳定：测试框架提供生命周期，Spring Test 接入生命周期并维护 `TestContext`，Spring Boot Test 再替换 bootstrapper 和 context loader。先把这条链路理顺，再看各种 `@*Test` 注解会轻松很多。
