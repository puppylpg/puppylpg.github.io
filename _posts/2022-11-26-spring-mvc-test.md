...
categories: [tech, spring, mvc, test]
tags: [spring, mvc, mockmvc, test]
description: "从 MockMvc 的使用方式进入源码，解释它如何在没有 Tomcat 的情况下模拟 Servlet 和 DispatcherServlet 请求处理。"
---
...
本文先讲 `MockMvc` 为什么比直接调用 controller 更接近真实 MVC 流程，再看 standalone 和 `WebApplicationContext` 两种构建方式，最后深入源码说明 `MockMvc` 如何构造 `DispatcherServlet` 并手动执行 filter chain。

`MockMvc` 最值得记住的一点是：它保留 Spring MVC 的完整处理链，但把网络和真实 server 拿掉了。

```mermaid
sequenceDiagram
    participant Test as Test Method
    participant MockMvc as MockMvc
    participant Req as MockHttpServletRequest
    participant Filter as Mock FilterChain
    participant DS as DispatcherServlet
    participant Ctrl as Controller
    participant Resp as MockHttpServletResponse

    Test->>MockMvc: perform(get("/api"))
    MockMvc->>Req: 构造 mock request
    MockMvc->>Filter: 执行 filter chain
    Filter->>DS: service(request, response)
    DS->>Ctrl: handler method
    Ctrl-->>DS: return value / exception
    DS-->>Resp: 写入 status/body/header
    Resp-->>MockMvc: MvcResult
    MockMvc-->>Test: andExpect(...)
```
...
等价的 `WebTestClient` 用法可以参考官方文档中的 [controller config](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#webtestclient-controller-config)。
...
等价的 `WebTestClient` 用法可以参考官方文档中的 [context config](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#webtestclient-context-config)。
...
Spring 官方文档也专门比较了 [`MockMvc` 与 end-to-end tests](https://docs.spring.io/spring-framework/docs/current/reference/html/testing.html#spring-mvc-test-vs-end-to-end-integration-tests) 的边界。
...
`ServletConfig`也准备好了，接下来就要创建SpringMVC里唯一的servlet了——`DispatcherServlet`。这里用到的实现是`TestDispatcherServlet`：它override了`DispatcherServlet`，把`DispatcherServlet`处理后的结果放到了spring test的`MvcResult`里，其他跟DispatcherServlet没什么区别。
...
2. `ServletConfig`内含`ServletContext`：`ServletContext`的名字为"`MockServletContext`"，不过这个名字没什么卵用。
    1. **context path设置为空**；
...
# 感想
`MockMvc` 不只是一个方便的 web layer 测试工具。它把 servlet、filter、`DispatcherServlet` 和 Spring MVC 组件压缩到同一个线程里执行，反而让 Spring MVC 的请求处理链路更容易被观察。理解 `MockMvc` 之后，再看真实 Tomcat 下的 Spring MVC 流程会清楚很多。
