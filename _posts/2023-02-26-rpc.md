...
description: "从本地方法调用的栈帧出发，推导 RPC 必须解决的方法表示、数据表示和数据传输问题。"
---

如果要搞分布式，那必然涉及到服务之间的相互调用，即远程过程调用（Remote Procedure Call，RPC）。
...
```mermaid
flowchart LR
    Local["本地方法调用"] --> Stack["栈帧<br/>参数 / 返回地址 / 局部变量"]
    Local --> PC["PC 指向被调方法地址"]
    Local --> Return["返回值写回调用方"]

    RPC["RPC"] --> Method["方法表示<br/>接口名 / 方法名 / 参数类型"]
    RPC --> Data["数据表示<br/>序列化 / 字节序 / 类型系统"]
    RPC --> Transport["数据传输<br/>TCP / HTTP / IPC / QUIC"]
    RPC --> Result["返回值与异常<br/>反序列化给调用方"]

    style Local fill:#e3f2fd,stroke:#1976d2
    style RPC fill:#fff3bf,stroke:#f59f00
...
![linux memory](/pics/linux/memory/memory-layout-c.jpg)
