...
tags: [elasticsearch]
description: "Elasticsearch 入门笔记：Docker 环境、文档结构、mapping、常见 field type 和基本 CRUD。"
---
...
参考文档：
- [Kibana 7.12 Docker 文档](https://www.elastic.co/guide/en/kibana/7.12/docker.html)
...
- [Stack Overflow：调整 WSL `vm.max_map_count`](https://stackoverflow.com/a/66547784/7676237)
- [TechOverflow：让 initial heap size 和 maximum heap size 一致](https://techoverflow.net/2020/12/16/how-to-fix-elasticsearch-1-initial-heap-size-not-equal-to-maximum-heap-size/)

> 还是linux好……想念Arch的第N天……想念Debian的第N/2天……

启动es，主要是设置jvm内存启动项，且必须让xms和xmx一致以解决上面的第一个问题；`vm.max_map_count`另行调大：
```dockerfile
...
- [本地 Kibana Dev Tools](http://localhost:5601/app/dev_tools#/console)

# 入门
- [中文入门文档](https://www.elastic.co/guide/cn/elasticsearch/guide/current/getting-started.html)
- [英文入门文档](https://www.elastic.co/guide/en/elasticsearch/reference/current/getting-started.html)
...
> 各种“索引”可以参考[官方员工文档示例](https://www.elastic.co/guide/cn/elasticsearch/guide/current/_indexing_employee_documents.html)。
...
```
- [官方 `_routing` 字段文档](https://www.elastic.co/guide/en/elasticsearch/reference/current/mapping-routing-field.html)
...
# 索引
## mapping
[Mapping](https://www.elastic.co/guide/en/elasticsearch/reference/current/mapping.html) 是文档的结构定义，相当于mysql的schema。但是不同于mysql严格的数据结构定义，mapping支持strict和dynamic两种类型。
