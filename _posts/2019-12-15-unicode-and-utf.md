...
description: "梳理 Unicode、UTF-8、UTF-16、UTF-32、代理对、BOM，以及 Java char、String.length() 和 code point 遍历的关系。"
---
...
flowchart LR
    ...
    style CodePoint fill:#e3f2fd,stroke:#4dabf7
    style UTF8 fill:#e8f5e9,stroke:#51cf66
    style UTF16 fill:#fff3bf,stroke:#f59f00
    style Bytes fill:#ffe3e3,stroke:#ff6b6b
```

这张图先把关系钉死：**Unicode 负责编号，UTF 负责把编号变成字节**。乱码大多发生在最后一步：用 A 编码写出的字节，被当成 B 编码读了。
...
查看Unicode Plane及内容：
- [Wikipedia：Unicode plane overview](https://en.wikipedia.org/wiki/Plane_(Unicode)#Overview)
- [Wikibooks：Unicode 7000-7FFF](https://en.wikibooks.org/wiki/Unicode/Character_reference/7000-7FFF)
...
**所以实际上成了一个进不如UTF-8，退不如UTF-32的尴尬方案。**
...
上面只是一个示例，实际上Unicode除了BMP，还有16块补充面板，每块面板`0x10000`个码点，所以除了BMP一共还有`0x100000`个码点需要编码，即`2^20 = 2^10 * 2^10`个。

所以UTF-16的两字节空间中，共计`2^10 + 2^10`个码点不编码字符，就可以组合后面需要的`2^10 * 2^10`个码点。**所以UTF-16没有使用2byte编码BMP中的所有字符，而是预留了`0x400 + 0x400 = 0x800`个码点不编码字符**，使用这0x800个（2048个）码点，前后两两组合出了后面16个面板所需要的`0x100000`个（1024 * 1024个）码点。

这两个`0x400`空间的码点，UTF-16选的是`0xD800-0xDBFF`和`0xDC00-0xDFFF`。

```mermaid
flowchart LR
    CP["补充面板码点\nU+10000 ~ U+10FFFF"]
    Minus["减去 0x10000\n得到 20 bit"]
    Split["拆成高 10 bit + 低 10 bit"]
    High["High Surrogate\n0xD800 + 高 10 bit"]
    Low["Low Surrogate\n0xDC00 + 低 10 bit"]
    Units["两个 UTF-16 code units\n也就是两个 Java char"]
    ...
    style CP fill:#e3f2fd,stroke:#4dabf7
    style High fill:#fff3bf,stroke:#f59f00
    style Low fill:#fff3bf,stroke:#f59f00
    style Units fill:#e8f5e9,stroke:#51cf66
```

代理对不是两个字符，而是**两个 UTF-16 code unit 合起来表示一个补充面板码点**。这句话后面理解 Java `char` 和 `String.length()` 会反复用到。

> BMP的字符都是2 byte表示，supplementary plane的字符都是4 byte表示。

延伸阅读：
- [UTF-16 surrogate pair 解释](https://stackoverflow.com/a/47505451/7676237)
...
可以查看一下 Unicode 在 BMP 的 [D000-DFFF 码点表](https://en.wikibooks.org/wiki/Unicode/Character_reference/D000-DFFF)，`0xD800-0xDFFF`这一块确实没有编码任何字符。
...
- [UTF-16 字符集列表](http://www.fileformat.info/info/charset/UTF-16/list.htm)

## UTF-32: use 32-bit code unit
UTF-32使用32bit（4 byte）作为一个编码的基本单元。显然UTF-32就是**定长编码**了，因为就算按定长去编码，$2^{32}$依然能表示出Unicode当前总共约$2^{21}$的编码空间。
...
`café`算几个字符？或者说几个码点？主要涉及到`é`，它可以是[字母e（U+0065）](http://www.ltg.ed.ac.uk/~richard/utf-8.cgi?input=e&mode=char)和[组合重音符号（U+0301）](http://www.ltg.ed.ac.uk/~richard/utf-8.cgi?input=0301&mode=hex)组合出来的，也可以是[一个单字符é（U+00E9）](http://www.ltg.ed.ac.uk/~richard/utf-8.cgi?input=00e9&mode=hex)。是的，就是这么神奇……
...
参阅：
- [Twitter/X counting characters 文档](https://developer.twitter.com/en/docs/basics/counting-characters)

# Java
可怜的Java，因为历史原因，导致现在内部还使用UTF-16表示字符串……

## char
在Java中，一个`char`是2 byte，代表的是一个 **UTF-16 code unit**。对于 BMP 中的非代理区字符，一个 code unit 的数值通常就等于它的 Unicode 码点；但对于补充面板字符，一个码点必须拆成两个 code unit，也就是两个`char`。

所以更准确的说法是：**Java 的`char`和 UTF-16 code unit 一一对应，不和“字符”一一对应，也不总是和 Unicode code point 一一对应**。只是大多数常用字符在 BMP 非代理区里，一个`char`刚好够用，于是大家平时才容易把这几个概念混成一锅粥。

```mermaid
flowchart LR
    BMP["BMP 非代理区码点\n例如 U+51AC 冬"]
    Char1["1 个 UTF-16 code unit\n1 个 Java char"]
    Supplement["补充面板码点\n例如 U+1F600 😀"]
    Pair["2 个 UTF-16 code units\n2 个 Java char"]
    String["Java String"]

    BMP --> Char1 --> String
    Supplement --> Pair --> String

    style BMP fill:#e8f5e9,stroke:#51cf66
    style Supplement fill:#fff3bf,stroke:#f59f00
    style Pair fill:#ffe3e3,stroke:#ff6b6b
```

**一个`char`只能表示一个 BMP 中（非代理区域）的字符。如果想表示补充面板的字符，要用两个`char`**！两个代理区域的 code unit 组合出一个补充面板的码点。
...
```java
        // use surrogate pair
        String smile0 = "\uD83D\uDE00";
        System.out.printf("smile0: %s%n", smile0);
```
要么指明露齿笑在补充面板的码点，然后使用`StringBuilder#appendCodePoint`或者`Character#toChars`将补充面板的码点**转换成两个`char`（两个代理区域的 code unit），最终只能以`String`的形式保存（因为一个`char`保存不下）**：
```java
...
参阅：
- [Char to Unicode more than U+FFFF in Java](https://stackoverflow.com/questions/9834964/char-to-unicode-more-than-uffff-in-java)

因此对于接收`char`作为参数的方法，如果传进来的字符是supplementary plane上的字符，实际只传进去了第一个 code unit（半个代理对，被截断了）。
...
Ref:
- [Why is Java char 2 bytes](https://stackoverflow.com/a/9354024/7676237)
...
由于 Java 的`char`和 UTF-16 code unit 一一对应，所以在 Java 8 及之前，其实返回的就是`char[]`中`char`的个数：
```java
...
```
对于java9，只是内部存储方式不同，虽然实现上不再用char数组表示了，但不影响逻辑上用到的 UTF-16 code unit 个数，因此并不影响接口本身：`length()`返回的依旧是 code unit 的个数。只不过现在计算有多少个 code unit 的时候不像原来直接获取`value.length`那么简单了，要分情况讨论：
```java
...
```
- 如果的确是紧凑字符串，byte数组的长度就是 code unit 的长度：`value.length >> 0`；
- 如果不是紧凑字符串，byte数组的长度是 code unit 长度的二倍：`value.length >> 1`；

具体而言，补充面板的字符都需要两个`char`来表示，也就是两个 code unit，所以**它们每个字符的`length()`就是2**：
```java
        // 一个像“冬”但不是“冬”的文字
...
| 内容 | Unicode code point 数 | Java `String.length()` |
|------|------------------------|------------------------|
| `A` | 1 | 1 |
| `冬` | 1 | 1 |
| `😀` | 1 | 2 |
...
所以`length()`不是“用户看到的字符数”，而是 UTF-16 code unit 数。这个坑在 emoji、补充面板汉字、组合字符里都会冒出来。
...
```
这样无论用print还是println，一定都能正确输出，因为每次都拿着一个完整的字符在操作。
...
参阅：
- [A Programmer's Introduction to Unicode](http://reedbeta.com/blog/programmers-intro-to-unicode/#diversity-and-inherent-complexity)
- [CharacterDemo.java](https://github.com/puppylpg/java-examples/blob/master/src/main/java/example/unicode/CharacterDemo.java)
