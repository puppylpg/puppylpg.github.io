...
description: "解释 CAS 与锁的取舍：乐观更新、失败重试、ABA 问题、AtomicStampedReference 的装箱坑，以及 Java 原子变量类的底层 CAS 支持。"
---
...
flowchart TD
    ...
    Read --> Compute --> Check
    Check -->|"是"| Swap
    Check -->|"否，说明被别人改过"| Retry
    Retry -->|"计数器 +1 这种场景"| Read
    Retry -->|"一次性状态切换场景"| End

    style Check fill:#fff3bf,stroke:#f59f00
    style Swap fill:#e8f5e9,stroke:#51cf66
    style Retry fill:#ffe3e3,stroke:#ff6b6b
```

所以 CAS 的关键不是“永远成功”，而是**失败时线程还活着**。它没有被挂起，可以立刻重试，也可以根据业务含义判断“别人已经替我做完了，那我不干了”。
...
ABA 的本质是：**值相同不代表历史没发生过**。如果数据结构的正确性依赖“中间有没有被动过”，那就必须额外带版本号。
...
```
`set`和`compareAndSet`都要提供一个新的stamp，一般设置为`AtomicStampedReference#getStamp() + 1`。

参考：
- [ABA 问题和 AtomicStampedReference 示例](https://www.cnblogs.com/549294286/p/3766717.html)
