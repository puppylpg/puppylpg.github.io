...
> **系统代理和 TUN 不是互斥的，两者可以同时生效。** 客户端界面上的“工作方式：代理模式 / 增强模式（TUN）”说的只是**捕获流量的主力手段**，不代表另一个开关被关掉——很多客户端在 TUN 模式下仍然会顺手把系统代理也写上（PandaFan 的“增强模式 + 设置系统代理：自动设置”就是这样）。这时机器上其实有两条入口：读公告栏的应用从 `127.0.0.1:7890` 进代理，不读的被 TUN 从路由层兜进去，两条路通向同一个内核、同一套规则引擎。所以判断机器实际开着哪几条路，别只看客户端界面，查系统侧的痕迹才算数：`scutil --proxy` 看公告栏，`netstat -nr` 看路由表。
{: .prompt-tip }
