...
概念讲完，拿一台真实同时开着公司 VPN 和代理软件（PandaFan）的 Mac 验证一遍。下面每条发现都标注了对应前文哪个结论。
...
curl dhr.didichuxing.com → 200 OK（昨天超时，今天通，差别只在连了 VPN）
```
...
本地监听端口    → 没有 7890/10080 等任何代理端口
TUN 路由       → 没有任何指向代理的路由
curl google    → 直连失败（000），没有代理在替它接力
```

PandaFan 进程确实在跑，但**系统代理没开、没有监听端口、没有 TUN 路由**——三处该有的痕迹一处都没有。也就是说此刻代理软件只是“开着”，并没有捕获任何流量（它的核心处于断开状态）。这印证了“同时开”一节的判断框架：**进程在跑不等于流量被接管**，判断代理是否生效，就去查三处——公告栏（`scutil --proxy`）、本地端口、路由表，至少一处要有痕迹。

此时如果在 PandaFan 里点下连接、打开系统代理，变化会精确落在前文预测的位置：公告栏被写上 `127.0.0.1:xxxx`，守规矩的应用经 `lo0` 找到本地代理；代理软件自己发出的连接再查路由表——目标命中 `10/8` 就进 `utun4` 走公司内网，命中默认路由就走 `en0` 出境。两层接力，各管一段。

```mermaid
flowchart TD
    A["应用发包"] --> B{"内核路由表"}
    B -->|"10/8 等 134 条公司路由"| C["utun4 → 公司 VPN 网关<br>分到的内网 IP：172.28.160.237"]
    B -->|"default 默认路由"| D["en0 → 小米路由器 192.168.31.1"]
    E["系统代理公告栏：空<br>（代理软件在跑但未捕获流量）"] -.-> A
```
