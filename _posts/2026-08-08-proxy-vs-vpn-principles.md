...
>
> `utun4`、`utun5` 里的数字也不代表固定角色，只反映当次创建顺序。VPN 或代理重连后编号可能互换，排障时应根据接口地址、路由目标和对应进程识别，不能把“`utun4` 永远是 Cisco”写进脚本或规则。
{: .prompt-info }
...
决策分两层按顺序发生：应用层先决定“要不要用代理”，网络层再决定“前一步最终产生的连接走哪张网卡”。这两层不会在路由表里直接抢同一个包，但第二层无法推翻第一层已经做出的代理选择。用前一节真机上的真实地址，看两条完整链路。
...
这套组合最稳妥的入口配置是：**代理启用 TUN，系统代理保持关闭**。这样 Chrome、命令行和不认识代理设置的应用都先产生面向真实目标的连接，路由表能够在最早的位置裁决；不会再有一部分流量先走系统代理、另一部分流量再走 TUN 的双入口差异。公司路由仍必须比代理级联路由更具体，企业 VPN 也必须是分离隧道；这两个条件缺一不可。
...
| 覆盖所有应用，同时使用分离隧道公司 VPN | TUN 代理，关闭系统代理 | 只保留一个捕获入口，让具体公司路由优先裁决 |
...
注意第一行：`curl` 是那种从不读系统代理的“不守规矩”应用，之前直连 Google 必然超时，现在直接通了——**TUN 在路由层接管，应用自不自觉都一样**。而公告栏保持空白并不是待修复的残留状态，而是最终方案的一部分：代理只保留 TUN 这一个入口，避免浏览器提前把公司域名交给代理；Cisco 再凭更具体的公司路由接走内网流量。
...
**Q：代理和 VPN 同时开，包去哪？**
系统代理 + VPN：应用层先决定用不用代理，网络层只能给代理决策后产生的连接选网卡；公司域名若被代理判成 `Proxy`，公司路由不会看到原始内网 IP。TUN + VPN：同一层由路由表裁决，路由更具体者赢。分离隧道 VPN 与 TUN 代理共存时，关闭系统代理、只保留 TUN 入口最容易预测，双方仍需各自排除服务器地址以防死循环。
