...
tags: [io, nio, aio]
description: "用 Future 和 CompletionHandler 理解 Java AIO，并对比 Reactor/NIO 与 Proactor/AIO 的读写分工。"
---
...
他们共同的接口`AsynchronousChannel`倒是没有提供什么read/write接口方法（**应该是子类的方法参数比较难统一，所以虽然行为类似，但没法定义一个统一的接口方法**，比如AsynchronousFileChannel是按文件位置read，AsynchronousSocketChannel是按socket流read，参数明显不一样），不过javadoc上写明了这一类异步channel一般怎么用。以read为例：
...
把“纸条”这件事画出来，就是Proactor味儿：

```mermaid
sequenceDiagram
    participant App as 当前线程
    participant AIO as AsynchronousChannel
    participant Kernel as OS / 异步线程池
    participant Buffer as ByteBuffer
    participant Handler as CompletionHandler

    App->>AIO: read(buffer, attachment, handler)
    AIO->>Kernel: 提交异步读
    AIO-->>App: 立即返回
    Kernel->>Buffer: 完成数据拷贝
    Kernel->>Handler: completed(size, attachment)
    Handler->>Buffer: flip()/decode()/业务处理
```

NIO的回调点是“可读了，快来自己读”；AIO的回调点是“我已经读完了，结果在buffer里”。差别就这一句，但程序结构差很多。
...
- Ref: [美团技术团队：Java NIO浅析](https://tech.meituan.com/2016/11/04/nio.html)
...
Ref:
- [Baeldung: Java NIO2 Asynchronous Socket Channel](https://www.baeldung.com/java-nio2-async-socket-channel)
