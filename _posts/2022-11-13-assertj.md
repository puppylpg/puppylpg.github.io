...
categories: [assertj]
tags: [assertj]
description: "从 assertThat 重载、断言对象继承体系和 fluent API，理解 AssertJ 为什么比裸 JUnit 断言顺手。"
---
...
```mermaid
flowchart TD
    Test["测试代码"] --> AssertThat["Assertions.assertThat(actual)"]
    AssertThat --> Overload["按 actual 类型选择重载"]
    Overload --> AssertObj["返回 XxxAssert 断言对象"]
    AssertObj --> Fluent["isNotNull().isEqualTo().startsWith()"]
    Fluent -->|不符合预期| Error["抛 AssertionError"]
    Fluent -->|符合预期| Self["返回 SELF 继续链式调用"]

    style AssertThat fill:#e3f2fd,stroke:#1976d2
    style AssertObj fill:#fff3bf,stroke:#f59f00
    style Error fill:#ffe3e3,stroke:#c62828
```

# 断言谁
[AssertJ overview](https://assertj.github.io/doc/#assertj-overview)
