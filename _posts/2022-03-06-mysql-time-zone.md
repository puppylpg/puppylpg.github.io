...
categories: [mysql]
tags: [mysql]
description: "用 DATETIME、TIMESTAMP、时区层级和 UNIX 时间戳的转换链路，复盘 MySQL 时间类型的坑。"
---
...
# 类型
MySQL 官方文档对[日期与时间类型](https://dev.mysql.com/doc/refman/8.0/en/datetime.html)有完整定义，这里只抓最容易踩坑的几类。
...
```mermaid
sequenceDiagram
    participant App as 应用/SQL 输入
    participant Session as MySQL session time_zone
    participant TS as TIMESTAMP 存储
    participant DT as DATETIME 存储

    App->>Session: 写入 '1970-01-01 08:00:01' 或 from_unixtime(1)
    Session->>TS: 转成 UTC 后存储
    Session->>DT: 按当前显示值直接存储
    TS-->>Session: 查询时按 session time_zone 转回显示值
    DT-->>Session: 查询时原样取出，不再按时区转换
```
...
## 变更时区
变更 MySQL 时区可以参考这个 [Stack Overflow 答案](https://stackoverflow.com/a/19069310/7676237)。
...
## 处理时间
还有其他很多处理时间的函数，功能丰富，可以查官方的[日期时间函数文档](https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html)，或者看更偏速查的[菜鸟教程 MySQL 函数列表](https://www.runoob.com/mysql/mysql-functions.html)。
