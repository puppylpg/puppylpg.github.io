...
tags: [java, executor]
description: "从 Executor#execute 的最小抽象讲到 ThreadPoolExecutor 的 worker、队列、扩容和拒绝策略。"
---
...
```mermaid
flowchart TD
    Submit["execute(Runnable)"] --> Core{"worker < corePoolSize?"}
    Core -->|是| CoreWorker["addWorker(command, true)<br/>firstTask 直接执行"]
    Core -->|否| Queue{"workQueue.offer(command)"}
    Queue -->|成功| Recheck["recheck runState / workerCount"]
    Queue -->|失败| Max{"worker < maximumPoolSize?"}
    Max -->|是| NonCore["addWorker(command, false)<br/>非 core worker"]
    Max -->|否| Reject["RejectedExecutionHandler"]
    Recheck --> Worker["Worker 从队列 take 任务并执行"]
    CoreWorker --> Worker
    NonCore --> Worker

    style Submit fill:#e3f2fd,stroke:#1976d2
    style Queue fill:#fff3bf,stroke:#f59f00
    style Reject fill:#ffe3e3,stroke:#c62828
