...
description: "从 CPython 的 GIL 讲起：Java 线程既能并发也能并行，默认启用 GIL 的单个 CPython 解释器只能并发；threading 和 asyncio 都走这条路，协程在调度上像单条 carrier 上的虚拟线程，await 是一次显式暂停。"
---
...
锁的范围也要放到它真正覆盖的那一层。它锁的是 Python 字节码，不是整个进程。线程去等网络、等磁盘、`sleep`，或进入会释放 GIL 的 C 代码时，锁会放开，别的线程就能继续执行 Python。NumPy 这类把重活放在 C 里做完再回来的库，以及 Python 3.13 开始提供、3.14 进入官方支持阶段的 [无 GIL 构建](https://peps.python.org/pep-0703/)，也可以绕开它。后者仍然不是现在的默认形态。

这里有两个常被混在一起的词，后面整篇文章都靠它们。

**并行**是同一时刻真有多段代码在不同核上执行。要把 8 个核拉满，靠的是并行。  
**并发**是一段时间里有很多任务都在推进：可以是你在等的时候我去干别的，也可以是几份计算按时间片交错执行。它不要求两个核在同一瞬间一起算。
...
只调用 `async def`，既不 `await`，也不通过 `create_task`、`gather` 等方式把它调度成任务，就没有人去驱动那份快照，结果也不会出现。所以 `async def` 本身不产生并发：协程对象还必须被等待或调度，执行到真正需要等待的地方时才会把线程让给别的任务。
...
Python 3.13 开始提供关闭 GIL 的构建；到 Python 3.14，[free-threaded CPython](https://docs.python.org/3/howto/free-threading-python.html) 已经进入官方支持阶段，但仍然是可选构建，不是默认下载和安装的那一个。普通 CPython 仍然带着 GIL；free-threaded 版本常用带 `t` 的解释器名区分，例如 `python3.14t`。

在这个版本里，熟悉的 `threading` 和 `ThreadPoolExecutor` 真能让纯 Python 计算同时跑在多个 CPU 核上。程序仍要自己把工作拆成多份、创建多条线程；解释器不会自动把一个普通 `for` 循环平摊到八个核。
...
另一条路不要求换成无 GIL 构建。Python 3.14 在标准库中加入了 [`InterpreterPoolExecutor`](https://docs.python.org/3/library/concurrent.futures.html#interpreterpoolexecutor)。它是 `ThreadPoolExecutor` 的子类，worker 在同一个进程的不同 OS 线程里运行；特别之处在于，每条 worker 线程拥有独立的 Python 解释器，每个解释器各有一把 GIL。几把锁互不阻塞，纯 Python 计算就能在多个核上同时执行。
