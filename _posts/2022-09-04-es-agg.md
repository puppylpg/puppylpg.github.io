...
- [pipeline agg](https://www.elastic.co/guide/en/elasticsearch/reference/master/search-aggregations-pipeline.html)：**操作的对象是其他聚合产出的结果，而非doc**，主要目的是对聚合结果进行后处理，所以它被称为pipeline agg，就像es的pipeline一样，进行的是数据“后操作”。分为两类：
    + parent：操作父聚合的输出，产生新的分桶或者metric，添加在现有bucket上；
...
# pipeline aggs - 后处理
pipeline agg操作的对象是其他聚合产出的结果，**所以有先后顺序之分**：其他agg先产生bucket，pipeline agg再进行后处理。
