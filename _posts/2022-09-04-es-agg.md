...
tags: [elasticsearch]
description: "Elasticsearch aggregation 笔记：bucket、metric、pipeline aggregation，以及分布式 terms 聚合误差。"
---
...
```mermaid
flowchart TD
    A[原始文档集合] --> B[bucket agg: 分桶]
    B --> C[metric agg: 对桶内文档求值]
    C --> D[pipeline agg: 对聚合结果后处理]
    B --> B1[子 bucket agg: 二级分桶]
    B1 --> C1[子 metric agg]
...
| 类型 | 输入 | 输出 | 类比 |
| --- | --- | --- | --- |
| bucket agg | 文档 | bucket | `group by` |
| metric agg | bucket 内文档 | 数值 | `sum/avg/count` |
| pipeline agg | 其他 agg 的输出 | 新数值或新 bucket | 对聚合结果做二次计算 |
...
```mermaid
flowchart TD
    S1[shard 1 取本地 top shard_size bucket] --> C[coordinating node 合并]
    S2[shard 2 取本地 top shard_size bucket] --> C
    S3[shard 3 取本地 top shard_size bucket] --> C
    C --> R[全局 top size bucket]
    M[某个全局热门 bucket<br/>可能在某个 shard 只是本地第 26 名] -.被漏掉部分计入误差.-> C

    style C fill:#fff3bf,stroke:#f59f00
    style M fill:#ffe3e3,stroke:#ff6b6b
```
...
```mermaid
flowchart LR
    A[date_histogram buckets] --> B[sum price per bucket]
    B --> C[parent pipeline: derivative<br/>结果写回每个 bucket]
    B --> D[sibling pipeline: avg_bucket<br/>结果和 buckets 同级]
