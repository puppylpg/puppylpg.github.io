...
发过halo infinite的kol的过去四个月的media按月聚合，统计每个月有多少media：
```
...
感觉可以每次请求都返回topX，然后取出client需要的那一页返回回去。虽然这样分页不方便，但至少返回的topN数据是对的。用bucket sort这种pipeline sort虽然能分页，但是它返回的数据很可能是错的，分页也没什么卵用。
