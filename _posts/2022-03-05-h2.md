...
categories: [h2]
tags: [h2]
description: "梳理 H2 的 JDBC URL、内存/文件存储、embedded/server 模式、MySQL 兼容和 Spring Boot 控制台。"
---
...
# URL
h2的 URL 细节可以查官方的[数据库 URL 文档](http://www.h2database.com/html/features.html#database_url)。
...
# 模式
H2 的连接方式可以分成官方文档里的[连接模式](https://www.h2database.com/html/features.html#connection_modes)：
...
```mermaid
quadrantChart
    title H2 的两条维度：数据放哪儿，谁来访问
    x-axis "内存" --> "文件"
    y-axis "embedded：同进程" --> "server：跨进程"
    quadrant-1 "server + file"
    quadrant-2 "server + memory"
    quadrant-3 "embedded + memory"
    quadrant-4 "embedded + file"
    "测试用例": [0.22, 0.22]
    "本地 demo": [0.72, 0.28]
    "远程调试": [0.28, 0.78]
    "小型持久服务": [0.76, 0.76]
...
### embeded + in memory
官方对[内存数据库](https://www.h2database.com/html/features.html#in_memory_databases)的说明里，最关键的是“名字”和“进程边界”。
...
如何开启 server 模式可以参考官方的[Using Server 文档](http://www.h2database.com/html/tutorial.html#using_server)。

Spring Boot 中开启 H2 TCP server 可以参考这个 [Stack Overflow 示例](https://stackoverflow.com/a/55847035/7676237)。
...
所以记得把spring security对`h2-console`这个path的校验关掉，具体写法可以参考这个 [Stack Overflow 答案](https://stackoverflow.com/a/59560136/7676237)。
...
参阅：[Baeldung 的 Spring Boot H2 指南](https://www.baeldung.com/spring-boot-h2-database)、[H2 内存库跨连接说明](https://stackoverflow.com/a/28950817/7676237)、[H2 console 连接内存库示例](https://stackoverflow.com/a/43862903/7676237)。
