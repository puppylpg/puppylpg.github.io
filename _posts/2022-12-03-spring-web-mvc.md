...
categories: [tech, spring, mvc, cors]
tags: [spring, mvc, dispatcher-servlet, cors]
description: "从 DispatcherServlet 初始化、WebApplicationContext 层级、请求处理流程到 CORS，整理 Spring Web MVC 的核心机制。"
---

Spring MVC 的全称是 [Spring Web MVC](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html)，来自于它的 module 名 `spring-webmvc`。
...
本文先看传统 Spring MVC 如何被 servlet 容器发现并初始化，再看 `DispatcherServlet` 依赖哪些 WebApplicationContext 和 MVC 组件，最后整理请求处理、异常处理、异步请求、CORS、View 和 client 支持。

传统 Spring MVC 的初始化核心，是 servlet 容器通过 SPI 找到 `WebApplicationInitializer`，再由它注册 `DispatcherServlet` 和 WebApplicationContext：

```mermaid
flowchart TD
    Tomcat["Tomcat 启动 Context"] --> SCI["SpringServletContainerInitializer<br/>ServletContainerInitializer"]
    SCI --> WAI["发现 WebApplicationInitializer"]
    WAI --> Root["创建 root WebApplicationContext<br/>service/repository 等"]
    WAI --> Servlet["注册 DispatcherServlet"]
    Servlet --> Child["创建 servlet WebApplicationContext<br/>controller/mvc config"]
    Child --> Components["HandlerMapping / HandlerAdapter<br/>ViewResolver / ExceptionResolver"]
    Components --> Ready["Spring MVC 可处理请求"]
    ...
    class Tomcat,SCI,WAI,Servlet servlet
    class Root,Child,Components spring
    class Ready ready
```
...
**SpringMVC的这个实现并没有直接初始化servlet container，而是委托给了`WebApplicationInitializer`**。所以开发者可以实现spring提供的`WebApplicationInitializer`以初始化`DispatcherServlet`。
...
Spring Boot 的 embedded container 可参考官方文档里的 [Embedded Servlet Container Support](https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#web.servlet.embedded-container)。
...
# 异常处理
异常处理可以参考 Spring Web MVC 文档中的 [`@ExceptionHandler`](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-ann-exceptionhandler)。

全局 controller advice 可以参考 [`@ControllerAdvice`](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-ann-controller-advice)。
...
# 异步request
异步 request 可以参考 Spring Web MVC 文档中的 [asynchronous requests](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-ann-async)。
...
更多背景可以参考 [MDN 的 CORS 文档](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)、[Cross-origin resource sharing](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing)、[MDN 的 OPTIONS 方法说明](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Methods/OPTIONS) 和阮一峰的 [CORS 详解](https://www.ruanyifeng.com/blog/2016/04/cors.html)。
...
# View
## groovy markup
Groovy markup 的示例见 Spring 文档里的 [Groovy Markup](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-view-groovymarkup-example)。

## pdf & excel
PDF 和 Excel ViewResolver 的支持见 Spring 文档里的 [PDF and Excel](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-view-document)。

# client
`RestTemplate`和`WebClient`
可以参考 Spring 文档里的 [webmvc client](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#webmvc-client)。

# websocket
WebSocket 支持见 Spring 文档里的 [WebSocket](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#websocket)。

# 感想
先从 `MockMvc` 观察 Spring MVC 的简化流程，再回头读 Spring Web MVC 文档，会更容易理解 `DispatcherServlet` 与 servlet 容器之间的边界。Spring MVC 的关键不是“替代 Tomcat”，而是把 Tomcat 交给它的请求收拢到一个前端控制器里，再分发给 Spring 世界里的 controller、interceptor、converter 和 view。
