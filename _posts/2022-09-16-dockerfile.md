...
description: "梳理 Dockerfile 常用指令、镜像层、ENTRYPOINT/CMD、基础镜像和 Spring Boot 分层镜像构建。"
---
...
Dockerfile 不是“脚本执行完得到一个目录”，而是**每个会改文件系统的步骤提交成一层 image layer**。后续构建能不能复用缓存，镜像大不大，基本都和这个事实有关。

```mermaid
flowchart TD
    A["FROM<br/>选择基础镜像"] --> B["WORKDIR / ENV / ARG<br/>设置构建上下文和元数据"]
    B --> C["COPY / ADD<br/>把文件放进构建容器"]
    C --> D["RUN<br/>执行命令并提交新 layer"]
    D --> E["EXPOSE / CMD / ENTRYPOINT<br/>记录运行时元数据"]
    E --> F["image<br/>只读 layer + config"]
    F --> G["docker run<br/>config 决定默认命令"]
```

| 指令 | 是否主要改变文件系统 | 会不会影响运行时 | 常见坑 |
|---|---|---|---|
| `FROM` | 是，决定基础层 | 是 | 基础镜像越大，后面再努力也有限 |
| `RUN` | 是，提交新 layer | 间接影响 | 多个 `RUN apt install` 会留下更多层和缓存垃圾 |
| `COPY` / `ADD` | 是 | 间接影响 | `ADD` 会自动解本地 tar，容易不自知 |
| `ENV` | 否，写入 image config | 是 | 运行时还存在，可被 `docker run -e` 覆盖 |
| `ARG` | 否，构建期变量 | 否 | final image 里没有它 |
| `CMD` / `ENTRYPOINT` | 否，写入 image config | 是 | exec form 不展开 shell 变量 |
...
## ENV
设置全局环境变量，k=v格式：
参考 [`ENV` 文档](https://docs.docker.com/engine/reference/builder/#env)。
...
参考 [`EXPOSE` 文档](https://docs.docker.com/engine/reference/builder/#expose)。
...
参考 [Stack Overflow 对 ADD/COPY 差异的解释](https://stackoverflow.com/a/24958548/7676237)。

官方建议，在用不到ADD的额外功能的时候，使用COPY：
参考 [Dockerfile best practices 里的 ADD or COPY](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/#add-or-copy)。
...
参考 [`docker run` 文档](https://docs.docker.com/engine/reference/commandline/run/)。

| Dockerfile 设置 | `docker run image` 实际执行 | `docker run image echo hi` 会怎样 |
|---|---|---|
| 只有 `CMD ["bash"]` | `bash` | `CMD` 被替换为 `echo hi` |
| `ENTRYPOINT ["java"]` + `CMD ["-jar", "app.jar"]` | `java -jar app.jar` | `java echo hi`，只替换 `CMD` |
| shell form `ENTRYPOINT java $JVM_ARGS ...` | `/bin/sh -c "java $JVM_ARGS ..."` | 后面的 command 基本没机会参与 |
| `--entrypoint /bin/zsh` | 覆盖镜像里的 entrypoint | 新 entrypoint + command/args |
...
参考 [Stack Overflow 对覆盖 ENTRYPOINT 的回答](https://stackoverflow.com/a/21564990/7676237)。
...
官方也有类似的 [ENTRYPOINT 默认命令示例](https://docs.docker.com/engine/reference/run/#entrypoint-default-command-to-execute-at-runtime)。
...
参考 [Unix Stack Exchange 对 `sh -c` 参数的解释](https://unix.stackexchange.com/a/152396/283488)。
...
参考 [`ENTRYPOINT` 文档](https://docs.docker.com/engine/reference/builder/#entrypoint)。
...
参考 [CMD 和 ENTRYPOINT 交互规则](https://docs.docker.com/engine/reference/builder/#understand-how-cmd-and-entrypoint-interact)。
...
```
但是不知为何脚本没法执行，所以索性用 `curl` 下载 [oh-my-zsh install.sh](https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh) 到本地，再添加到镜像里执行。
...
参考 [Debian Locale 文档](https://wiki.debian.org/Locale)。
...
参考 [pure 主题 issue](https://github.com/sindresorhus/pure/issues/300#issuecomment-386371460)、[oh-my-zsh issue](https://github.com/ohmyzsh/ohmyzsh/issues/7426#issuecomment-632832807) 和 [Unix Stack Exchange 对 locale 的解释](https://unix.stackexchange.com/a/669735/283488)。
...
```mermaid
flowchart TD
    J["Spring Boot executable jar"] --> E["layertools extract"]
    E --> D1["dependencies<br/>稳定第三方依赖"]
    E --> D2["snapshot-dependencies<br/>变化较频繁"]
    E --> D3["spring-boot-loader<br/>启动器"]
    E --> D4["application<br/>业务代码和资源"]
    D1 --> L1["Docker layer 1"]
    D2 --> L2["Docker layer 2"]
    D3 --> L3["Docker layer 3"]
    D4 --> L4["Docker layer 4<br/>最常变"]
```
...
参考：
参考 [Baeldung 的 Spring Boot Docker images 文章](https://www.baeldung.com/spring-boot-docker-images)。
