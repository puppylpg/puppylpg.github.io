...
动态代理就是由jvm自动生成一个代理，而不是开发者写一个类似上面的静态代理。此时，开发者只需要指定代理的行为即可，剩下的全都由Java搞定。

Java创建动态代理的流程：
1. 开发者指定一个接口；
2. jvm根据指定的接口，自己动态生成一个类，实现该接口；
2. 接口里每一个方法都是这么实现的：每一个对该方法的调用，都使用开发者指定的一个handler去完成；
3. 该handler的返回值，将作为动态代理对象方法调用后的返回值。

**所以理解Java动态代理的关键，就是区分好Java会做什么，开发者要做什么**。开发者做好自己的事情就好了，其他的都会由Java按照契约搞定。

所以开发者实际只需要操心两件事：
1. 为哪个接口创建动态代理；
2. 动态代理调用handler时要做什么；
...
2. **且该handler的返回值，将作为动态代理方法调用后的返回值**。
...
```
实现一个InvocationHandler，实际就是实现他的invoke接口。Java会在动态类的方法被调用时，调用该接口的invoke方法，并传进来三个参数：
- proxy：Java生成的动态代理类产生的对象；
- method、args：方法名和方法参数，两部分结合起来就可以决定类里的一个方法；
...
## JDK vs. CGLib
Java的动态代理**只能为接口生成代理，不能为具体类生成代理**。而CGLib则可以为具体的类生成动态代理，它的思路大致是：
- 动态生成一个类的子类；
...
另外CGLib创建动态代理比jdk慢，但是创建出的代理运行效率比jdk创建的动态代理高，所以如果要代理singleton，最好用CGLib。不过jdk动态代理的另一个好处是不需要引入额外依赖，有jdk就够了，CGLib技术则显然需要引入第三方CGLib依赖。
