...
description: "从 Class 对象、反射 API 到静态代理和 JDK 动态代理，理解运行时类型信息如何被利用。"
---
...
```mermaid
flowchart TD
    ClassFile[".class 字节码"] --> ClassLoader["ClassLoader 加载"]
    ClassLoader --> ClassObject["Class<T> 对象<br/>字段/方法/构造器/注解"]
    Instance["普通对象"] -->|getClass| ClassObject
    ClassObject --> Reflection["反射 API<br/>Field / Method / Constructor"]
    Reflection --> DynamicProxy["动态代理<br/>Proxy + InvocationHandler"]
    DynamicProxy --> Target["目标对象方法调用"]

    style ClassObject fill:#e3f2fd,stroke:#1976d2
    style Reflection fill:#fff3bf,stroke:#f59f00
    style DynamicProxy fill:#e8f5e9,stroke:#2e7d32
```
