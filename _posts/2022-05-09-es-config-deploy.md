...
tags: [elasticsearch]
description: "Elasticsearch 集群配置与部署笔记：节点发现、角色、配置优先级、数据路径、高可用和节点迁移。"
---
...
dynamic无疑使非常便利的，但是可能考虑到一些安全问题，并不是所有的配置都是dynamic。比如[`reindex.remote.whitelist`](https://www.elastic.co/guide/en/elasticsearch/reference/7.17/docs-reindex.html#reindex-from-remote)，因为安全问题所以[故意不搞成dynamic](https://github.com/elastic/elasticsearch/issues/29153)。
...
```mermaid
flowchart TD
    A[Transient setting<br/>运行时临时配置] --> B[Persistent setting<br/>运行时持久配置]
    B --> C[elasticsearch.yml]
    C --> D[Default setting]
    A -.覆盖.-> C
    B -.覆盖.-> C

    style A fill:#fff3bf,stroke:#f59f00
    style B fill:#e8f5e9,stroke:#51cf66
```
...
| 角色 | 主要工作 | 常见注意点 |
| --- | --- | --- |
| `master` | 维护 cluster state、索引创建删除、分片分配 | HA 至少 3 个 master-eligible 节点 |
| `data` | 存 shard，执行读写 | 磁盘、page cache、IO 最关键 |
| `ingest` | 跑 pipeline processor | pipeline 重时可以单独拆 ingest 节点 |
| coordinating-only | scatter/gather 客户端请求 | role 为空，但并不轻松，聚合和深分页会吃内存 |
...
```mermaid
stateDiagram-v2
    [*] --> DisableAllocation: cluster.routing.allocation.enable=none
    DisableAllocation --> StopOldNode: 停旧节点
    StopOldNode --> Yellow: replica 转 primary\n缺副本所以 yellow
    Yellow --> StartNewNode: 启新节点
    StartNewNode --> EnableAllocation: allocation 设回 all/null
    EnableAllocation --> Green: 恢复 replica\n同步 translog
    Green --> [*]
```
