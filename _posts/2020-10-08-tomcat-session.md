...
description: "从 HttpSession、StandardSession、Manager 和 JSESSIONID 往返，理解 Tomcat 如何给无状态 HTTP 加状态。"
---
...
sequenceDiagram
    ...
    participant Connector as Connector/Request
    participant Manager as Manager
    participant Servlet as Servlet
    participant Response as Response

    Browser->>Connector: 首次请求，没有 JSESSIONID
    Connector->>Manager: createSession()
    Manager-->>Connector: StandardSession(id)
    Connector->>Servlet: request.getSession()
    Servlet->>Servlet: session.setAttribute(...)
    Response-->>Browser: Set-Cookie: JSESSIONID=id
    Browser->>Connector: 下次请求带 Cookie: JSESSIONID=id
    Connector->>Manager: findSession(id)
    Manager-->>Servlet: 同一个 HttpSession view
```
