...
description: "Elasticsearch 字段存储笔记：_source、store、doc_values、fields、stored_fields、docvalue_fields 和 runtime field。"
---
...
```mermaid
flowchart TD
    A[原始 JSON 文档] --> B[_source<br/>整条原文 stored field]
    A --> C[index<br/>倒排索引 token -> doc]
    A --> D[doc_values<br/>列式正排 doc -> value]
    A --> E[store=true<br/>单独 stored field]
    C --> F[搜索 / 过滤]
    D --> G[排序 / 聚合 / docvalue_fields / script doc[]]
    B --> H[_source filtering / fields 取原文再按 mapping 格式化]
    E --> I[stored_fields]

    style C fill:#e3f2fd,stroke:#4dabf7
    style D fill:#e8f5e9,stroke:#51cf66
...
### Lucene的`doc_values`
Lucene的`doc_values`是把倒排索引（key to value）倒过来（value to key），英文叫 **uninvert the inverted index**，“把倒排索引倒过来”，中文称之为正排索引。
...
| 查询参数 | 数据来源 | 返回特点 | 适合场景 |
| --- | --- | --- | --- |
| `_source` | `_source` 原文 | 原始写入值 | 返回整条或较多字段 |
| `fields` | 通常从 `_source` 取，再按 mapping 格式化 | 能返回 runtime field | 想要 mapping 语义下的值 |
| `stored_fields` | `store=true` 的字段 | 只查显式 stored 字段 | 极少数热字段、超大 `_source` |
| `docvalue_fields` | `doc_values` | 列式读取，轻量 | 返回少数可 doc_values 的字段 |
...
![elastic-benachmarks](/pics/elasticsearch/storage/elastic-benchmarks.png)
