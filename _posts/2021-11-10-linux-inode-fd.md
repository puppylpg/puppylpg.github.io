...
tags: [linux, inode, fd]
description: "梳理 Linux 文件名、目录项、inode、打开文件结构和文件描述符之间的关系。"
---
...
> - [ext4 inode 分配的一篇说明](https://chenjiehua.me/linux/linux-ext4fs-disk-inode.html)
...
```mermaid
flowchart LR
    Name["文件名<br/>world.txt"] --> Dentry["目录项<br/>name -> inode number"]
    Dentry --> Inode["inode<br/>权限/owner/mtime/block位置"]
    Inode --> Blocks["磁盘数据块"]

    Proc["进程"] --> FDTable["fd table"]
    FDTable -->|"fd=3"| OpenFile["open file<br/>offset/flags"]
    OpenFile --> Inode
```

这么拆开之后就顺了：**文件名是给人看的入口，inode是磁盘上的文件身份，fd是进程手里的运行时句柄**。同一个inode可以有多个文件名，也可以被多个进程打开出多个`file`结构，各自维护自己的offset。名字、身份、手柄，不是一回事。
