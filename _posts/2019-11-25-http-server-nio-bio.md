...
categories: [nio]
tags: [nio]
description: "用老师答疑的类比解释 HTTP Server 的 BIO、多线程和 Reactor NIO 线程模型，以及连接、线程和事件之间的关系。"
---

如果想写个 web 服务，处理比如 HTTP 请求，首先要决定自己的 server 选用什么线程模型。不同的线程模型对系统吞吐和连接承载能力有极大的影响。最基本的方向有两类：基于线程（thread-based）的模型，事件驱动（event-driven）的模型。
...
sequenceDiagram
    ...
    C1->>Main: connect
    Main->>C1: accept
    C1->>Main: request / 等待 I/O
    Note over Main: 线程被 C1 占着
    C2-->>Main: connect / 排队等
    Main->>C1: response
    C1->>Main: close
    Main->>C2: accept
...
单线程 BIO 的本质就是：**一个线程从 accept 到 read/write 再到 close，全程陪一个连接耗着**。连接一多，后面的请求只能在门口干等。
...
sequenceDiagram
    ...
    C1->>Main: connect
    Main->>W1: 分配连接
    W1->>C1: read / write / 等待下一次 request
    C2->>Main: connect
    Main->>W2: 分配连接
    W2->>C2: read / write / 等待下一次 request
    Note over W1,W2: 连接不活跃时，线程也跟着闲置
```
...
sequenceDiagram
    ...
    C1->>Selector: register OP_READ
    C2->>Selector: register OP_READ
    C1-->>Selector: socket readable
    Selector->>Worker: dispatch read event
    Worker->>C1: read / handle / write
    Worker-->>Selector: 处理完归还线程
    C2-->>Selector: socket readable
    Selector->>Worker: dispatch read event
    Worker->>C2: read / handle / write
```

Reactor 的关键不是“没有线程”，而是**线程不再和连接绑定**。线程只在事件就绪、确实有活儿要干的时候出现；没活儿的时候，连接挂在 selector 上等通知，线程回池里处理别的连接。
...
## Proactor Pattern
TBD：可以先看 [Proactor 论文](https://www.dre.vanderbilt.edu/~schmidt/PDF/Proactor.pdf)。
