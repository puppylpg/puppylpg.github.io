...
description: "用同一个计数器压测 synchronized、ReentrantLock、ReadWriteLock、StampedLock、AtomicLong 和 LongAdder，观察不同同步策略的性能取舍。"
---
...
# 场景
设置n个读线程，m个写线程，争用一个共享变量。读线程只是读一下，并记录一次读取；写线程将共享变量+1，并记录一次写入。当共享变量达到target大小时，线程全部退出，输出总读取次数，总写入次数，以及整个过程的总时间。
...
flowchart LR
    ...
    style Readers fill:#e3f2fd,stroke:#4dabf7
    style Writers fill:#fff3bf,stroke:#f59f00
    style Counter fill:#e8f5e9,stroke:#51cf66
    style Stop fill:#ffe3e3,stroke:#ff6b6b
```

这不是严谨的 JMH benchmark，更像一个“同场竞技”的直觉实验：同样的读写压力，换掉`Counter`内部同步策略，看结果大概怎么漂。
...
## Lock
显式锁。和内置锁在加锁语义上没什么区别。详见[锁]({% post_url 2019-12-10-lock %})。
...
- ReadWriteLock和Lock比起来，则是划分出专门的Read lock，使得读与读不再互斥访问；
...
## Atomic
CAS实现，和锁比起来，是一种乐观行为，类似于StampedLock里的乐观读，但是无论读写都是乐观的。是一种利用底层硬件的非阻塞乐观同步行为。详见[CAS vs. 锁]({% post_url 2019-12-09-cas-vs-lock %})。
```java
package example.concurrency.synchronization.lock.counters.counter;
...
> 详情参考：[How LongAdder performs better than AtomicLong](https://stackoverflow.com/questions/30691083/how-longadder-performs-better-than-atomiclong)
...
以上参阅：
- [The Java Specialists' Newsletter：StampedLock](https://www.javaspecialists.eu/archive/Issue215.html)
...
# 总结
参阅：
- [Java 8 StampedLocks vs. ReadWriteLocks and synchronized](https://blog.overops.com/java-8-stampedlocks-vs-readwritelocks-and-synchronized/)
