...
sequenceDiagram
    ...
    DS->>HI: preHandle()
    DS->>HA: handle()
    HA->>Ctrl: invoke handler method
    alt 正常返回 ModelAndView / Body
        Ctrl-->>HA: return value
        HA-->>DS: ModelAndView or response body
        DS->>HI: postHandle()
        DS->>VR: resolve view / write body
    else 抛出异常
        Ctrl-->>HA: exception
        HA-->>DS: exception
...
public class SpitterWebInitializer extends AbstractAnnotationConfigDispatcherServletInitializer {
    ...
    // getRootConfigClasses / getServletConfigClasses 告诉框架去哪装 bean
}
```
...
用户从此不用写 Servlet。`main` 还是 Tomcat 的；`lookup` 找到的变成框架的 `DispatcherServlet`，下一层才轮到 `@Controller`。

# SpringMVC如何做的
## 世界分为两步
原本servlet协议就把servlet容器和开发者做的工作拆分成了两步。有了DispatcherServlet之后，这两步依然没变：
1. tomcat调用DispatcherServlet；
2. 开发者完全活在DispatcherServlet的世界；
...
现在知道它为什么叫DispatcherServlet了——所有的请求都交给它处理，它再把请求dispatch出去！分发给谁？分发给开发者熟悉的@Controller。
...
4. SpringMVC由Tomcat调用，构造了DispatcherServlet：开发者只要在DispatcherServlet里写Controller就行了；
