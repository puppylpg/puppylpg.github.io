...
description: "Elasticsearch 分片读写机制：协调节点、query then fetch、primary-replica 写入、segment、refresh、flush 和 translog。"
---
...
这里不要把 queue 理解成“整个集群把请求串成单线程”。它更像各类请求进入对应线程池前的等待区：搜索、写入、bulk、enrich 等各自有容量和拒绝策略。队列能缓冲突刺流量，但满了之后照样拒绝，不能靠它兜底无限并发。
...
es的查询是按照得分给结果排序的。如果返回top10，有两个 primary shard，那每个分片都要返回10个文档，最后由协调节点把这20个文档统一排序，返回top10。曾经，es的搜索方式`search_type`有两大类可选：
...
sequenceDiagram
    ...
    N->>S1: query phase
    N->>S2: query phase
    S1-->>N: top10 doc id + score
    S2-->>N: top10 doc id + score
    N->>N: merge sort 取全局 top10
    N->>S1: fetch phase 取 _source
    N->>S2: fetch phase 取 _source
    N-->>C: 返回最终 top10
```
...
# 分布式写入 - 写请求必须给到 primary shard
[协调节点收到写请求后](https://www.elastic.co/guide/cn/elasticsearch/guide/current/distrib-write.html)：
1. 按照`_routing`确定请求应该写的分片；
2. **把请求发给拥有该分片 primary shard 的节点**；
3. primary shard写成功后，要不要返回写成功给协调节点，需要看情况：
    - **按照默认配置，只要 primary shard 可用，就会执行写入，并告诉协调节点写成功了**；
    - 如果想稳妥一些，~~可以设置replica shard写成功才返回，相应的写请求就变慢了：**primary shard写成功后，把请求转发给replica shard，replica写成功，才返回给协调节点“写成功”，协调节点再返回200给客户端**~~；
...
sequenceDiagram
    ...
    C->>N: index / update / delete
    N->>N: hash(_routing) 定位分片
    N->>P: 转发写请求
    P->>P: 写 in-memory buffer + translog
    P->>R: replication request
    R-->>P: replica 写入结果
    P-->>N: _shards 成功/失败统计
    N-->>C: response
```
...
```mermaid
stateDiagram-v2
    [*] --> InMemoryBuffer: index doc
    InMemoryBuffer --> SearchableSegment: refresh\n写入 page cache 并打开 reader
    SearchableSegment --> CommittedSegment: flush\n生成 commit point
    SearchableSegment --> MergedSegment: merge\n小段合并为大段
    CommittedSegment --> MergedSegment
    MergedSegment --> [*]
```
