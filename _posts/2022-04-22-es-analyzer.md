...
tags: [elasticsearch]
description: "Elasticsearch analyzer 机制笔记：char filter、tokenizer、token filter、n-gram 和 search analyzer。"
---
...
```mermaid
flowchart LR
    A[原始文本<br/>THE Quick FoX] --> B[char_filter<br/>字符级清洗]
    B --> C[tokenizer<br/>拆成 token]
    C --> D[token filter<br/>小写 / 词干 / 同义词 / n-gram]
    D --> E[倒排索引<br/>token -> doc]
...
| 组件 | 输入 | 输出 | 典型用途 |
| --- | --- | --- | --- |
| `char_filter` | 原始字符串 | 处理后的字符串 | 去 HTML、替换字符 |
| `tokenizer` | 字符串 | token 流 | 按词、路径、URL 等切分 |
| `filter` | token 流 | 新 token 流 | 小写、词干、同义词、n-gram |
...
flowchart LR
    ...
    style T fill:#e3f2fd,stroke:#4dabf7
...
```mermaid
flowchart TD
    A[索引词 pikachu] --> B[index analyzer: edge n-gram]
    B --> C[pi / pik / pika]
    D[搜索词 pipi] --> E[search analyzer: keyword + lowercase]
    E --> F[pipi]
    C --> G[匹配倒排索引]
    F --> G
    G --> H{是否命中?}
    H -->|不会因为 pi 前缀误命中| I[更符合 autocomplete 预期]
