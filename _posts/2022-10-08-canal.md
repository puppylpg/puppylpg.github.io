...
tags: [canal, mysql]
description: "记录 Canal 容器化部署中的 admin、server、instance、cluster、Zookeeper、Kafka 和 binlog 配置坑。"
---
...
Canal 的文档不好读，一个原因是概念名一堆，还互相套。先把干活链路放在脑子里：

```mermaid
flowchart LR
    MySQL["MySQL<br/>binlog"] --> Instance["canal instance<br/>解析指定库表"]
    Instance --> Server["canal server<br/>承载 instance"]
    Server --> Kafka["Kafka topic<br/>flatMessage"]
    Admin["canal-admin<br/>配置和启停"] -. "下发配置" .-> Server
    Admin -. "创建/分配" .-> Instance
    ZK["Zookeeper<br/>cluster HA / running 标记"] -. "选主/注册" .-> Server
    TSDB["TSDB<br/>保存表结构快照"] -. "辅助解析历史 binlog" .-> Instance
```

| 概念 | 负责什么 | 这篇踩坑重点 |
|---|---|---|
| `admin` | Web UI 和配置中心 | 配置在 admin/mysql 里，不一定在本地文件里 |
| `server` | 真正跑 canal 的进程 | 容器启动时只需要知道 admin 地址和 cluster |
| `instance` | 一个具体同步任务 | 配 MySQL、表过滤、Kafka topic |
| `cluster` | 多 server 的 HA 组 | 依赖 Zookeeper 决定哪个 server 跑 instance |
| `TSDB` | 表结构历史 | 解析历史 binlog 时需要对应时刻的 schema |
...
| 优先级 | 配置来源 | 说明 |
|---|---|---|
| 1 | 环境变量 | 容器化最容易改，也最容易悄悄覆盖别处 |
| 2 | admin 上存储的配置 | cluster/server/instance 的主要配置来源 |
| 3 | server、instance 本地配置 | 最底层；instance 自己的配置又比 server 里的 instance 默认配置更近 |
...
参考 [Canal Admin QuickStart](https://github.com/alibaba/canal/wiki/Canal-Admin-QuickStart)。
...
这些端口可以参考 [Canal AdminGuide](https://github.com/alibaba/canal/wiki/AdminGuide)。
...
# Ref
- [Canal 容器化参考一](https://juejin.cn/post/6872218169978257422)
- [Canal 容器化参考二](https://its201.com/article/XUEJIA2S/104432830)
- [Canal 容器化参考三](https://blog.csdn.net/daziyuanazhen/article/details/106098887)
- [Canal 容器化参考四](https://www.cnblogs.com/throwable/p/13449920.html)

# canal的一些特性
## binlog消息合并

canal会把临近的多条binlog消息合并为一条：
参考 [Canal issue 3216](https://github.com/alibaba/canal/issues/3216)。
...
那究竟为什么搭这么痛苦？因为sb canal的文档实在是太不当人了！压根儿就没好好写！而且canal中间经过大的迭代，引入canal-admin，但是迭代的文档并没有好好修正、重新整理，所以很多地方新老文档都在，让人很费解。
