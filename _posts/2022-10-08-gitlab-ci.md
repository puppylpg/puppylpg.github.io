...
tags: [gitlab, ci]
description: "梳理 GitLab CI 的 runner、pipeline、stage、job、rules、artifacts、services 和父子流水线配置。"
---
...
GitLab CI 的配置很多，但主线很简单：**GitLab 创建 pipeline，runner 领 job，在 job image 里执行 script；job 之间通过 stage 顺序、artifacts、dependencies/needs 传递状态**。

```mermaid
flowchart TD
    G["Git push / MR / schedule / API"] --> P["pipeline"]
    P --> W["workflow rules<br/>先决定整条 pipeline 要不要跑"]
    W --> S1["stage: test"]
    W --> S2["stage: package"]
    W --> S3["stage: build-push-image"]
    W --> S4["stage: deploy"]
    S1 --> J1["unit_test job<br/>maven image"]
    S2 --> J2["package job<br/>产出 target/"]
    J2 --> A["artifacts<br/>target/ + build.env"]
    A --> J3["build-image job<br/>docker cli image"]
    J3 --> D["docker:dind service<br/>dockerd"]
    J3 --> R["Harbor / registry"]
    R --> J4["deploy job<br/>更新 Rancher/K8s"]
```

| 关键字 | 作用 | 容易误解的点 |
|---|---|---|
| `image` | job 主容器，用来执行 `script` | 想用 `mvn`/`docker`，必须这个镜像里有对应命令 |
| `services` | 旁路服务容器，如 MySQL、Redis、`docker:dind` | 不能给 job 主容器“安装命令” |
| `artifacts` | 保存 job 产物，传给后续 job | 默认后续 job 会下载前面所有 artifacts，可用 `dependencies` 收窄 |
| `rules` / `only` / `except` | 控制 job 是否创建 | 新分支上的 `rules:changes` 可能全部为 true |
| `workflow` | 控制整条 pipeline 是否创建 | 比 job rules 更早生效 |
...
参考 [GitLab CI/CD 文档](https://docs.gitlab.com/ee/ci/)。

一个示例：
参考 [GitLab CI/CD quick start](https://docs.gitlab.com/ee/ci/quick_start/)。

# runner
想执行CI/CD，得有runner。runner可以在实体机上部署，也可以部署为docker container。可以共享，也可以专用：
参考 [GitLab Runner 文档](https://docs.gitlab.com/runner/)。

runner是跑CI/CD的基础，但并不属于程序员关心的范围，由op搭好就行了。程序员更需要关心的是CI/CD的执行文件怎么写。

# `.gitlab-ci.yml`
参考 [创建 `.gitlab-ci.yml` 的文档](https://docs.gitlab.com/ee/ci/quick_start/#create-a-gitlab-ciyml-file)。
...
参考 [`stages` 文档](https://docs.gitlab.com/ee/ci/yaml/index.html#stages)。

定义job的时候，需要使用stage关键字标明它在哪个stage执行：
参考 [`stage` 文档](https://docs.gitlab.com/ee/ci/yaml/index.html#stage)。
...
## job
job代表了要执行的任务。
参考 [GitLab CI jobs 文档](https://docs.gitlab.com/ee/ci/jobs/)。
...
job一般都会设置执行条件：
- `rules`：复杂条件
    + 参考 [`rules` 文档](https://docs.gitlab.com/ee/ci/yaml/index.html#rules)
- `only`/`except`：简单条件
    + 参考 [`only`/`except` 文档](https://docs.gitlab.com/ee/ci/yaml/index.html#only--except)
...
参考 [GitLab pipeline architectures 文档](https://docs.gitlab.com/ee/ci/pipelines/pipeline_architectures.html)。

### basic pipeline
参考 [basic pipelines 文档](https://docs.gitlab.com/ee/ci/pipelines/pipeline_architectures.html#basic-pipelines)。

最简单直白的pipeline，一条线走下去。大多数pipeline都是这样的。

### parent-child
参考 [child-parent pipelines 文档](https://docs.gitlab.com/ee/ci/pipelines/pipeline_architectures.html#child--parent-pipelines) 和 [downstream pipelines 文档](https://docs.gitlab.com/ee/ci/pipelines/downstream_pipelines.html#parent-child-pipelines)。
...
参考 [merge request pipelines 文档](https://docs.gitlab.com/ee/ci/pipelines/merge_request_pipelines.html)。

**每次commit就执行的pipeline是branch pipeline，只有在创建了merge request且commit的情况下才执行的pipeline，是merge request pipeline**。

### avoid duplicate pipelines
参考 [avoid duplicate pipelines 文档](https://docs.gitlab.com/ee/ci/jobs/job_control.html#avoid-duplicate-pipelines)。
...
# variable
参考 [GitLab predefined variables 文档](https://docs.gitlab.com/ee/ci/variables/predefined_variables.html)。
...
参考 [GitLab CI/CD YAML syntax reference](https://docs.gitlab.com/ee/ci/yaml/)。

# 一个父子pipeline示例
一个样例：maven多模块项目，当子模块的文件内容发生变动时，父pipeline触发子pipeline，给子模块打包docker镜像、上传镜像到hub、部署镜像到rancher。
...
## image
参考 [`image` 文档](https://docs.gitlab.com/ee/ci/yaml/#image)。
...
参考 [Stack Overflow 上关于新分支 `rules:changes` 的回答](https://stackoverflow.com/a/73569727/7676237) 和 [GitLab issue 293645](https://gitlab.com/gitlab-org/gitlab/-/issues/293645)。
...
参考 [`rules:changes:compare_to` 文档](https://docs.gitlab.com/ee/ci/yaml/#ruleschangescompare_to)。
