...
categories: [http]
tags: [http]
description: "从请求行、请求头、请求体到状态行、响应头、响应体，拆开 HTTP 这坨 plain text。"
---
...
```mermaid
flowchart LR
    Client["Client / Browser"] --> Request["HTTP Request<br/>请求行 + 请求头 + 请求体"]
    Request --> Server["Server"]
    Server --> Response["HTTP Response<br/>状态行 + 响应头 + 响应体"]
    Response --> Client

    Request --> ReqLine["请求行<br/>method + uri + version"]
    Request --> ReqHeader["请求头<br/>Host / Accept / User-Agent"]
    Request --> ReqBody["请求体<br/>表单、JSON、文件等"]
    Response --> Status["状态行<br/>version + status code"]
    Response --> ResHeader["响应头<br/>Content-Type / Length / Encoding"]
    Response --> ResBody["响应体<br/>HTML / JSON / 图片等"]

    style Request fill:#e3f2fd,stroke:#1976d2
    style Response fill:#e8f5e9,stroke:#2e7d32
    style Server fill:#fff3bf,stroke:#f59f00
```
