...
categories: [docker]
tags: [docker]
description: "解释 Docker 网络背后的 netfilter、tun/tap、veth 和 Linux bridge 等虚拟网络设备。"
---
...
Docker 单机网络不是靠一个神秘黑盒完成的，而是几种 Linux 网络能力拼起来的：

| 组件 | 像什么 | Docker 里主要做什么 |
|---|---|---|
| netfilter/iptables | 内核网络协议栈的插件点和配置工具 | 决定哪些包能转发、哪些包要 NAT/DNAT |
| veth pair | 一根虚拟网线，两端各一张网卡 | 一端在容器 netns，一端接到宿主机 bridge |
| Linux bridge | 虚拟交换机 | 把同一 Docker network 的容器接到同一个二层网络 |
| tun/tap | 用户态程序接入协议栈的虚拟网卡 | VPN、隧道等场景常用，帮助理解虚拟网卡模型 |
...
flowchart LR
    subgraph Host["宿主机"]
        ...
        V1["veth-host-a"]
        V2["veth-host-b"]
        IPT["iptables<br/>FORWARD/NAT/DNAT"]
        ETH["eth0<br/>物理网卡"]
    end
...
![Netfilter_hook](/pics/docker/network/netfilter-hook.png)
...
sequenceDiagram
    ...
    participant Kernel as Linux 协议栈
    participant Tun as tun/tap 虚拟网卡
    participant VPN as 用户态 VPN 程序
    App->>Kernel: 写 socket，目标 A
    Kernel->>Tun: 路由到虚拟网卡
    Tun->>VPN: 把 IP 包交给用户态
    VPN->>Kernel: 加密/封装后再发往目标 B
```
...
![linux_bridge_container](/pics/docker/network/linux-bridge-container.png)
