...
description: "把页内目录的思想推广到页间，解释 InnoDB 为什么用 B+ 树、聚簇索引、二级索引和回表。"
---
...
```mermaid
flowchart TD
    Root["根页<br/>目录项：主键边界 + 页号"] --> InternalA["中间页 A<br/>目录项"]
    Root --> InternalB["中间页 B<br/>目录项"]
    InternalA --> Leaf1["叶子页 1<br/>完整行记录"]
    InternalA --> Leaf2["叶子页 2<br/>完整行记录"]
    InternalB --> Leaf3["叶子页 3<br/>完整行记录"]
    InternalB --> Leaf4["叶子页 4<br/>完整行记录"]
    Leaf1 <--> Leaf2
    Leaf2 <--> Leaf3
    Leaf3 <--> Leaf4

    Secondary["二级索引叶子页<br/>索引列 + 主键"] -. "回表：再按主键查聚簇索引" .-> Root

    style Root fill:#fff3bf,stroke:#f59f00
    style InternalA fill:#e3f2fd,stroke:#1976d2
    style InternalB fill:#e3f2fd,stroke:#1976d2
    style Leaf1 fill:#e8f5e9,stroke:#2e7d32
    style Leaf2 fill:#e8f5e9,stroke:#2e7d32
    style Leaf3 fill:#e8f5e9,stroke:#2e7d32
    style Leaf4 fill:#e8f5e9,stroke:#2e7d32
    style Secondary fill:#ffe3e3,stroke:#c62828
```
...
这个对比也可以参考 Stack Overflow 上关于[B+树和跳表取舍的讨论](https://stackoverflow.com/a/23814116/7676237)，以及这篇[跳表与 B+ 树的对比文章](https://ost.51cto.com/posts/11398)。
