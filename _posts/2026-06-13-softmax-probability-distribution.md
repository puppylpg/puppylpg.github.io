...
| 温度参数 | 为什么温度能让分布变平或变尖？为什么叫温度？ | 不同温度下的概率柱状图 |
| 知识蒸馏 | 温度在蒸馏里有什么用？ | soft target 对比 |
...
理解了 temperature 对分布形状的影响之后，再看它为什么叫“温度”就更自然了。这个名字来自统计力学里的玻尔兹曼分布。

这里不用深究物理公式，只需要记住它借来的那个直觉：

> 温度越高，系统越容易发生波动，粒子越不只停在最稳定、最常见的位置；那些原本很少被访问到的位置，也更可能被“撞上”。

这个直觉搬到 Softmax 里，就变成：

- **低温**：系统更“冷”，更倾向于待在最稳定的位置；对应到 Softmax，就是更倾向于最高 logit，分布更尖锐。
- **高温**：系统更“热”，波动更强，不常出现的位置也更可能被访问；对应到 Softmax，就是低分选项也能分到更多概率，原本不容易被选中的位置也更可能被映射上。

也可以用一个更口语化的比喻来记：

> 温度越低，分布越冷静，越忠于最高分；温度越高，分布越躁动，原本低概率的选项也越有机会被选中。

如果换成社会流动性的类比，低温像一个更固化的秩序：头部优势被放大，靠后的选项很难翻身。高温像一个更动荡的秩序：原本靠后的选项也更可能被抽中。当然，这只是帮助记忆 Softmax 的比喻，现实社会远比这个数学模型复杂。

这也是为什么温度参数常出现在生成模型采样里：调整温度可以控制输出更保守还是更多样。

## 知识蒸馏里的温度

temperature 还有一个很重要的场景：知识蒸馏。

知识蒸馏的目标是：让一个较小的学生模型学习一个较大的教师模型。直觉上，学生模型不只是学习“正确答案是哪一个”，还要学习教师模型对其它选项的判断。

比如一张图的正确类别是“猫”。普通标签只会告诉学生模型：

$$
[1,\ 0,\ 0,\ 0]
$$

这表示“猫”是正确答案，其它类别都错。但教师模型的输出可能是：

$$
[0.82,\ 0.12,\ 0.04,\ 0.02]
$$

这就多了一层信息：教师模型虽然最相信“猫”，但它也觉得“虎”比“汽车”更像。这些非最高类别里的相对关系，通常被称为 **dark knowledge**。

问题是，如果教师模型太自信，Softmax 输出会非常尖锐：

$$
[0.98,\ 0.01,\ 0.006,\ 0.004]
$$

这时非最高类别都接近 $$0$$，学生模型几乎看不到它们之间的差别。提高 temperature 后，教师模型的输出会被“软化”：

$$
[0.65,\ 0.20,\ 0.10,\ 0.05]
$$

这个分布仍然保留“猫”最高，但把其它类别之间的细微差异显露出来了。学生模型学到的就不只是“猫是对的”，还包括“猫和虎更接近，猫和汽车差得远”。

所以在知识蒸馏里，温度参数的作用可以概括为：

> 提高温度，让教师模型的概率分布变软，把原本被压到接近 $$0$$ 的次优类别关系暴露出来，方便学生模型学习。
...
- temperature 借用了统计力学里的温度概念：低温让分布更尖锐，高温让分布更平滑。
- 在知识蒸馏里，提高 temperature 可以软化教师模型的输出，让学生模型看到非最高类别之间的细微关系。
