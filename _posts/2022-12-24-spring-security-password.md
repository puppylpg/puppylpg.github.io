...
tags: [spring-security, password, bcrypt, password-encoder]
description: "梳理 Spring Security 中密码存储、bcrypt 强度选择和 DelegatingPasswordEncoder 的兼容设计。"
---

Spring Security 主要解决认证和授权问题，而用户名密码认证仍然是最常见、也最容易被低估的一环。密码存储不是简单地“加密后放进数据库”，它涉及单向散列、盐、计算成本、历史算法兼容和未来算法迁移。

本文从密码存储的演进讲起，再看 bcrypt 的工作因子选择，最后落到 Spring Security 的 `PasswordEncoder` 和 `DelegatingPasswordEncoder` 设计。

1. Table of Contents, ordered
{:toc}

文章的主线是：**密码不能可逆存储；密码验证要故意变慢；系统还要长期兼容已经落库的旧 hash**。

密码存储方案的演进，本质上是在提高攻击者批量试错的成本：
...
timeline
    ...
    One-way hash
        : 无法直接反解
        : 但可被 rainbow table 批量命中
    Salted hash
        : 每个密码加入随机 salt
        : 彩虹表失效
    Adaptive one-way function
...
当然，攻击者的机器和业务系统的机器不一样，所以它未必也需要 1s 才能验证一次，但至少不会像普通 hash 一样一秒计算十亿次。
...
# bcrypt
Spring Security 支持的 bcrypt 算法，就是一种 adaptive one-way function。
...
# `PasswordEncoder`
Spring Security 对密码验证的抽象是`PasswordEncoder`接口，它只有`encode(CharSequence rawPassword)`和`matches(CharSequence rawPassword, String encodedPassword)`两个接口，没有 decode，因为它处理的是 one-way function。

## `DelegatingPasswordEncoder`
Spring Security 5.0 之前的默认`PasswordEncoder`是`NoOpPasswordEncoder`，其实就是明文存储。显然，后来使用`BCryptPasswordEncoder`作为默认`PasswordEncoder`更合理。**但是 Spring Security 不能直接把默认 encoder 改成`BCryptPasswordEncoder`，否则应用一升级 Spring Security，数据库里已经存储的密码 hash 就可能全部无法验证。**
...
所以 Spring Security 引入了`DelegatingPasswordEncoder`：
...
[Spring Boot CLI](https://docs.spring.io/spring-security/reference/features/authentication/password-storage.html#authentication-password-storage-boot-cli)有一个方便的功能：使用 bcrypt 生成一个字符串的 hash。
...
```
这个命令的存在也说明，密码编码不是边缘工具，而是 Spring Security 使用链路里非常基础的一步。
...
# 感想
这套设计最关键的地方不是 bcrypt 本身，而是把“安全算法会升级”当成长期事实来处理。`DelegatingPasswordEncoder`用前缀保存算法标识，让新密码走新算法、旧密码继续可验证，是一套很务实的演进方案。
