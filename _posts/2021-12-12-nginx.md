...
categories: [nginx]
tags: [nginx]
description: "从 master/worker 架构、server/location 匹配到静态资源与反向代理，系统整理 Nginx 配置。"
---

之前因为折腾小服务器，好好部署了一下nginx：
- 在[折腾小服务器 - netdata与nginx](/life/2021/12/08/vps-netdata-nginx/)中，介绍了nginx如何作为反向代理将请求转发给其他服务、启用basic认证、http转https；
- 在[折腾小服务器 - nginx与https](/life/2021/12/11/vps-nginx-https/)中，介绍了怎么给nginx开启https，进行加密通信；
...
```mermaid
flowchart TD
    Master["master process<br/>读配置、管理 worker"] --> Worker1["worker process"]
    Master --> Worker2["worker process"]
    Request["HTTP request"] --> Listen["listen ip:port"]
    Listen --> ServerName["server_name / Host header"]
    ServerName --> Location["location 匹配 URI"]
    Location --> Static["root / alias<br/>静态文件"]
    Location --> Proxy["proxy_pass<br/>反向代理后端"]

    style Master fill:#e3f2fd,stroke:#1976d2
    style Location fill:#fff3bf,stroke:#f59f00
    style Proxy fill:#e8f5e9,stroke:#2e7d32
```

# 基础操作
- install：[Installing nginx](http://nginx.org/en/docs/install.html)
- 新手教程：[Beginner's Guide](http://nginx.org/en/docs/beginners_guide.html)
