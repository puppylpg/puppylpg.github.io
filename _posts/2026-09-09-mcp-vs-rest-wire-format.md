...
客户端先从服务端发现操作，再发起调用，这个思路在 REST 中也有对应设计。前文用来对比的是常见的 HTTP JSON API；REST 本身还包含超媒体约束：**资源响应除了提供数据，也提供后续可用操作的链接或表单。** 这种设计称为 **HATEOAS（Hypermedia as the Engine of Application State，超媒体作为应用状态的引擎）**，在 [Richardson 成熟度模型](https://martinfowler.com/articles/richardsonMaturityModel.html#level3)中对应 Level 3。它在大模型出现之前，就已用于减少客户端对服务端实现细节的依赖。

### 从订单数据到当前可用的操作

查询一个待支付订单，响应可以同时带上支付和取消入口。下面使用 `_links` 风格展示响应 body；HATEOAS 是设计约束，并没有规定所有 API 都必须采用这一种 JSON 格式：
...
`pay` 和 `cancel` 是本例约定的业务关系名。客户端预先理解它们分别代表支付和取消，知道怎样展示按钮、怎样处理交互；请求使用什么 HTTP 方法、携带哪些字段，也需要通过双方理解的媒体类型、操作描述或关系约定明确，单独一个 `href` 不包含全部调用信息。

服务端根据订单状态和用户权限等条件决定返回哪些入口。订单允许取消，就返回 `cancel`；不允许取消，就不返回它。客户端据此展示或隐藏取消按钮。[Spring HATEOAS 教程](https://spring.io/guides/tutorials/rest/)用订单的可取消、可完成操作演示了同样的分工。

### 服务端执行条件判断，客户端保留动作的含义

这种设计把一部分 `if-else` 集中到了服务端。两种客户端实现的差别可以这样理解：

| 事项 | 客户端自行判断 | 服务端提供操作入口 |
|---|---|---|
| 按钮显示条件 | 根据订单状态等条件判断 | 检查有无 `cancel` |
| 请求地址 | 按约定拼接 URL | 使用返回地址 |
| 动作含义与交互 | 客户端预先理解并实现 | 客户端仍需理解并实现 |
| 实际选择 | 用户或既定策略决定 | 用户或既定策略决定 |

**服务端执行判断逻辑，下发当前可用操作的结果。** 比如取消条件从“未支付”变为“未支付且未进入审核流程”，客户端仍然按 `cancel` 是否出现来控制按钮，无需同步实现新增的业务条件。
...
sequenceDiagram
    ...
    C->>S: 查询订单
    S->>S: 按订单状态和权限计算可用操作
    S-->>C: 订单数据与 pay、cancel 入口
    C->>U: 展示已知的支付、取消按钮
    U->>C: 选择取消
    C->>S: 按约定向 cancel 地址发起请求
    S->>S: 重新检查条件并执行取消
    S-->>C: 已取消的订单与当前可用入口
    C->>U: 更新界面，移除支付和取消按钮
```

这里委托给服务端的是**“此刻允许哪些动作”的判断**。实际是否执行、何时执行，仍由用户或客户端策略选择；入口存在也不保证稍后的请求一定成功，服务端执行时仍要检查权限和最新状态。

对于自动程序，`cancel` 的语义需要事先通过代码、媒体类型或共享约定建立。一个只认识支付和取消的程序，不会因为响应新增 `refund` 就自动学会退款策略。对于通用交互客户端，理解业务的人也可以是用户：浏览器理解链接和表单的处理规则，人理解页面中的支付、取消含义并作出选择。[Fielding 对超媒体的解释](https://roy.gbiv.com/untangled/2008/rest-apis-must-be-hypertext-driven)明确保留了这种共同语义的前提。

### 同一种分工，还能减少地址、导航和表单的重复实现

服务端多提供操作所需的信息，客户端就可以少依赖一些具体实现。订单按钮只是其中一个场景：

| 场景 | 服务端提供的信息 | 客户端减少的工作 |
|---|---|---|
| 地址调整 | `pay` 关系下的实际 URL | 拼接和硬编码路径 |
| 顺序分页 | 下一页的 `next` 链接 | 计算页码、构造游标 |
| 关联资源导航 | 客户、物流、发票的链接 | 根据 ID 推导资源路径 |
| 通用表单 | 字段、约束与提交方式 | 单独实现基础表单交互 |

例如，[Spring Data REST 的分页响应](https://docs.spring.io/spring-data/rest/reference/paging-and-sorting.html)提供 `next`、`prev` 关系；只负责顺序翻页的客户端可以跟随链接，而不自行计算下一页地址。表单则需要比 `_links` 更丰富的描述：[HTML 表单](https://html.spec.whatwg.org/multipage/forms.html)已经定义字段和提交规则，浏览器可以展示服务端新增的标准控件，用户负责填写。仅返回一个链接，并不会自动获得完整表单的能力。

这些收益依赖双方保持共享语义。地址或可用条件变化时，客户端可以少修改；业务动作的含义、所需输入或结果结构改变时，仍需检查兼容性。

### 与大模型使用 MCP 对照：动作的含义由谁理解

HATEOAS 和 MCP 都能让调用方在运行时发现操作，但典型的传统超媒体客户端与大模型应用，理解业务的方式不同：

| 对比点 | 传统 HATEOAS 客户端 | 大模型应用使用 MCP |
|---|---|---|
| 发现的内容 | 资源响应中的链接、表单<br>及当前可用操作 | 工具名称、描述、输入 schema<br>及可选的输出 schema |
| 动作的理解 | 程序预先实现语义；<br>通用界面可由用户理解 | 模型结合描述、schema<br>和用户目标解释用途 |
| 下一步选择 | 用户或预先编写的策略 | 模型提出调用；<br>Host 按策略组织执行 |
| 服务端判断 | 按资源状态等提供操作；<br>执行时校验条件 | 提供工具集合；<br>执行时校验业务条件 |

以本文的 `memo_list_memos` 为例，Host 可以把工具说明和 `inputSchema` 交给模型。模型结合“查找最近的公开笔记”这一目标，选择工具并生成参数，从而减少为每一种业务工具单独编写选择分支的需要。**模型解释工具描述，MCP Client 组织和传输协议消息，Host 管理调用过程，Server 执行业务。** 这些角色不能混成一个“会理解所有接口的 Client”；官方[架构说明](https://modelcontextprotocol.io/docs/2026-07-28/learn/architecture)区分了 Host 和 Client 的职责。

**工具目录的可发现性，也不等于具体业务操作的可执行性。** 即使 `tools/list` 包含 `cancel_order`，也不能据此认定订单 `123` 当前允许取消；仍需要业务结果、工具约定或执行时校验来确定。目录可以按请求的授权范围提供不同工具，但它并不天然等同于某个订单响应中的可用操作集合。

MCP 的[工具规范](https://modelcontextprotocol.io/specification/2026-07-28/server/tools#user-interaction-model)允许模型根据上下文选择工具，同时也明确不强制特定交互方式。传统代码可以调用 MCP，大模型也可以使用 REST 或超媒体 API。**模型带来的变化，是可以借助自然语言描述和上下文理解、选择动作；协议负责让操作及其契约可被获取和调用，并不保证业务判断正确。**
