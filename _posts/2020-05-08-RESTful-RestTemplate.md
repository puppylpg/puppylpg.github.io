...
description: "从 REST 资源表述、HTTP 方法、Content-Type/Accept 到 RestTemplate 的请求响应转换链路。"
---
...
```mermaid
sequenceDiagram
    participant Client as RestTemplate Client
    participant Converter as HttpMessageConverter
    participant Server as Spring MVC Controller

    Client->>Converter: Java Object + Content-Type
    Converter->>Server: 序列化为 JSON/XML/Protobuf 请求体
    Server->>Converter: @RequestBody 反序列化为 Java Object
    Server-->>Converter: Controller 返回 Java Object
    Converter-->>Client: 按 Accept 序列化响应体
    Client->>Converter: 反序列化 response body
```
...
# http post 格式
POST 的语义可以参考 [MDN 对 HTTP POST 的说明](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST)。
