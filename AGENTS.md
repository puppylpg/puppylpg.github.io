...
对话、追问和修改意见只是文章的素材来源，不是读者拥有的阅读上下文。默认读者没有参与任何前置对话、没有提出过问题，也不知道作者曾纠正过什么误解。新增或重写正文时必须：

- 按知识依赖顺序组织内容：先建立对象、背景和目标，再介绍组件与关系、操作流程、内部原理，最后收束边界和结论。具体文章可以调整层次，但任何概念都应在依赖它的叙述之前出现。
- 把对话中的追问答案拆解后放回对应概念的逻辑位置，改写成陈述式正文；不要把回答原样追加成对“不可见问题”的答复。只有用户明确要求 FAQ 时才保留问答结构。
- 禁止在没有明确前文铺垫时使用“真正的问题是”“容易误解”“先停一下”“答案是”“还剩最后一步”等答疑式过渡。反问只有在正文已经自然建立该问题时才可使用。
- 完稿后执行冷读检查：只读标题和每节首句，确认未参与对话的读者也能知道这一节在讲什么、为什么此时出现、依赖哪些前置概念；任一项不清楚就重新排序或补齐上下文。
