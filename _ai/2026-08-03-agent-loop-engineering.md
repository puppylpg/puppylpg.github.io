...
契约描述的是循环必须承担的职责，真正落地时还需要把职责装进具体部件。[小林 coding 对 Loop Engineering 的实践拆解](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247558717&idx=1&sn=1b7e118a44b39ff3fff33c392f784897)把常见实现归纳为自动化、Worktree、Skill、Connector、Sub-agent 和外部记忆。这份清单的价值不在于重新定义 Loop，而在于把抽象契约翻译成可以配置和组合的工程原语。

| 工程部件 | 主要承接的职责 | 能解决什么 | 不能替代什么 |
|---|---|---|---|
| 自动化 | 触发 | 按时间或事件唤醒任务 | 目标、验收和停止逻辑 |
| Git Worktree | 动作隔离与状态所有权 | 让并行 Agent 在独立目录和分支中修改代码 | API 契约协调、合并与集成验证 |
| Skill | 上下文 | 固化相对稳定的规范、流程和领域知识 | 本轮进度与历史结果 |
| Connector / MCP | 动作 | 连接工单、代码托管、消息和其他外部系统 | 权限边界与高风险操作审批 |
| Sub-agent | 动作或反馈 | 分工探索、并行执行或引入独立评估视角 | 测试、策略规则等客观证据 |
| 外部记忆 | 状态 | 在文件、数据库或任务系统中保存进度、证据和下一步 | 对信息正确性与时效性的验证 |

其中，Skill 与外部记忆最容易混淆。**Skill 保存“通常应该怎样做”，状态保存“这一次已经做到哪里”**：前者应随工程规范演进，后者则在每轮执行后更新。把动态进度塞进 Skill 会让规则不断污染，把稳定规范只留在会话记忆里又会让每次运行重新推导。

这些部件也不是每个 Loop 都必须一次配齐。只读巡检可能不需要 Worktree，确定性检查也不需要另起一个 Review Agent；最小闭环只要有清晰目标、触发方式、可验证反馈、必要状态和停止条件，就可以先运行。任务开始并行、跨系统或跨会话之后，再按暴露出来的风险增加隔离、连接器和独立评估，能够避免为了追求完整清单而制造新的复杂度。
...
- **理解债：** Agent 产出代码的速度超过团队理解代码的速度，仓库里的现实会逐渐脱离维护者脑中的系统模型；代码即使通过检查，故障定位、架构演进和责任交接也会越来越困难。
- **认知投降：** 人工审批仍然存在于流程图中，审阅者却不再形成独立判断，只是默认接受 Agent 的结论，最终让人工闸门退化成形式上的“同意”按钮。

理解债（Comprehension Debt）和认知投降（Cognitive Surrender）不能靠再增加一个 Review Agent 自动消除。高风险改动仍应要求责任人阅读关键 diff、复述设计理由、核对验证证据与需求之间的关系，并把重要决策写入可以追溯的记录。目标不是让人重新逐行包办所有工作，而是确保人的批准仍然代表真实理解和独立判断。

因此，Loop Engineering 不是“让 Agent 永远运行”的技巧，而是**限制循环怎样运行，同时保留人类理解力**的工程：为目标设边界，为动作设权限，为状态设所有权，为验证保留独立证据，为失败准备停止和升级路径，也为高风险结果保留不能外包的判断责任。
...
补充材料把 Worktree、Skill、Connector、Sub-agent 和磁盘记忆摆到同一张实现货架上，又进一步提出理解债与认知投降，使讨论从“循环需要哪些控制机制”延伸到“自动化之后人是否仍然理解并负责”。前者便于把抽象设计映射到现成工具，后者则提醒读者，人工节点是否存在和人工判断是否真实发生，是两件不同的事。
