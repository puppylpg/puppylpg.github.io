...
categories: [life, vps, linux, swap]
tags: [linux, vps, swap, memory, oom, ps, smem]
description: "给 1G 内存 VPS 扩容 swap，并记录 swapoff、dd 被 OOM killer 教训后如何用 ps/smem 排查内存占用。"
math: true
mermaid: true
---
...
[升级 swap 的流程](https://arcolinux.com/how-to-increase-the-size-of-your-swapfile/)本身比较简单。需要注意 swap 文件的名字不是固定的，我的是`/swap`：
```bash
$ sudo swapoff -a
# 2G
...
```mermaid
flowchart LR
    Old[旧 swap: 512M] --> Off[swapoff -a]
    Off --> Create[dd 创建新的 /swap]
    Create --> Format[mkswap /swap]
    Format --> On[swapon /swap]
    On --> New[新 swap: 2G]

    style Off fill:#fff3bf,stroke:#d9480f
    style New fill:#e8f5e9,stroke:#2b8a3e
```
...
```mermaid
flowchart TD
    SwapUsed[已有页面在 swap 里] --> Swapoff[执行 swapoff]
    Swapoff --> Move[内核尝试把页面搬回 RAM]
    Move --> Enough{RAM 够不够?}
    Enough -- 够 --> Done[swapoff 成功]
    Enough -- 不够 --> OOM[OOM killer 出手]
    OOM --> Killed[swapoff 或其他进程被 kill]

    style OOM fill:#ffe3e3,stroke:#c92a2a
    style Move fill:#fff3bf,stroke:#d9480f
```
...
这里的经验是：**`bs`不是越大越牛，内存小的时候它就是一个临时缓冲区炸弹**。在 1G RAM 的小 VPS 上，`bs=128M count=16`比直接把`bs`拉到 1G 老实多了。
...
| 指标 | 含义 | 适合回答的问题 |
|------|------|----------------|
| RSS | 进程实际占用的物理内存，不含 swap | 谁正在吃 RAM |
| VSZ | 虚拟地址空间，不等于真实内存占用 | 进程申请了多大虚拟空间 |
| Swap | 被换出的页面大小 | 谁被塞进 swap 了 |
| Load | 系统负载 | 是不是内存不足拖到磁盘疯狂换页 |
...
`ps`和`smem`配合起来，视角大概是这样：

```mermaid
flowchart LR
    Processes[进程列表] --> PS[ps --sort -rss]
    Processes --> Smem[smem -s swap -r]
    PS --> RAM[定位 RAM 大户]
    Smem --> Swap[定位 swap 大户]
    RAM --> Action[决定关哪个服务/调哪个配置]
    Swap --> Action

    style Action fill:#e8f5e9,stroke:#2b8a3e
```
