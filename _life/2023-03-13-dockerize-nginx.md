...
### 2026-08-31：升级到 Memos 0.30.0

最初的启动命令使用 `neosmemo/memos:stable`，这个 tag 会随稳定版本更新，但已经创建的容器不会自动换成新镜像。这次检查容器内版本仍是 `0.29.1`，因此先备份数据，再把镜像固定升级到 `neosmemo/memos:0.30.0`。
...
表示 Docker 创建并管理一个名为 `memos_data` 的 **named volume**，再把它挂到容器内的 `/var/opt/memos`。容器只是运行程序，删除和重建容器不会删除这个独立的数据卷；新容器继续挂载同名 volume，就能读到原来的数据。

```mermaid
flowchart LR
    container[Memos 容器] -->|挂载到 /var/opt/memos| volume[memos_data volume]
    volume --> db[(memos_prod.db<br/>SQLite 数据库)]
    volume --> wal[SQLite WAL / SHM]
    volume --> files[附件与缩略图缓存]

    classDef app fill:#ffe5ec,stroke:#d85a7f,color:#3b1723;
    classDef data fill:#f4f4f5,stroke:#9ca3af,color:#1f2937;
    class container app;
    class volume,db,wal,files data;
```

可以从 Docker 查询宿主机上的实际目录：

```bash
docker volume inspect memos_data | jq -r '.[0].Mountpoint'
# /var/lib/docker/volumes/memos_data/_data
```

升级前实测 volume 约为 `233 MiB`，其中 `memos_prod.db` 是约 `138 MiB` 的 SQLite 主数据库，此外还有 `memos_prod.db-wal`、`memos_prod.db-shm` 和约 `95 MiB` 的 `.thumbnail_cache`。Memos 的[官方备份文档](https://usememos.com/docs/operations/backup-restore)还把本地附件目录列为需要备份的内容，因此不要只假设所有数据永远都在一个 `.db` 文件里，**备份整个 volume 最稳妥**。

需要特别注意：`docker rm memos` 只删除容器，不会删除 named volume；`docker volume rm memos_data` 或清理未使用 volume 的命令才可能真正删除持久化数据。

#### 升级前完整备份

SQLite 使用 WAL 模式时，直接复制一个正在写入的 `memos_prod.db` 可能拿不到一致快照。手动升级时，最简单可靠的方案是短暂停止 Memos，再把整个 volume 打包：
...
这次生成的升级前备份为 `/home/pichu/backups/memos/memos-data-pre-0.30.0-20260831-211147.tar.gz`，压缩后约 `231 MiB`。这种同机备份适合防范升级失败和误操作；如果还要防范 VPS 磁盘损坏或整机丢失，应再把备份同步到另一台机器或对象存储，并定期做一次恢复演练。

日常不停机备份可以使用 SQLite 自带的 `.backup` 命令，它能正确处理 WAL；如果附件使用本地文件存储，还要同时备份附件目录。不能只定时 `cp memos_prod.db`。

#### 使用固定版本重建容器

先拉取并核对目标镜像，然后删除已经停止的旧容器，以原来的 volume 和反向代理环境变量启动新容器：
...
`0.30.0` 在没有配置 `MEMOS_INSTANCE_URL` 时默认使用 private mode：匿名用户不能读取 memo，登录用户仍可正常使用。这次保留了这个默认行为。如果以后需要恢复匿名访问，可以给容器增加 `MEMOS_INSTANCE_URL=https://memos.puppylpg.top` 后重新创建。

最后从容器版本、启动日志、HTTPS 入口和匿名访问边界四个方面验收：
...
本次结果是版本返回 `0.30.0`、启动日志显示 `Access mode: private`、HTTPS 返回 `200`，匿名读取 memo API 返回 `401 authentication required`。数据库迁移会在新版本启动时自动执行；如果需要回退，必须恢复升级前的备份，不能让旧版程序直接读取已经被新版迁移过的数据库，具体边界见 Memos 的[升级说明](https://usememos.com/docs/operations/upgrade)。
