...
flowchart LR
    ...
    subgraph host[VPS 宿主机]
        ...
        subgraph docker[Docker bridge 网络]
            ...
            apps[业务容器<br/>wedding / ttq / memos / portainer / dailytxt / v2ray]
        end
    end
...
| `v2ray5` | `ghcr.io/v2fly/v2ray:latest-extra` | 10087 | 代理服务（v5.49.0 当前版） |
...
flowchart TB
    subgraph host["VPS 宿主机"]
        ...
        subgraph network["Docker bridge 网络<br/>子网 172.17.0.0/16"]
            direction LR
            proxy["nginx-proxy<br/>172.17.0.5"]
            ttq_node["ttq<br/>172.17.0.3"]
            memos_node["memos<br/>172.17.0.7"]
            v2ray_node["v2ray<br/>172.17.0.2"]
            v2ray5_node["v2ray5<br/>172.17.0.6"]
            other["... 其他容器"]
        end
    end
    ...
    class proxy,ttq_node,memos_node,v2ray_node,v2ray5_node,other app;
    class network net;
```

在这台 VPS 上，确实只有一个 `bridge` 网络，9 个运行中的容器全部接在里面：

| 容器名 | 实际 IP | 暴露端口 | 外部映射 |
|--------|---------|----------|----------|
| `nginx-proxy` | `172.17.0.5` | 80, 443 | `0.0.0.0:80→80`, `0.0.0.0:443→443` |
| `nginx-proxy-acme` | `172.17.0.8` | — | 无 |
| `ttq` | `172.17.0.3` | 80 | 无 |
| `memos` | `172.17.0.7` | 5230 | 无 |
| `v2ray` | `172.17.0.2` | 10087 | 无 |
| `v2ray5` | `172.17.0.6` | 10087 | 无 |
| `dailytxt` | `172.17.0.10` | 80 | 无 |
| `wedding` | `172.17.0.4` | 80 | 无 |
| `portainer` | `172.17.0.9` | 9443 | 无 |

### 为什么端口相同却不冲突？

`v2ray` 和 `v2ray5` 内部都用 `10087` 端口，但由于它们拥有**不同的 IP 地址**，就像两个不同手机都开了微信，完全不会冲突：

```
172.17.0.2:10087   ← v2ray (v4)
172.17.0.6:10087   ← v2ray5 (v5)
      ↑                ↑
   不同 IP          不同 IP
```
...
只有 `nginx-proxy` 把端口映射到了宿主机（`-p 80:80 -p 443:443`），其他 8 个容器都是**只暴露内部端口**，外面直接访问不到。外部流量必须先到达 `nginx-proxy`，再由它按规则转发到对应容器的内网 IP。

```mermaid
flowchart TB
    subgraph bridge["Docker bridge 网络 (172.17.0.0/16)"]
        direction TB
        proxy["nginx-proxy<br/>172.17.0.5:80/443"]
        acme["acme<br/>172.17.0.8"]
        ttq_node["ttq<br/>172.17.0.3:80"]
        memos_node["memos<br/>172.17.0.7:5230"]
        v2ray_node["v2ray<br/>172.17.0.2:10087"]
        v2ray5_node["v2ray5<br/>172.17.0.6:10087"]
        portainer_node["portainer<br/>172.17.0.9:9443"]
        dailytxt_node["dailytxt<br/>172.17.0.10:80"]
        wedding_node["wedding<br/>172.17.0.4:80"]
    end
    ...
    proxy -->|"WebSocket<br/>/v2ray5"| v2ray5_node
    ...
    class users user;
    class proxy gateway;
    class acme,ttq_node,memos_node,v2ray_node,v2ray5_node,portainer_node,dailytxt_node,wedding_node app;
```
...
| `v2ray5` | `puppylpg.top` | `10087` | — | `/v2ray5` | `puppylpg.top` |
...
- `VIRTUAL_PATH`：当多个容器共享同一个域名时，按路径区分（v2ray 和 v2ray5）。
...
对于 v2ray，同一个域名 `puppylpg.top` 按 `location /v2ray` 和 `location /v2ray5` 拆分到两个不同的 upstream，分别打到 `172.17.0.2:10087` 和 `172.17.0.6:10087`。
...
v2ray 使用 WebSocket 传输层，nginx-proxy 负责处理 WebSocket 的 `Upgrade` 和 `Connection` header 转发。由于容器声明了 `VIRTUAL_PATH=/v2ray`，nginx-proxy 自动生成 `location /v2ray` 块，将匹配到该路径的请求转发到 `172.17.0.2:10087`。
...
flowchart LR
    ...
    subgraph new[v5 当前服务]
        direction TB
        path[/Path: /v2ray5/]
        v5[v2ray5<br/>v5.49.0<br/>10087/tcp]
...
v2ray5 与 v2ray 的唯一外部差异是 WebSocket 路径不同（`/v2ray5` vs `/v2ray`），内部差异是 alterId（0 vs 64）和版本（v5 vs v4）。nginx-proxy 自动生成 `location /v2ray5` 块，将请求转发到 `172.17.0.6:10087`。当前 v5 服务使用 `ghcr.io/v2fly/v2ray:latest-extra` 镜像，运行版本为 `V2Ray 5.49.0`。

# V2Ray v4→v5 升级分析

## 为什么不能直接升级

直接从 `v4.23.4` 升级到 `v5.49.0` 会遇到两个**硬性不兼容**：

### 1. 启动命令格式变更

v5 的 Docker 镜像改为子命令式 CLI，要求显式传入 `run` 子命令。当前使用的 GHCR 镜像入口是 `/opt/v2ray/bin/v2ray`：

| 版本 | 正确启动命令 |
|------|-------------|
| v4.x | `v2ray -config=/etc/v2ray/config.json` |
| v5.x | `/opt/v2ray/bin/v2ray run -config=/etc/v2ray/config.json` |

如果沿用 v4 的 `docker run` 命令（包含 `v2ray -config=...`），最终执行的命令会变成：

```
v2ray v2ray -config=/etc/v2ray/config.json
```
...
2. **部署新实例**：创建 `v2ray5` 容器（v5.49.0），使用新路径 `/v2ray5` 避免冲突。
3. **修正启动命令**：使用 `run -config=...` 而非直接传 `-config=...`。
...
另一个细节是内部端口：v4 和 v5 容器内部都使用 `10087`，但由于 Docker 网络隔离（`172.17.0.2` 与 `172.17.0.6`），不会冲突。对外暴露的差异仅体现在 nginx-proxy 的 path 路由上——`location /v2ray` 转发到 `172.17.0.2:10087`，`location /v2ray5` 转发到 `172.17.0.6:10087`。
