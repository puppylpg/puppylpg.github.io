...
description: "在现有 Radarr 下载管线上接入 Jellyfin、Jellyseerr、Vaultwarden 与 Homepage，形成从点播、下载、播放到密码管理和统一入口的完整树莓派 homelab。"
math: true
mermaid: true
---
...
首次访问 `http://raspberrypi.local:5055` 时选择 Jellyfin 登录并使用 Jellyfin 管理员账号完成初始化，然后在设置中关联 Radarr：Host 填 Compose 服务名 `radarr`，端口填 `7878`，API Key 从 Radarr 设置页复制。搜索和海报同样依赖 TMDB，因此容器也需要通过 V2Ray 代理访问外网。
...
整合后的 homelab 可以分成三层：Jellyseerr、Homepage 和 Bitwarden 客户端直接面向使用者；Jellyfin、Radarr 等应用提供播放、下载和管理能力；Caddy、Vaultwarden、AdGuard Home 与 V2Ray 提供入口、凭据和网络支撑。各组件保留独立职责，通过明确的接口连接，而不是由一个服务包办全部功能。
