...
**路由策略仍然建议选择"绕过大陆"（Bypass LAN & China）**。虽然 Zero Omega 已经在浏览器层做了精分流，但 v2ray 端保留路由策略有两层价值：

1. **兜底防漏**：万一 Zero Omega 漏了某个国内 CDN（如 `*.bdstatic.com`、`*.alicdn.com`），被误发到 v2ray，"绕过大陆"会把它纠正回直连。
2. **命令行刚需**：Git、Docker、npm 这些无脑走代理的工具，全靠 v2ray 这层路由策略来避免国内流量绕路。

| v2ray 路由策略 | Git 克隆 GitHub | Git 克隆 Gitee | 效果 |
|---------------|----------------|---------------|------|
| **绕过大陆** | 走代理出国 ✅ | 直连 ⚡ | 各走各路 |
| **全局** | 走代理出国 ✅ | 也走代理出国 🐢 | 国内流量被拖累 |

因此，**"不改变系统代理"只是改变流量怎么进入 v2ray，进入后的路由规则依然生效**，保持"绕过大陆"是最稳妥的选择。
