...
tags: [linux, vps, nginx, network, https, tls, certbot, letsencrypt]
description: "给 Nginx 上 HTTPS：从自签名证书、Let's Encrypt、Certbot 到 Wireshark 抓包验证 Basic Auth 明文问题。"
math: true
mermaid: true
---
...
# https
参考：[MDN 对 HTTPS 的说明](https://developer.mozilla.org/en-US/docs/Glossary/https)。
...
## 非对称加密
TLS 握手阶段会用到非对称加密能力。为什么？当然是因为开放的网络世界，对称加密没法做到安全分发密钥。但是非对称加密可以，因为只需要分发公钥就行了，而且每个人都可以知道，不需要保密。而私钥自己要保存好。之后就可以：公钥加密，私钥解密；反之亦然；私钥签名，公钥认证。真正传输大量数据时，TLS 通常会协商出对称密钥，靠对称加密来扛吞吐。
...
## TLS
参考：[MDN 对 TLS 的说明](https://developer.mozilla.org/en-US/docs/Glossary/TLS)。

现代浏览器都支持TLS，不然怎么处理https请求。

## digital certificate
参考：[MDN 对 digital certificate 的说明](https://developer.mozilla.org/en-US/docs/Glossary/Digital_certificate)。
...
- [证书验证流程科普](https://www.wosign.com/FAQ/faq2016-0309-03.htm)

还有一系列其他基础介绍的文章：
- [HTTPS 基础](https://www.wosign.com/FAQ/faq2016-0309-01.htm)
- [TLS 加密](https://www.wosign.com/FAQ/faq2016-0309-02.htm)
- [PKI（Public Key Infrastructure）公钥基础](https://www.wosign.com/basic/aboutPKI.htm)
...
sequenceDiagram
    ...
    participant Site as 网站 Nginx

    Browser->>Site: 发起 HTTPS 连接
    Site-->>Browser: 返回站点证书和公钥
    Browser->>Browser: 使用本地 CA 根证书验证证书链
    Browser->>Site: 使用协商出的密钥继续加密通信
```
...
参考：[Windows 信任自签名证书](https://cnzhx.net/blog/self-signed-certificate-as-trusted-root-ca-in-windows/)。

但是后来我发现一个免费帮网站做签名的CA，真是万分意外！下面一一介绍。

## 生成自签名的certificate
通过openssl生成证书和私钥：
- [How to generate a self-signed SSL certificate using OpenSSL](https://stackoverflow.com/questions/10175812/how-to-generate-a-self-signed-ssl-certificate-using-openssl?rq=1)
...
- [Certbot for Nginx on Debian](https://certbot.eff.org/instructions?ws=nginx&os=debianbuster)

### 安装snapd
第一步是在Debian上安装snapd：
- [Debian 安装 snapd](https://snapcraft.io/docs/installing-snap-on-debian)
...
Certbot 的工作可以理解成“自动帮你和 CA 跑 ACME 流程”：

```mermaid
flowchart TD
    Certbot[Certbot] --> ReadNginx[读取 Nginx server_name]
    ReadNginx --> Choose[选择要签发的域名]
    Choose --> Challenge[完成 ACME 域名验证]
    Challenge --> LE[Let's Encrypt 签发证书]
    LE --> Files[/fullchain.pem + privkey.pem/]
    Files --> Nginx[Nginx ssl_certificate 配置]
    Certbot --> Renew[定时自动续期]

    style LE fill:#e8f5e9,stroke:#2b8a3e
    style Files fill:#fff3bf,stroke:#d9480f
```
...
## 配置证书到nginx
nginx配置http文档：
- [Nginx HTTPS server 配置文档](http://nginx.org/en/docs/http/configuring_https_servers.html)
...
```mermaid
flowchart TD
    HTTP[http://netdata.puppylpg.xyz] --> Nginx80[Nginx listen 80]
    Nginx80 --> Redirect[307 跳转到 https]
    Redirect --> HTTPS[https://netdata.puppylpg.xyz]
    HTTPS --> Nginx443[Nginx listen 443 ssl]
    Nginx443 --> Netdata[反代到 Netdata]

    style Redirect fill:#fff3bf,stroke:#d9480f
    style Nginx443 fill:#e8f5e9,stroke:#2b8a3e
```
...
| 抓包视角 | HTTP | HTTPS |
|----------|------|-------|
| Host/Header | 可以直接看到 | HTTP 内容被 TLS 包住，看不到 |
| Basic Auth | Base64 后仍是明文等价物 | 被 TLS 加密保护 |
| Wireshark 过滤 | 可以按 `http.host` | 通常按 `ip.addr` + `tcp.port == 443` |
| 代价 | 简单、省资源 | 包更大，CPU 多一点加解密开销 |
