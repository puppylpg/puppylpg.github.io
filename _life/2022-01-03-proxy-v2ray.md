...
categories: [life, network, proxy, v2ray, nginx]
tags: [network, proxy, v2ray, vmess, websocket, tls, nginx]
description: "记录 Shadowsocks 端口异常后迁移到 V2Ray，并用 VMess over WebSocket over TLS over TCP + Nginx 伪装流量的折腾过程。"
math: true
mermaid: true
---

之前在[代理 - shadowsocks](/life/2021/11/09/proxy/)介绍了使用ss做代理的方式，部署简单，使用方便。但用了一个月之后，奇奇怪怪的事情发生了 ┓( ´∀` )┏
...
据说是被GFW检测到了：
- [shadowsocks-libev issue #1932](https://github.com/shadowsocks/shadowsocks-libev/issues/1932)

按照[这篇概述](https://haoel.github.io/)，找到了 [websocket + tls + nginx](https://guide.v2fly.org/advanced/wss_and_web.html) 的方案，把翻墙流量伪装成访问网站的流量，再使用 TLS 加密一波，相当难被发现。

整个迁移动机其实很简单：

```mermaid
flowchart TD
    SS[Shadowsocks 明端口] --> Blocked[端口疑似被识别/干扰]
    Blocked --> ChangePort[换端口临时恢复]
    ChangePort --> Risk[本质风险仍在]
    Risk --> V2Ray[V2Ray + VMess]
    V2Ray --> WSS[WebSocket + TLS + Nginx]
    WSS --> WebLike[看起来更像普通 HTTPS 网站流量]

    style Blocked fill:#ffe3e3,stroke:#c92a2a
    style Risk fill:#fff3bf,stroke:#d9480f
    style WSS fill:#e8f5e9,stroke:#2b8a3e
```
...
可以先用一个结构图理解 V2Ray 的“盒子”模型：

```mermaid
flowchart LR
    Prev[上一个节点] -->|outbound 协议| Inbound[V2Ray inbound]
    Inbound --> Router[路由/协议转换]
    Router --> Outbound[V2Ray outbound]
    Outbound -->|下一个节点 inbound 能理解的协议| Next[下一个节点]

    style Inbound fill:#e3f2fd,stroke:#1971c2
    style Outbound fill:#e8f5e9,stroke:#2b8a3e
    style Router fill:#fff3bf,stroke:#d9480f
```
...
```mermaid
sequenceDiagram
    participant Browser as 浏览器/系统应用
    participant Local as 本机 V2Ray inbound
    participant ClientOut as 本机 V2Ray outbound
    participant ServerIn as VPS V2Ray inbound
    participant ServerOut as VPS V2Ray outbound
    participant Target as 目标网站

    Browser->>Local: SOCKS/HTTP
    Local->>ClientOut: 路由后交给 outbound
    ClientOut->>ServerIn: VMess
    ServerIn->>ServerOut: 解包并路由
    ServerOut->>Target: Freedom 直连
    Target-->>Browser: 响应按原链路返回
```
...
以上更详细的介绍可以参考：
- [v2fly 大白话教程](https://guide.v2fly.org/)
- [v2fly 新手教程](https://www.v2fly.org/guide/start.html)
- [v2fly 配置文档](https://www.v2fly.org/config/overview.html)
- 有兴趣还可以看看 [v2ray 开发文档](https://www.v2fly.org/developer/intro/compile.html)

## 客户端
如果直接使用从[v2ray-core](https://github.com/v2fly/v2ray-core/releases)下载的v2ray作为client，用起来不是很方便。

之前买别人的代理的时候，用了基于v2ray的client软件：
- Windows 用的是 [v2rayN](https://github.com/2dust/v2rayN)；
- Android 用的是 [v2rayNG](https://github.com/2dust/v2rayNG)；
...
参考：[WebSocket + TLS + Web 配置教程](https://guide.v2fly.org/advanced/wss_and_web.html)。
...
- [websocket 的形象解释](https://www.zhihu.com/question/20215561)
...
整体配置主要就两步：
1. 配置v2ray客户端和nginx之间通过websocket + tls方式通信。nginx配置http可以参考[折腾小服务器 - netdata与nginx](/life/2021/12/08/vps-netdata-nginx/)，数字证书配置方式可以参考[折腾小服务器 - nginx与https](/life/2021/12/11/vps-nginx-https/)；
...
flowchart LR
    ...
    subgraph VPS[VPS]
        Nginx[Nginx 443]
        V2Server[V2Ray Server 127.0.0.1:10086]
    end

    Target[目标网站]

    Browser -->|SOCKS/HTTP| V2Client
    V2Client -->|VMess over WebSocket over TLS| Nginx
    Nginx -->|WebSocket 反代| V2Server
    V2Server -->|Freedom| Target

    style Nginx fill:#fff3bf,stroke:#d9480f
    style V2Server fill:#e8f5e9,stroke:#2b8a3e
```
...
## v2ray服务器配置
照着这个配就行了：
- [WSS and web 服务器配置](https://guide.v2fly.org/advanced/wss_and_web.html#%E6%9C%8D%E5%8A%A1%E5%99%A8%E9%85%8D%E7%BD%AE)
...
```
配置文件的格式参考:
- [V2Ray 配置文件格式](https://www.v2ray.com/chapter_02/01_overview.html)
...
| 层次 | 负责什么 | 本文配置 |
|------|----------|----------|
| 业务代理协议 | 认证用户、承载代理流量 | VMess |
| 传输伪装 | 看起来像普通 WebSocket 通信 | WebSocket |
| 加密与证书 | 避免中间人看到明文内容 | TLS |
| 基础传输 | 真正跑在公网 TCP 连接上 | TCP 443 |
...
> v2ray docker参考[Docker - 容器化](/life/2022/03/20/dockerize/)
...
# 和网站在一起
有一篇相当详细的上述配置类似的文章：
- [V2Ray + WebSocket + TLS + Web 相关配置文章](https://pincong.rocks/article/15493)
...
另一个建议就是不要使用`/v2ray`这种过于明显的路径，太扎眼。我不确定实际上 GFW 会不会特意针对这些特殊名字的路径进行侦测，但是我仔细一想，既然 websocket 流量已经经过 TLS 加密了，那数据包访问路径究竟是不是`/v2ray`，GFW 是看不到的。就算 GFW 主动使用`/v2ray`路径，nginx 已经配置了 HTTP 访问`/v2ray`路径返回 404，所以就算主动探测，返回 404 什么也证明不了。
...
# 其他
客户端还可以和服务器之间用以下协议通信：
- [伪装成 http](https://guide.v2fly.org/advanced/httpfake.html)
    + 这个很好玩的样子，和直接使用 http 还不一样，不是 vmess over http，而是 [vmess 伪装成 http](https://www.v2fly.org/config/transport.html#streamsettingsobject)
...
- [直接使用 websocket](https://guide.v2fly.org/advanced/websocket.html)
- [直接使用 http2](https://guide.v2fly.org/advanced/h2.html)
    + 据说 [http2 贼快](https://http2.akamai.com/demo)
- [http2 + tls + web](https://guide.v2fly.org/advanced/h2_tls_web.html)
    ...
    + nginx和v2ray之间使用的http2可以用`h2c`，他们之间因为是本地机器，不需要强制使用tls了：
        - [基于 http2 传输配置](https://www.v2ray.com/chapter_02/transport/h2.html)
        - [v2ray 新增 h2c 支持](https://v2ray.com/chapter_00/01_versions.html#20190712-v4200)
    + nginx早就支持http2了；
...
- [V2Ray 相关文章](https://steemit.com/cn/@v2ray/unzev)
