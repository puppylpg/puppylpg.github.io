...
- 用户明确要求发布到 Tech 时，目标改为 `_posts/`，分类为 `[tech, trends]`，构建命令增加 `--target tech`。
...
4. 在 `_ai/` 和 `_posts/` 中定位时间上最近的上一期 `*-ai-trends-YYYY-MM.md`，完整阅读其正文，并填写 `periodComparison`。对照当前 `capture.json` 与上一期文章中的图表标题、关键数据点和口径；没有证据时不得声称数值变化。
5. 保持每张图的 `order`、`section` 和 `originalTitle` 不变。
6. 对明确数字写 `direct:` 证据；对视觉判断写 `inference:` 证据；不得把相关性写成因果。

完成全部逐图解读后再归纳跨图趋势，不能先写结论再让单图迎合结论；最终文章展示时将结论和理由放到最前面。每个 section 的 `summary` 必须是这一组图的整体结论，不要只写“本节将观察什么”。术语缩写必须补充英文全称。

`periodComparison` 必须包含上一期标题、站内 URL、抓取日期、整体对比小结和 3–8 条变化。每条变化包含 `change`、`evidence`、`comparability`：前两项说明发生了什么及两期证据，后一项说明是同口径可比、仅方向可比，还是因口径切换不可直接比较。若仓库中确实没有上一期，写明基线缺失，并允许 `changes` 为空；不得拿来源页以外的第三方数据冒充上一期。
...
用户要求发布到 Tech 时增加 `--target tech`；默认省略该参数，仍写入 `_ai/`。

脚本生成：

- `_ai/YYYY-MM-DD-ai-trends-YYYY-MM.md`，或 Tech 目标下的 `_posts/YYYY-MM-DD-ai-trends-YYYY-MM.md`
- `assets/img/ai/ai-trends/ai-trends-YYYY-MM/*.png`
- `$RUN_DIR/data/blog-build.json`

文章结构固定为：数据来源与背景 → 整体趋势总结及判断依据 → 与上一期相比 → 方法与局限 → 数据范围和读图方法 → 带英文全称的术语 → 按来源 section 展开。每个 section 先给“本组结论”，再逐图解释；每张图包含中文标题、原始英文标题、图片、读图方式、图表观察、1–6 条关键模型或数据点、解读边界和可折叠证据。
...
- TOC 格式正确，来源是 Markdown 内联链接；整体总结、逐期差异和方法局限位于读图方法及单图之前，且每条总结都有判断依据。
- “与上一期相比”位于整体总结和方法局限之间；存在上一期时包含基线链接、3–8 条差异及逐条可比性说明。
...
- 沿用上一期结论却不做差异章节，或把不同指标口径的数值硬算成环比升降。
